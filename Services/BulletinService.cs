...
public sealed partial class BulletinService
{
    ...
    public async Task<List<BulletinTemplate>> LoadTemplatesAsync()
    {
        ...
        RemoveOutdatedPerMilitaryTemplateOverrides(saved);
        RemoveOutdatedBirthBenefitOverrides(saved);
        ...
        var result = builtIn.Select((x, index) => new BulletinTemplate
        {
            Name = x.Key,
            Text = x.Value,
            Category = ResolveTemplateCategory(x.Key),
    ...
    public IReadOnlyList<string> SuggestOptions(string? key, string? templateName = null)
    {
        ...
        if (normalized.Contains("CODIGOSAQUE", StringComparison.OrdinalIgnoreCase) || normalized.Contains("CODIGOATRASADO", StringComparison.OrdinalIgnoreCase) || (template.Contains("AUXILIOALIMENTACAO", StringComparison.OrdinalIgnoreCase) && normalized.Contains("CODIGO", StringComparison.OrdinalIgnoreCase)))
            return
            [
                "NR0058 - Aux Alim comum/B", "NR0052 - Aux Alim 5X", "NR0053 - Aux Alim 10X",
                "AR0058 - Aux Alim comum/B atrasado", "AR0052 - Aux Alim 5X atrasado", "AR0053 - Aux Alim 10X atrasado",
                "FR0058 - Diferença Aux Alim comum/B", "FR0052 - Diferença Aux Alim 5X", "FR0053 - Diferença Aux Alim 10X",
                "DR0058 - Devolução Aux Alim comum/B", "DR0052 - Devolução Aux Alim 5X", "DR0053 - Devolução Aux Alim 10X"
            ];
    ...
    public Dictionary<string, string> EnrichFormValues(string? templateName, IReadOnlyDictionary<string, string>? formValues)
    {
        ...
        ApplyCalculatedPaymentFields(templateName, result, null);
        return result;
    }

    public Dictionary<string, string> EnrichIndividualFormValues(string? templateName, IReadOnlyDictionary<string, string>? values, MilitaryRecord military)
    {
        var result = (values ?? new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase))
            .ToDictionary(pair => pair.Key, pair => pair.Value ?? string.Empty, StringComparer.OrdinalIgnoreCase);
        ApplyCalculatedPaymentFields(templateName, result, military);
        return result;
    }

    private static void ApplyIndividualCalculations(string? templateName, IDictionary<string, string> values, MilitaryRecord military)
        => ApplyCalculatedPaymentFields(templateName, values, military);

    private static void ApplyCalculatedPaymentFields(string? templateName, IDictionary<string, string> values, MilitaryRecord? military)
    {
        ApplyAuxilioAlimentacaoCalculations(templateName, values);
        ApplyGratificacaoRepresentacaoCalculations(templateName, values, military);
    }
    ...
    private static void ApplyAuxilioAlimentacaoCalculations(string? templateName, IDictionary<string, string> values)
    {
        ...
        if (template.Contains("PRM", StringComparison.OrdinalIgnoreCase) || template.Contains("MUSEU", StringComparison.OrdinalIgnoreCase))
        {
            ApplyAuxilioAlimentacaoPrmCalculations(template, values);
            return;
        }

        var tipoLancamento = ResolveAuxFoodLaunchType(template, values);
        var buckets = new[]
        {
            ResolveAuxFoodBucket(values, 1, ResolveAuxFoodRubric(1, tipoLancamento)),
            ResolveAuxFoodBucket(values, 5, ResolveAuxFoodRubric(5, tipoLancamento)),
            ResolveAuxFoodBucket(values, 10, ResolveAuxFoodRubric(10, tipoLancamento))
        };
        var hasBucketLaunch = buckets.Any(item => item.Quantity > 0m || !string.IsNullOrWhiteSpace(item.DaysText));
        if (hasBucketLaunch)
        {
            ApplyAuxFoodBuckets(values, buckets,
                template.Contains("ATRASAD", StringComparison.OrdinalIgnoreCase) ? "CODIGO_ATRASADO" : "CODIGO_SAQUE");
            return;
        }

        // Compatibilidade com modelos antigos: código único + quantidade de dias.
        var codeText = FindValue(values, "CODIGO_SAQUE")
                       ?? FindValue(values, "CODIGO_ATRASADO")
                       ?? FindValue(values, "CODIGO");
        var codeMatch = Regex.Match(codeText ?? string.Empty, @"(?:(?:NR|AR|FR|DR)00|A)(?<codigo>52|53|58)", RegexOptions.IgnoreCase);
        if (!codeMatch.Success) return;
        var multiplier = (decimal)AuxilioAlimentacaoRubricRule.MultiplierFromSuffix(codeMatch.Groups["codigo"].Value);
        if (multiplier <= 0m) return;

        var daysText = FindValue(values, "QTD_DIAS")
                       ?? FindValue(values, "QUANTIDADE_DIAS")
                       ?? FindValue(values, "DIAS")
                       ?? FindValue(values, "DIAS_MISSAO")
                       ?? FindValue(values, "DIAS_SERVICO")
                       ?? FindValue(values, "PERIODO_MISSAO")
                       ?? FindValue(values, "PERIODO");
        var days = ResolveDayQuantity(daysText);
        if (days <= 0m) return;

        var etapa = ResolveEtapaComum(values);
        ...
        values["ETAPA_COMUM"] = etapa.ToString("N2", PtBr);
        values["VALOR_ETAPA_COMUM"] = etapa.ToString("N2", PtBr);
        if (!IsMeaningfulFieldValue("QTD_DIAS", FindValue(values, "QTD_DIAS")))
            values["QTD_DIAS"] = ((int)days).ToString("00", PtBr);
    }

    private static void ApplyAuxilioAlimentacaoPrmCalculations(string template, IDictionary<string, string> values)
    {
        var tipoLancamento = ResolveAuxFoodLaunchType(template, values);
        var buckets = new[]
        {
            ResolveAuxFoodBucket(values, 1, ResolveAuxFoodRubric(1, tipoLancamento), "DIAS_PRM_COMUM", "QTD_DIAS_PRM_COMUM", "CODIGO_PRM_COMUM"),
            ResolveAuxFoodBucket(values, 5, ResolveAuxFoodRubric(5, tipoLancamento), "DIAS_PRM_5X", "QTD_DIAS_PRM_5X", "CODIGO_PRM_5X"),
            ResolveAuxFoodBucket(values, 10, ResolveAuxFoodRubric(10, tipoLancamento), "DIAS_PRM_10X", "QTD_DIAS_PRM_10X", "CODIGO_PRM_10X")
        };
        ApplyAuxFoodBuckets(values, buckets, "CODIGO_PRM");

        values["DIAS_PRM_COMUM"] = CleanAuxFoodDaysForSaving(buckets[0]);
        values["DIAS_PRM_5X"] = CleanAuxFoodDaysForSaving(buckets[1]);
        values["DIAS_PRM_10X"] = CleanAuxFoodDaysForSaving(buckets[2]);
        values["QTD_DIAS_PRM_COMUM"] = CleanAuxFoodQuantityForSaving(buckets[0]);
        values["QTD_DIAS_PRM_5X"] = CleanAuxFoodQuantityForSaving(buckets[1]);
        values["QTD_DIAS_PRM_10X"] = CleanAuxFoodQuantityForSaving(buckets[2]);
        values["QTD_DIAS_PRM_TOTAL"] = ((int)buckets.Sum(item => item.Quantity)).ToString("00", PtBr);
        values["VALOR_PRM_COMUM"] = buckets[0].Total.ToString("N2", PtBr);
        values["VALOR_PRM_5X"] = buckets[1].Total.ToString("N2", PtBr);
        values["VALOR_PRM_10X"] = buckets[2].Total.ToString("N2", PtBr);
        values["VALOR_TOTAL_PRM"] = buckets.Sum(item => item.Total).ToString("N2", PtBr);
    }

    private static string ResolveAuxFoodRubric(int multiplier, AuxilioAlimentacaoTipoLancamento? tipoLancamento)
        => tipoLancamento is null ? string.Empty : AuxilioAlimentacaoRubricRule.Resolve(multiplier, tipoLancamento.Value);

    private static AuxilioAlimentacaoTipoLancamento? ResolveAuxFoodLaunchType(string template, IEnumerable<KeyValuePair<string, string>> values)
    {
        var declaredType = template.Contains("DEVOLUCAO", StringComparison.OrdinalIgnoreCase)
            ? AuxilioAlimentacaoTipoLancamento.Devolucao
            : template.Contains("DIFERENCA", StringComparison.OrdinalIgnoreCase)
                ? AuxilioAlimentacaoTipoLancamento.Diferenca
                : template.Contains("ATRASAD", StringComparison.OrdinalIgnoreCase)
                    ? AuxilioAlimentacaoTipoLancamento.Atrasado
                    : (AuxilioAlimentacaoTipoLancamento?)null;

        if (declaredType is AuxilioAlimentacaoTipoLancamento.Diferenca or AuxilioAlimentacaoTipoLancamento.Devolucao)
            return declaredType;

        var referenceText = FindValue(values, "MES_REFERENCIA");
        var paymentText = FindValue(values, "MES_PAGAMENTO")
                          ?? FindValue(values, "MES_SOLICITACAO_ATRASADO");
        if (TryParseMonth(referenceText ?? string.Empty, out var reference)
            && TryParseMonth(paymentText ?? string.Empty, out var payment))
        {
            var referenceMonth = new DateTime(reference.Year, reference.Month, 1);
            var paymentMonth = new DateTime(payment.Year, payment.Month, 1);
            if (referenceMonth > paymentMonth) return null;

            var inferredType = referenceMonth < paymentMonth
                ? AuxilioAlimentacaoTipoLancamento.Atrasado
                : AuxilioAlimentacaoTipoLancamento.Normal;
            if (declaredType == AuxilioAlimentacaoTipoLancamento.Atrasado
                && inferredType != AuxilioAlimentacaoTipoLancamento.Atrasado)
                return null;
            return inferredType;
        }
    ...
    private sealed record AuxFoodBucket(int Multiplier, string Code, string DaysKey, string QuantityKey, string CodeKey, string DaysText, bool HasExplicitQuantity, decimal Quantity, decimal ValuePerDay, decimal Total)
    {
        public string DisplayDays => string.IsNullOrWhiteSpace(DaysText) ? "-" : DaysText.Trim();
        public string QuantityText => ((int)Quantity).ToString("00", BulletinService.PtBr);
    }

    private static AuxFoodBucket ResolveAuxFoodBucket(IDictionary<string, string> values, int multiplier, string code, string? daysKey = null, string? quantityKey = null, string? codeKey = null)
    {
        daysKey ??= $"DIAS_{multiplier}X";
        quantityKey ??= $"QTD_DIAS_{multiplier}X";
        codeKey ??= $"CODIGO_{multiplier}X";
        var daysText = FindValue(values, daysKey) ?? string.Empty;
        var quantityText = FindValue(values, quantityKey);
        var hasExplicitQuantity = TryResolveExplicitDayQuantity(quantityText, out var quantity);
        if (!hasExplicitQuantity) quantity = ResolveDayQuantity(daysText);
        var etapa = ResolveEtapaComum(values);
        var valuePerDay = Math.Round(etapa * multiplier, 2, MidpointRounding.AwayFromZero);
        var total = Math.Round(valuePerDay * quantity, 2, MidpointRounding.AwayFromZero);
        return new AuxFoodBucket(multiplier, code, daysKey, quantityKey, codeKey, daysText, hasExplicitQuantity, quantity, valuePerDay, total);
    }


    private static string CleanAuxFoodDaysForSaving(AuxFoodBucket bucket)
        => bucket.HasExplicitQuantity && bucket.Quantity <= 0m
            ? string.Empty
            : string.IsNullOrWhiteSpace(bucket.DaysText) ? string.Empty : bucket.DaysText.Trim();

    private static string CleanAuxFoodQuantityForSaving(AuxFoodBucket bucket)
        => bucket.HasExplicitQuantity || bucket.Quantity > 0m ? bucket.QuantityText : string.Empty;

    private static void ApplyAuxFoodBuckets(IDictionary<string, string> values, IReadOnlyList<AuxFoodBucket> buckets, string codeKey)
    {
        var etapa = ResolveEtapaComum(values);
        var totalDays = buckets.Sum(item => item.Quantity);
        var total = buckets.Sum(item => item.Total);
        var active = buckets.Where(item => item.Quantity > 0m).ToList();

        foreach (var bucket in buckets)
        {
            values[bucket.DaysKey] = CleanAuxFoodDaysForSaving(bucket);
            values[bucket.QuantityKey] = CleanAuxFoodQuantityForSaving(bucket);
            if (string.IsNullOrWhiteSpace(bucket.Code)) values.Remove(bucket.CodeKey);
            else values[bucket.CodeKey] = bucket.Code;
            values[$"VALOR_{bucket.Multiplier}X"] = bucket.Total.ToString("N2", PtBr);
            values[$"VALOR_DIA_{bucket.Multiplier}X"] = bucket.ValuePerDay.ToString("N2", PtBr);
        }

        values.Remove("CODIGO_SAQUE");
        values.Remove("CODIGO_ATRASADO");
        values.Remove("CODIGO_PRM");
        values.Remove("CODIGOS_SAQUE");
        var codes = active.Select(item => item.Code).Where(code => !string.IsNullOrWhiteSpace(code)).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
        values[codeKey] = codes.Count == 0 ? "-" : string.Join(" + ", codes);
        values["CODIGOS_SAQUE"] = values[codeKey];
        values["QTD_DIAS"] = ((int)totalDays).ToString("00", PtBr);
        values["QTD_DIAS_TOTAL"] = ((int)totalDays).ToString("00", PtBr);
        values["VALOR_TOTAL"] = total.ToString("N2", PtBr);
        values["VALOR_TOTAL_INDIVIDUAL"] = total.ToString("N2", PtBr);
        values["ETAPA_COMUM"] = etapa.ToString("N2", PtBr);
        values["VALOR_ETAPA_COMUM"] = etapa.ToString("N2", PtBr);
        values["VALOR_DIA"] = active.Count == 1 ? active[0].ValuePerDay.ToString("N2", PtBr) : "0,00";
        values["VALOR_REFERENCIA_DIA"] = values["VALOR_DIA"];
        values["MULTIPLICADOR_ETAPA"] = active.Count == 1 ? active[0].Multiplier.ToString(CultureInfo.InvariantCulture) : "MISTO";
    }

    private static void ApplyGratificacaoRepresentacaoCalculations(string? templateName, IDictionary<string, string> values, MilitaryRecord? military)
    {
        var template = NormalizeKey(templateName);
        if (military is null || !template.Contains("GRATIFICACAOREPRESENTACAO", StringComparison.OrdinalIgnoreCase)) return;
        if (!template.Contains("2", StringComparison.OrdinalIgnoreCase) && !NormalizeKey(FindValue(values, "PERCENTUAL_BASE")).Contains("2")) return;
        var salary = SalaryService.Official2026.TryGetValue(MilitaryRankService.Canonicalize(military.Rank), out var official)
            ? official
            : 0m;
        if (salary <= 0m) return;
        var value = Math.Round(salary * 0.02m, 2, MidpointRounding.AwayFromZero);
        values["SOLDO"] = salary.ToString("N2", PtBr);
        values["PERCENTUAL_BASE"] = "2% do soldo";
        values["VALOR_TOTAL"] = value.ToString("N2", PtBr);
        values["VALOR_TOTAL_INDIVIDUAL"] = value.ToString("N2", PtBr);
    }

    private static decimal ResolveEtapaComum(IEnumerable<KeyValuePair<string, string>> values)
    {
        var etapaText = FindValue(values, "VALOR_ETAPA_COMUM")
                        ?? FindValue(values, "ETAPA_COMUM")
                        ?? "13,50";
        return TryParseDecimalLoose(etapaText, out var etapa) && etapa > 0m ? etapa : 13.50m;
    }

    private static decimal ResolveDayQuantity(string? text)
    {
        if (TryParseDecimalLoose(text, out var direct) && direct > 0m && Regex.IsMatch(text ?? string.Empty, @"^\s*\d{1,3}\s*$")) return direct;
        var raw = text ?? string.Empty;
        if (string.IsNullOrWhiteSpace(raw)) return 0m;

        if (TryResolveDateIntervalDays(raw, out var intervalDays)) return intervalDays;

        var normalized = RemoveAccents(raw).ToUpperInvariant();
        var completeDates = Regex.Matches(normalized, @"\b\d{1,2}[\/\-.]\d{1,2}[\/\-.]\d{2,4}\b")
            .Select(m => TryParseAuxFoodDate(m.Value, out var date) ? date.Date : DateTime.MinValue)
            .Where(d => d != DateTime.MinValue)
            .Distinct()
            .Count();
        if (completeDates > 1) return completeDates;

        var months = @"JAN|FEV|MAR|ABR|MAI|JUN|JUL|AGO|SET|OUT|NOV|DEZ|JANEIRO|FEVEREIRO|MARCO|ABRIL|MAIO|JUNHO|JULHO|AGOSTO|SETEMBRO|OUTUBRO|NOVEMBRO|DEZEMBRO";
        var monthList = Regex.Match(normalized, $@"(.+?)\s+(?:{months})\s*\d{{2,4}}\b");
        var prefix = monthList.Success
            ? monthList.Groups[1].Value
            : Regex.Split(normalized, $@"\b(?:{months})\b").FirstOrDefault() ?? normalized;
        prefix = Regex.Replace(prefix, @"\b\d{3,4}\b", " ");
        var numbers = Regex.Matches(prefix, @"(?<!\d)(\d{1,2})(?!\d)")
            .Select(m => int.TryParse(m.Groups[1].Value, out var n) ? n : 0)
            .Where(n => n is >= 1 and <= 31)
            .Distinct()
            .Count();
        return numbers;
    }

    private static bool TryResolveDateIntervalDays(string raw, out decimal days)
    {
        days = 0m;
        var normalized = RemoveAccents(raw).ToUpperInvariant();
        var match = Regex.Match(normalized, @"(?<start>\d{1,2}[\/\-.]\d{1,2}(?:[\/\-.]\d{2,4})?).{0,20}?(?:\s+A\s+|\s+ATE\s+|[-–—])(?<end>\d{1,2}[\/\-.]\d{1,2}(?:[\/\-.]\d{2,4})?)");
        if (!match.Success) return false;
        if (!TryParseAuxFoodDate(match.Groups["start"].Value, out var start) || !TryParseAuxFoodDate(match.Groups["end"].Value, out var end)) return false;
        if (end < start) return false;
        days = (decimal)(end.Date - start.Date).TotalDays + 1m;
        return days > 0m;
    }

    private static bool TryParseAuxFoodDate(string value, out DateTime date)
    {
        var text = (value ?? string.Empty).Trim();
        var formats = new[] { "dd/MM/yyyy", "d/M/yyyy", "dd/MM/yy", "d/M/yy", "dd-MM-yyyy", "d-M-yyyy", "dd-MM-yy", "d-M-yy", "dd.MM.yyyy", "d.M.yyyy", "dd.MM.yy", "d.M.yy", "dd/MM", "d/M", "dd-MM", "d-M", "dd.MM", "d.M" };
        if (DateTime.TryParseExact(text, formats, PtBr, DateTimeStyles.AllowWhiteSpaces, out date))
        {
            if (date.Year == 1) date = new DateTime(DateTime.Today.Year, date.Month, date.Day);
            return true;
        }
        return DateTime.TryParse(text, PtBr, DateTimeStyles.AllowWhiteSpaces, out date);
    }
    ...
    public static string NormalizeTemplateName(string? value)
    {
        ...
        text = Regex.Replace(text, @"\s+-\s+", " - ");
        if (MilitaryRankService.Normalize(text).Equals("dependencia economica inclusao", StringComparison.Ordinal))
            return BirthBenefitBulletinWorkflow.EconomicDependency;
    ...
    {
        foreach (var key in source.Keys.Where(BelongsToDedicatedModule).ToList())
            source.Remove(key);
    }
    ...
    private static void RemoveOutdatedPerMilitaryTemplateOverrides(Dictionary<string, string> source)
    {
        foreach (var pair in source.ToList())
        {
            var name = NormalizeKey(pair.Key);
            var text = pair.Value ?? string.Empty;
            var normalizedText = NormalizeKey(text);

            var isOutdatedAuxFood = name.StartsWith("AUXILIOALIMENTACAO", StringComparison.OrdinalIgnoreCase)
                                    && (normalizedText.Contains("A535X", StringComparison.OrdinalIgnoreCase)
                                        || normalizedText.Contains("AR00535X", StringComparison.OrdinalIgnoreCase)
                                        || normalizedText.Contains("A5210X", StringComparison.OrdinalIgnoreCase)
                                        || normalizedText.Contains("AR005210X", StringComparison.OrdinalIgnoreCase)
                                        || normalizedText.Contains("AR0048", StringComparison.OrdinalIgnoreCase)
                                        || normalizedText.Contains("AR0043", StringComparison.OrdinalIgnoreCase)
                                        || normalizedText.Contains("AR0042", StringComparison.OrdinalIgnoreCase)
                                        || normalizedText.Contains("CODIGODESAQUECODIGOSAQUE", StringComparison.OrdinalIgnoreCase)
                                        || (name.Contains("ATRASAD", StringComparison.OrdinalIgnoreCase)
                                            && Regex.IsMatch(text, @"\[\[\s*CODIGO_(?:ATRASADO|SAQUE)(?!\s*#2)", RegexOptions.IgnoreCase))
                                        || normalizedText.Contains("QUANTIDADEDEDIASQTDDIAS", StringComparison.OrdinalIgnoreCase)
                                        || normalizedText.Contains("VALORTOTALINDIVIDUALVALORTOTAL", StringComparison.OrdinalIgnoreCase)
                                        || !normalizedText.Contains("VALORAMBOS", StringComparison.OrdinalIgnoreCase)
                                        || !normalizedText.Contains("MOTIVODOSAQUE", StringComparison.OrdinalIgnoreCase)
                                        || (!normalizedText.Contains("DIAS1X", StringComparison.OrdinalIgnoreCase)
                                            && !normalizedText.Contains("DIASPRMCOMUM", StringComparison.OrdinalIgnoreCase))
                                        || (name.Contains("PRM", StringComparison.OrdinalIgnoreCase)
                                            && (!normalizedText.Contains("FUNDAMENTOAR0058", StringComparison.OrdinalIgnoreCase)
                                                || !normalizedText.Contains("ART66CBNCOMIIDOART67", StringComparison.OrdinalIgnoreCase)
                                                || !normalizedText.Contains("CODIGOPRM5X", StringComparison.OrdinalIgnoreCase)))
                                        || ((name.Contains("PRM", StringComparison.OrdinalIgnoreCase)
                                             || name.Contains("MOTORISTA", StringComparison.OrdinalIgnoreCase)
                                             || normalizedText.Contains("MOTORISTADOCOMANDANTE", StringComparison.OrdinalIgnoreCase))
                                            && (normalizedText.Contains("OMLOCALDEORIGEM", StringComparison.OrdinalIgnoreCase)
                                                || normalizedText.Contains("OMLOCALDEDESTINO", StringComparison.OrdinalIgnoreCase)
                                                || normalizedText.Contains("DIEXNUMERO2", StringComparison.OrdinalIgnoreCase)
                                                || normalizedText.Contains("DIEXDATA2", StringComparison.OrdinalIgnoreCase))));
            var isOutdatedPaymentTransfer = name.Equals("PAGAMENTOTRANSFERENCIADEPAGAMENTO", StringComparison.OrdinalIgnoreCase)
                                            && normalizedText.Contains("ORIUNDOSDAOMOMORIGEM", StringComparison.OrdinalIgnoreCase);
            var isOutdatedGratRep2 = name.StartsWith("GRATIFICACAOREPRESENTACAO2", StringComparison.OrdinalIgnoreCase)
                                     && normalizedText.Contains("VALORTOTALVALORTOTAL", StringComparison.OrdinalIgnoreCase);

            if (isOutdatedAuxFood || isOutdatedPaymentTransfer || isOutdatedGratRep2)
                source.Remove(pair.Key);
        }
    }

    private static void RemoveOutdatedBirthBenefitOverrides(Dictionary<string, string> source)
    {
        foreach (var pair in source.ToList())
        {
            if (!pair.Key.Equals(BirthBenefitBulletinWorkflow.EconomicDependency, StringComparison.OrdinalIgnoreCase)) continue;
            var text = MilitaryRankService.Normalize(pair.Value);
            if (!text.Contains("implantad", StringComparison.Ordinal)) source.Remove(pair.Key);
        }
    }

    public static bool BelongsToDedicatedModule(string? name)
    {
        var normalized = NormalizeKey(NormalizeTemplateName(name));
        return normalized.StartsWith("GRATIFICACAODEREPRESENTACAO", StringComparison.OrdinalIgnoreCase)
            || normalized.StartsWith("GRATIFICACAOREPRESENTACAO", StringComparison.OrdinalIgnoreCase)
            || normalized.StartsWith("AUXILIOTRANSPORTE", StringComparison.OrdinalIgnoreCase)
            ...
            || normalized.StartsWith("PENSAOJUDICIAL", StringComparison.OrdinalIgnoreCase);
    }

    public static string ResolveTemplateCategory(string? name)
    {
        var normalized = NormalizeKey(NormalizeTemplateName(name));
        if (normalized.Contains("IMPLANTACAOSIPPES", StringComparison.OrdinalIgnoreCase)
            || normalized.Contains("REIMPLANTACAO", StringComparison.OrdinalIgnoreCase)
            || normalized.Contains("EXCLUSAODECADASTRO", StringComparison.OrdinalIgnoreCase)
            || normalized.Contains("TRANSFERENCIADEPAGAMENTO", StringComparison.OrdinalIgnoreCase))
            return "Cadastro e movimentação";

        if (normalized.Contains("ADICIONAL", StringComparison.OrdinalIgnoreCase)
            || normalized.Contains("COMPENSACAOORGANICA", StringComparison.OrdinalIgnoreCase)
            || normalized.Contains("PERMANENCIA", StringComparison.OrdinalIgnoreCase)
            || normalized.Contains("HABILITACAO", StringComparison.OrdinalIgnoreCase)
            || normalized.Contains("LOCALIDADEESPECIAL", StringComparison.OrdinalIgnoreCase))
            return "Adicionais e direitos";

        if (normalized.Contains("AUXILIO", StringComparison.OrdinalIgnoreCase)
            || normalized.Contains("DEPENDENCIAECONOMICA", StringComparison.OrdinalIgnoreCase)
            || normalized.Contains("PREESCOLAR", StringComparison.OrdinalIgnoreCase)
            || normalized.Contains("SALARIOFAMILIA", StringComparison.OrdinalIgnoreCase))
            return "Benefícios e dependentes";

        if (normalized.Contains("FUSEX", StringComparison.OrdinalIgnoreCase)
            || normalized.Contains("PNR", StringComparison.OrdinalIgnoreCase))
            return "Saúde, moradia e descontos";

        if (normalized.Contains("EXERCICIOSANTERIORES", StringComparison.OrdinalIgnoreCase)
            || normalized.Contains("COMPENSACAOPECUNIARIA", StringComparison.OrdinalIgnoreCase)
            || normalized.Contains("FICHAFINANCEIRA", StringComparison.OrdinalIgnoreCase))
            return "Pagamentos especiais";
    ...
    private static void NormalizePreferenceTemplateNames(BulletinPreferences preferences)
    {
        ...
        preferences.PerMilitaryFormValues = MergeByNormalizedTemplate(preferences.PerMilitaryFormValues);
    ...
    private static Dictionary<string, Dictionary<int, Dictionary<string, string>>> MergeByNormalizedTemplate(Dictionary<string, Dictionary<int, Dictionary<string, string>>> source)
    {
        ...
        {
            var name = NormalizeTemplateName(pair.Key);
            if (!result.TryGetValue(name, out var byMilitary))
            {
                byMilitary = new Dictionary<int, Dictionary<string, string>>();
                result[name] = byMilitary;
            }

            if (pair.Value is null) continue;
            foreach (var item in pair.Value)
            {
                if (!byMilitary.TryGetValue(item.Key, out var values))
                {
                    values = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
                    byMilitary[item.Key] = values;
                }
    ...
    public List<BulletinFieldDefinition> DetectItemFields(string text, bool includeAutomatic = false)
    {
        var match = ItemBlockRegex().Match(text ?? string.Empty);
        return match.Success ? DetectFields(match.Groups[1].Value, includeAutomatic) : [];
    }

    public List<BulletinFieldDefinition> DetectGlobalFields(string text, bool includeAutomatic = false)
    {
        var source = text ?? string.Empty;
        var match = ItemBlockRegex().Match(source);
        if (match.Success) source = source.Remove(match.Index, match.Length);
        return DetectFields(source, includeAutomatic);
    }
        ...
        IReadOnlyDictionary<int, Dictionary<string, string>>? perMilitaryKeys = null)
    {
        ...
        if (selected.Count == 1)
        {
            ...
            ApplyIndividualCalculations(template.Name, common, selected[0]);
        }
        ...
        {
            var before = RemoveOptionalTokenLines(source[..itemMatch.Index], common);
            var itemTemplate = itemMatch.Groups[1].Value;
            var after = RemoveOptionalTokenLines(source[(itemMatch.Index + itemMatch.Length)..], common);
            ...
            foreach (var military in selected)
            {
                ...
                ApplyIndividualCalculations(template.Name, values, military);
                var cleanedItemTemplate = RemoveOptionalTokenLines(itemTemplate, values);
                cleanedItemTemplate = SuppressInactiveAuxilioAlimentacaoLines(template.Name, cleanedItemTemplate, values);
                items.Add(ReplaceTokens(cleanedItemTemplate, values, military).Trim());
            }
        ...
        {
            rendered = ReplaceTokens(RemoveOptionalTokenLines(source, common), common, selected.Count == 1 ? selected[0] : null).TrimEnd();
            if (selected.Count > 0)
            {
        ...
        var plainText = BulletinTableCodec.ToPlainText(rendered);
        return new BulletinRenderResult
        {
            Text = plainText,
            StructuredText = rendered,
            BoldRanges = FindWarNameRanges(plainText, selected),
    ...
    private static string RemoveOptionalTokenLines(string text, IReadOnlyDictionary<string, string> values)
    {
        ...
        {
            var optionalKeys = TokenRegex().Matches(line).Cast<Match>()
                .Select(FirstGroup)
                .Select(raw => ParseField(raw).Key)
                .Where(IsOptionalFieldKey)
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .ToList();
            if (optionalKeys.Count > 0 && optionalKeys.All(key => !IsMeaningfulFieldValue(key, FindValue(values, key))))
                continue;
            kept.Add(line);
        }
    ...
    private static bool IsOptionalFieldKey(string? key)
    {
        var normalized = NormalizeKey(key);
        return normalized.Contains("OPCIONAL", StringComparison.OrdinalIgnoreCase)
               || normalized.Contains("CONDICIONAL", StringComparison.OrdinalIgnoreCase);
    }
    ...
    public FlowDocument BuildDocument(BulletinRenderResult result)
    {
        ...
        var structured = string.IsNullOrEmpty(result.StructuredText) ? result.Text : result.StructuredText;
        var plainOffset = 0;
        foreach (var segment in BulletinTableCodec.Split(structured))
        {
            if (segment.Table is not null)
            {
                document.Blocks.Add(BuildTableBlock(segment.Table));
                plainOffset += BulletinTableCodec.ToTabSeparatedText(segment.Table).Length;
                continue;
            }
            ...
            var segmentStart = plainOffset;
            var segmentEnd = segmentStart + segment.Text.Length;
            var cursor = 0;
            foreach (var range in ranges.Where(item => item.Start < segmentEnd && item.Start + item.Length > segmentStart))
            {
                var localStart = Math.Max(0, range.Start - segmentStart);
                var localEnd = Math.Min(segment.Text.Length, range.Start + range.Length - segmentStart);
                if (localStart > cursor) AddTextWithLineBreaks(paragraph, segment.Text[cursor..localStart], false);
                if (localEnd > localStart) AddTextWithLineBreaks(paragraph, segment.Text[localStart..localEnd], true);
                cursor = Math.Max(cursor, localEnd);
            }
            if (cursor < segment.Text.Length) AddTextWithLineBreaks(paragraph, segment.Text[cursor..], false);
            if (paragraph.Inlines.Count > 0) document.Blocks.Add(paragraph);
    ...
    private static System.Windows.Documents.Table BuildTableBlock(BulletinTableData source)
    {
        var data = BulletinTableCodec.Normalize(source);
        var table = new System.Windows.Documents.Table
        {
            CellSpacing = 0,
            Margin = new System.Windows.Thickness(0, 7, 0, 7),
            FontFamily = new FontFamily(BulletinTextFormatter.StandardFontFamily),
            FontSize = BulletinTextFormatter.StandardWpfFontSize
        };
        var availableWidth = 690d * data.WidthPercent / 100d;
        foreach (var width in data.ColumnWidths)
            table.Columns.Add(new TableColumn { Width = new System.Windows.GridLength(availableWidth * width / 100d) });

        var group = new TableRowGroup();
        for (var rowIndex = 0; rowIndex < data.Rows; rowIndex++)
        {
            var row = new TableRow();
            for (var columnIndex = 0; columnIndex < data.Columns; columnIndex++)
            {
                var paragraph = new Paragraph
                {
                    Margin = new System.Windows.Thickness(0),
                    TextAlignment = columnIndex == 0 ? System.Windows.TextAlignment.Left : System.Windows.TextAlignment.Center,
                    FontWeight = data.HasHeader && rowIndex == 0 ? System.Windows.FontWeights.Bold : System.Windows.FontWeights.Normal
                };
                AddTextWithLineBreaks(paragraph, data.Cells[rowIndex][columnIndex], data.HasHeader && rowIndex == 0);
                row.Cells.Add(new TableCell(paragraph)
                {
                    BorderBrush = System.Windows.Media.Brushes.Black,
                    BorderThickness = new System.Windows.Thickness(0.65),
    ...
    private string ReplaceTokens(string text, IReadOnlyDictionary<string, string> values, MilitaryRecord? military)
    {
        return TokenRegex().Replace(text ?? string.Empty, match =>
        {
            ...
            {
                var table = BulletinTableCodec.ParseOrCreate(value, parsed.TableRows, parsed.TableColumns, parsed.TableWidthPercent, parsed.TableHasHeader);
                for (var row = 0; row < table.Rows; row++)
                    for (var column = 0; column < table.Columns; column++)
                        table.Cells[row][column] = ReplaceTokens(table.Cells[row][column], values, military);
                return BulletinTableCodec.EncodeMarker(BulletinTableCodec.Serialize(table));
            }
    ...
    private static string SuppressInactiveAuxilioAlimentacaoLines(string? templateName, string itemTemplate, IReadOnlyDictionary<string, string> values)
    {
        var template = NormalizeKey(templateName);
        if (!template.Contains("AUXILIOALIMENTACAO", StringComparison.OrdinalIgnoreCase)) return itemTemplate;
        if (string.IsNullOrWhiteSpace(itemTemplate)) return itemTemplate;

        var isPrm = template.Contains("PRM", StringComparison.OrdinalIgnoreCase) || template.Contains("MUSEU", StringComparison.OrdinalIgnoreCase);
        var normalizedText = NormalizeLineEndings(itemTemplate);
        var lines = normalizedText.Split('\n');
        var kept = new List<string>(lines.Length);
        foreach (var line in lines)
        {
            var normalizedLine = NormalizeKey(line);
            if (ShouldSuppressAuxilioAlimentacaoLine(normalizedLine, values, isPrm)) continue;
            kept.Add(line);
        }
        return string.Join(Environment.NewLine, kept);
    }

    private static bool ShouldSuppressAuxilioAlimentacaoLine(string normalizedLine, IReadOnlyDictionary<string, string> values, bool isPrm)
    {
        if (string.IsNullOrWhiteSpace(normalizedLine)) return false;
        if (isPrm)
        {
            if (LineBelongsToAuxBucket(normalizedLine, "PRMCOMUM", "1X", "A58", "0058") && !HasAuxFoodLaunch(values, "QTD_DIAS_PRM_COMUM", "DIAS_PRM_COMUM")) return true;
            if (LineBelongsToAuxBucket(normalizedLine, "PRM5X", "5X", "A52", "0052") && !HasAuxFoodLaunch(values, "QTD_DIAS_PRM_5X", "DIAS_PRM_5X")) return true;
            if (LineBelongsToAuxBucket(normalizedLine, "PRM10X", "10X", "A53", "0053") && !HasAuxFoodLaunch(values, "QTD_DIAS_PRM_10X", "DIAS_PRM_10X")) return true;
            return false;
        }

        if (LineBelongsToAuxBucket(normalizedLine, "1X", "COMUM", "A58", "0058") && !HasAuxFoodLaunch(values, "QTD_DIAS_1X", "DIAS_1X")) return true;
        if (LineBelongsToAuxBucket(normalizedLine, "5X", "A52", "0052") && !HasAuxFoodLaunch(values, "QTD_DIAS_5X", "DIAS_5X")) return true;
        if (LineBelongsToAuxBucket(normalizedLine, "10X", "A53", "0053") && !HasAuxFoodLaunch(values, "QTD_DIAS_10X", "DIAS_10X")) return true;
        return false;
    }

    private static bool LineBelongsToAuxBucket(string normalizedLine, params string[] markers)
        => markers.Any(marker => normalizedLine.Contains(NormalizeKey(marker), StringComparison.OrdinalIgnoreCase));

    private static bool HasAuxFoodLaunch(IReadOnlyDictionary<string, string> values, string quantityKey, string daysKey)
    {
        if (TryResolveExplicitDayQuantity(FindValue(values, quantityKey), out var quantity))
            return quantity > 0m;
        return ResolveDayQuantity(FindValue(values, daysKey)) > 0m;
    }

    private static bool TryResolveExplicitDayQuantity(string? text, out decimal quantity)
    {
        quantity = 0m;
        var value = (text ?? string.Empty).Trim();
        if (!Regex.IsMatch(value, @"^\d{1,3}$", RegexOptions.CultureInvariant)) return false;
        return decimal.TryParse(value, NumberStyles.Integer, PtBr, out quantity) && quantity >= 0m;
    }
    ...
    private static BulletinFieldDefinition ParseField(string raw)
    {
        ...
        else if (metaUpper.StartsWith("TABELA"))
        {
            field.Type = "table";
            var dimensions = Regex.Match(metaUpper, @"TABELA\s*=\s*(?<rows>\d{1,2})\s*[X×]\s*(?<columns>\d{1,2})", RegexOptions.CultureInvariant);
            if (dimensions.Success)
            {
                field.TableRows = Math.Clamp(int.Parse(dimensions.Groups["rows"].Value, PtBr), 1, 30);
                field.TableColumns = Math.Clamp(int.Parse(dimensions.Groups["columns"].Value, PtBr), 1, 12);
            }
            var width = Regex.Match(metaUpper, @"LARGURA\s*=\s*(?<width>\d{1,3})", RegexOptions.CultureInvariant);
            if (width.Success) field.TableWidthPercent = Math.Clamp(int.Parse(width.Groups["width"].Value, PtBr), 30, 100);
            var header = Regex.Match(metaUpper, @"CABECALHO\s*=\s*(?<header>[^;]+)", RegexOptions.CultureInvariant);
            if (header.Success) field.TableHasHeader = !NormalizeKey(header.Groups["header"].Value).Equals("NAO", StringComparison.OrdinalIgnoreCase);
            var data = Regex.Match(meta, @"(?:^|;)\s*DADOS\s*=\s*(?<data>[A-Za-z0-9_-]+)", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
            if (data.Success && BulletinTableCodec.TryDecodeDefinition(data.Groups["data"].Value, out var defaultTable))
            {
                field.TableDefaultValue = BulletinTableCodec.Serialize(defaultTable);
                field.TableRows = defaultTable.Rows;
                field.TableColumns = defaultTable.Columns;
                field.TableWidthPercent = defaultTable.WidthPercent;
                field.TableHasHeader = defaultTable.HasHeader;
            }
        }
        return field;
    }
    ...
    private static string FormatValue(string value, BulletinFieldDefinition field)
    {
        return field.Type switch
        {
            ...
            "table" => BulletinTableCodec.EncodeMarker(value),
            _ => value
        };
    }
    ...
    {
        foreach (var format in new[] { "MM/yyyy", "M/yyyy", "MM/yy", "M/yy", "yyyy-MM", "MMM/yy", "MMM yy", "MMM yyyy", "MMMM/yyyy", "MMMM yyyy", "MMMM 'de' yyyy" })
            if (DateTime.TryParseExact(value?.Trim(), format, PtBr, DateTimeStyles.AllowWhiteSpaces, out date)) return true;

        var normalized = RemoveAccents(value).ToUpperInvariant().Replace(".", string.Empty, StringComparison.Ordinal).Trim();
        var namedMonth = Regex.Match(normalized,
            @"^(?<month>JAN|FEV|MAR|ABR|MAI|JUN|JUL|AGO|SET|OUT|NOV|DEZ)(?:EIRO|CO|IL|O|HO|H0|STO|EMBRO|UBRO|EMBRO)?[\s/\-]+(?<year>\d{2}|\d{4})$");
        if (namedMonth.Success)
        {
            ...
            var month = months[namedMonth.Groups["month"].Value];
            var year = int.Parse(namedMonth.Groups["year"].Value, CultureInfo.InvariantCulture);
            if (year < 100) year = PtBr.Calendar.ToFourDigitYear(year);
