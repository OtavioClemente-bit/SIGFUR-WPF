...
public sealed class SettingsService
{
    ...
    public async Task SaveFontScaleAsync(double scale)
    {
        var root = await _json.LoadNodeAsync(_paths.AppSettingsFile) as JsonObject ?? new JsonObject();
        root["font_scale"] = GlobalUiScaleService.Sanitize(scale);
        await _json.SaveNodeAsync(_paths.AppSettingsFile, root);
    }
    ...
    private static WindowStateData SanitizeWindowState(WindowStateData state)
    {
        ...
        state.UiScale = Math.Clamp(FiniteOr(state.UiScale, 1.0), GlobalUiScaleService.MinimumScale, GlobalUiScaleService.MaximumScale);
        return state;
    }
    ...
    private static List<string> ReadStringList(JsonNode? node, string current)
    {
        ...
        foreach (var organization in OrganizationDefaults.FeaturedNames)
            if (!result.Contains(organization, StringComparer.OrdinalIgnoreCase)) result.Add(organization);
        return result;
    }
