...
public sealed class SisbolBulletinCheckItem
{
    public int Year { get; init; }
    public int Month { get; init; }
...
public sealed class SisbolBulletinCheckResult
{
    public int Year { get; init; }
    public int BulletinTypeCode { get; init; }
    public string BulletinTypeName { get; init; } = string.Empty;
    public int? Month { get; init; }
    public List<SisbolBulletinCheckItem> AvailableFiles { get; } = [];
    public List<string> Errors { get; } = [];
    public int Available => AvailableFiles.Count;
}
...
public sealed class SisbolAutomationService : IDisposable
{
    ...
    public SisbolSubmissionHistoryService History { get; }
    ...
    public SisbolAutomationService(AppPaths paths, JsonFileService json, LogService log)
    {
        ...
        _json = json;
        _log = log;
        History = new SisbolSubmissionHistoryService(paths, json, log);
    }
    ...
    public async Task<string> OpenLoginAsync(SisbolSettings settings, string password, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            await Task.Run(() =>
            {
                ...
                // A navegação pode devolver o foco ao SIGFUR. Reforça aqui que a
                // janela humana de login/captcha deve permanecer visível e à frente.
                ShowBrowser(_driver);
            }, cancellationToken);

            return "O navegador ficará visível para o captcha. Digite o código e pressione Enter: ao confirmar a entrada, o SIGFUR ocultará o navegador automaticamente. Se preferir, use ‘Já entrei — validar e ocultar’.";
        }
        catch (Exception ex)
        {
        ...
        try
        {
            ...
            return await Task.Run(() =>
            {
                try
                {
                    WaitReady(_driver, 10);
                    WaitForDomIdle(_driver, TimeSpan.FromSeconds(2));
                    if (!LooksLikeAuthenticatedSisbolPage(_driver))
                    {
                        _confirmed = false;
                        ShowBrowser(_driver);
                        return LooksLikeLoginOrCaptcha(_driver)
                            ? (false, "O SisBol ainda está na tela de login/captcha. Conclua o login no navegador e tente validar novamente.")
                            : (false, "O SisBol ainda está carregando ou não confirmou a entrada. Aguarde a página inicial e tente validar novamente.");
                    }

                    return CompleteAuthenticatedLogin(_driver, settings);
                }
                catch (Exception ex)
                {
                    _confirmed = false;
    ...
    /// <summary>
    /// Verificação rápida usada enquanto o usuário resolve o captcha. Quando o
    /// Enter abre uma página autenticada, oculta o navegador automaticamente.
    /// Não aguarda a fila nem interfere em uma validação manual em andamento.
    /// </summary>
    public async Task<(bool Success, string Message)> TryCompleteLoginAfterEnterAsync(
        SisbolSettings settings,
        CancellationToken cancellationToken = default)
    {
        if (!await _gate.WaitAsync(0, cancellationToken)) return (false, string.Empty);
        try
        {
            if (!DriverAlive() || _driver is null) return (false, string.Empty);
            return await Task.Run(() =>
            {
                try
                {
                    if (!LooksLikeAuthenticatedSisbolPage(_driver)) return (false, string.Empty);
                    return CompleteAuthenticatedLogin(_driver, settings);
                }
    ...
    private (bool Success, string Message) CompleteAuthenticatedLogin(IWebDriver driver, SisbolSettings settings)
    {
        _confirmed = true;
        _cachedAlive = true;
        _cachedReady = true;
        settings.HideAfterLogin = true;
        if (HideBrowser(driver))
            return (true, "SisBol preparado. Login confirmado e navegador ocultado; a sessão será reutilizada por todos os módulos.");
    ...
    public async Task<SisbolBulletinCheckResult> CheckGeneratedBulletinsAsync(
        int bulletinTypeCode,
        int year,
        IProgress<string>? progress = null,
        CancellationToken cancellationToken = default,
        int? month = null)
    {
        if (bulletinTypeCode is not 1 and not 3)
            throw new ArgumentOutOfRangeException(nameof(bulletinTypeCode), "Use 1 para Boletim Interno ou 3 para Aditamento do Furriel.");
        if (year < 2000 || year > DateTime.Today.Year + 1)
            throw new ArgumentOutOfRangeException(nameof(year), "Ano inválido para consulta de boletim.");
        if (month is < 1 or > 12)
            throw new ArgumentOutOfRangeException(nameof(month), "Mês inválido para consulta de boletim.");
        if (year == DateTime.Today.Year && month.HasValue && month.Value > DateTime.Today.Month)
            throw new InvalidOperationException("O mês escolhido ainda não chegou. Para o ano atual, consulte somente até o mês corrente.");

        var typeName = bulletinTypeCode == 1 ? "Boletim Interno" : "Aditamento do Furriel";
        var result = new SisbolBulletinCheckResult
        {
            Year = year,
            Month = month,
            BulletinTypeCode = bulletinTypeCode,
        ...
        try
        {
            ...
            return await Task.Run(() =>
            {
                ...
                {
                    cancellationToken.ThrowIfCancellationRequested();
                    progress?.Report($"Verificando {typeName} {currentMonth:00}/{year}...");
                    var pageUrl = $"{sisbolBaseUrl.TrimEnd('/')}/band/baixar_boletim.php?codTipoBol={bulletinTypeCode}&ano={year}&mes={currentMonth}";
                    try
                    {
                        driver.Navigate().GoToUrl(pageUrl);
                        ...
                        {
                            _confirmed = false;
                            throw new InvalidOperationException("A sessão do SisBol não foi aceita nesta página de consulta. Prepare/valide o SisBol novamente e tente outra vez.");
                        }
                        ...
                        {
                            var link = links[i];
                            result.AvailableFiles.Add(new SisbolBulletinCheckItem
                            {
                                Year = year,
                                Month = currentMonth,
                                FileName = NormalizeSisbolBulletinFileName(link.FileName, bulletinTypeCode, year, currentMonth, i + 1),
                                Url = link.Url
                            });
                        }
                    }
                    ...
                    {
                        result.Errors.Add($"{currentMonth:00}/{year}: {ex.Message}");
                        try { _log.WriteAsync($"Falha ao verificar {typeName} {currentMonth:00}/{year}.", ex).GetAwaiter().GetResult(); } catch { }
                    }
                }
    ...
    public async Task<SisbolPersonIndexDownloadResult> DownloadSubjectIndexAsync(
        DateTime startDate,
        DateTime endDate,
        int bulletinTypeCode,
        string outputDirectory,
        ...
        CancellationToken cancellationToken = default)
    {
        if (bulletinTypeCode <= 0) throw new ArgumentOutOfRangeException(nameof(bulletinTypeCode), "Tipo de boletim invalido.");
        if (endDate.Date < startDate.Date) (startDate, endDate) = (endDate.Date, startDate.Date);
        if (startDate.Year < 2000 || endDate.Year > DateTime.Today.Year + 1)
            throw new ArgumentOutOfRangeException(nameof(startDate), "Informe um periodo valido para o indice do SisBol.");
        if (string.IsNullOrWhiteSpace(outputDirectory))
            throw new ArgumentException("Informe a pasta de destino do Indice por Assunto.", nameof(outputDirectory));
        ...
        {
            if (!DriverAlive() || _driver is null || !_confirmed)
                throw new InvalidOperationException("Prepare o SisBol primeiro. Faca login/captcha uma vez em 'Preparar SisBol' e deixe a sessao pronta.");
            ...
            return await Task.Run(async () =>
            {
                ...
                var startText = startDate.ToString("dd/MM/yyyy", CultureInfo.InvariantCulture);
                var endText = endDate.ToString("dd/MM/yyyy", CultureInfo.InvariantCulture);
                progress?.Report("Abrindo tela de geracao do Indice por Assunto no SisBol...");
                var beforeHandles = driver.WindowHandles.ToHashSet(StringComparer.OrdinalIgnoreCase);
                driver.Navigate().GoToUrl(indexUrl);
                WaitReady(driver, 20);
                WaitForDomIdle(driver, TimeSpan.FromSeconds(2), cancellationToken);
                ...
                {
                    _confirmed = false;
                    throw new InvalidOperationException("A sessao do SisBol expirou. Prepare/valide o SisBol novamente.");
                }

                progress?.Report($"Preenchendo periodo {startText} a {endText}...");
                object? scriptResult = null;
                try
                {
                    scriptResult = ((IJavaScriptExecutor)driver).ExecuteScript(@"
                    ...
                    const okFim = setValue(['dt_final', 'dtFinal', 'data_final'], fim);
                    if (!okInicio || !okFim) {
                        return { ok: false, message: 'Campos de data do indice nao encontrados.' };
                    }
                    const candidates = Array.from(document.querySelectorAll('input,button,a'));
                    const btn = candidates.find(x => {
                        const name = normalizeText(x.getAttribute('name'));
                        ...
                        const onclick = normalizeText(x.getAttribute('onclick'));
                        return name === 'assunto'
                            || value.includes('indice por assunto')
                            || text.includes('indice por assunto')
                            || onclick.includes('assunto');
                    });
                    if (btn) {
                        btn.click();
                        return { ok: true, message: 'Comando enviado pelo botao assunto.', tipoOk: okTipo };
                    }
                    if (typeof window.gerarIndice === 'function') {
                        window.gerarIndice('assunto');
                        return { ok: true, message: 'Comando enviado por gerarIndice assunto.', tipoOk: okTipo };
                    }
                    const form = document.querySelector('form');
                    if (form) {
                        const hidden = document.createElement('input');
                        hidden.type = 'hidden';
                        hidden.name = 'assunto';
                        hidden.value = 'Gerar Indice por Assunto';
                        form.appendChild(hidden);
                        form.submit();
                        return { ok: true, message: 'Comando enviado por submit do formulario.', tipoOk: okTipo };
                    }
                    return { ok: false, message: 'Botao Gerar Indice por Assunto nao encontrado. A pagina nao possui name=assunto, onclick com assunto nem funcao gerarIndice.' };
                    ", bulletinTypeCode, startText, endText);
                }
                catch (UnhandledAlertException)
                {
                    // SisBol can open a native confirm() immediately after clicking the index command.
                }
                var confirmText = WaitForAndAcceptAlert(driver, TimeSpan.FromSeconds(8));
                if (!string.IsNullOrWhiteSpace(confirmText))
                    progress?.Report("Confirmacao do SisBol aceita. Gerando PDF do Indice por Assunto...");

                if (scriptResult is IDictionary<string, object> scriptMap && scriptMap.TryGetValue("ok", out var okValue) && okValue is bool ok && !ok)
                {
                    var message = scriptMap.TryGetValue("message", out var m) ? m?.ToString() : null;
                    throw new InvalidOperationException(message ?? "Nao consegui acionar o botao Gerar Indice por Assunto.");
                }

                progress?.Report("Aguardando geracao do PDF do Indice por Assunto...");
                Thread.Sleep(1600);
                try
                {
                    var newHandle = driver.WindowHandles.FirstOrDefault(handle => !beforeHandles.Contains(handle));
                ...
                var delayedConfirm = WaitForAndAcceptAlert(driver, TimeSpan.FromSeconds(3));
                if (!string.IsNullOrWhiteSpace(delayedConfirm))
                    progress?.Report("Confirmacao adicional do SisBol aceita. Aguardando PDF...");
                try { WaitReady(driver, 25); }
                catch (UnhandledAlertException)
                {
                    WaitForAndAcceptAlert(driver, TimeSpan.FromSeconds(3));
                ...
                {
                    _confirmed = false;
                    throw new InvalidOperationException("A sessao do SisBol expirou ao gerar o indice. Prepare/valide o SisBol novamente.");
                }

                var fileName = $"{startDate:yyyy-MM-dd}_{endDate:yyyy-MM-dd}_indice_por_assunto.pdf";
                var destination = Path.Combine(outputDirectory, fileName);
                var currentUrl = driver.Url ?? indexUrl;
                var links = ExtractSisbolDownloadLinks(driver, currentUrl);
                if (links.Count > 0)
                {
                    progress?.Report("Baixando PDF do Indice por Assunto...");
                    await DownloadSisbolFileAsync(http, links[0].Url, destination, cancellationToken);
                    result.FilePath = destination;
                    HideBrowser(driver);
                    return result;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var structuredOpeningText = RemoveTrailingConsequences(plainText);
        var openingText = BulletinTableCodec.ToPlainText(structuredOpeningText);
        ...
        var payload = new SisbolMatterPayload
        {
            ...
            OpeningTextHtml = BuildHtml(structuredOpeningText, military),
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // A gravação ocorre em uma fila independente. O envio confirmado não
            // fica preso esperando leitura ou escrita do arquivo de histórico.
            History.Enqueue(payload, military);
        }
        catch (Exception ex)
        {
    ...
    private static void AddChromiumArguments(Action<string> add, string profile)
    {
        ...
        add("--window-size=1280,900");
        // O navegador precisa nascer visível para a etapa humana do captcha. Após
        // autenticar, HideBrowser remove a janela da tela e da barra de tarefas.
        add("--window-position=60,40");
    }
            ...
            try
            {
                var result = ((IJavaScriptExecutor)driver).ExecuteScript(@"
                    try {
                        ...
                        function style(el){try{el.style.removeProperty('font-family');el.style.removeProperty('font-size');el.style.removeProperty('line-height');el.style.textAlign='justify'}catch(e){}}
    ...
    private static bool LooksLikeAuthenticatedSisbolPage(IWebDriver driver)
    {
        try
        {
            if (LooksLikeLoginOrCaptcha(driver)) return false;
            var url = driver.Url ?? string.Empty;
            if (!Uri.TryCreate(url, UriKind.Absolute, out var uri)
                || !uri.AbsolutePath.Contains("/band/", StringComparison.OrdinalIgnoreCase)) return false;

            var result = ((IJavaScriptExecutor)driver).ExecuteScript(@"
                function norm(s){return String(s||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase();}
                const ready=document.readyState==='complete'||document.readyState==='interactive';
                const body=norm(document.body?document.body.innerText:'').replace(/\s+/g,' ').trim();
                const inputs=Array.from(document.querySelectorAll('input'));
                const meta=e=>norm((e.name||'')+' '+(e.id||'')+' '+(e.placeholder||'')+' '+(e.title||''));
                const password=inputs.some(i=>String(i.type||'').toLowerCase()==='password');
                const captcha=inputs.some(i=>/captcha|caracter|codigo|seguranca/.test(meta(i)));
                const strong=/sair|logout/.test(body);
                const authenticated=strong||/boletim|materia|indice|consultar|lancar nota|menu principal/.test(body);
                const loginWords=/captcha|caracteres|codigo de seguranca|autenticacao|senha|\bentrar\b/.test(body);
                return Boolean(ready&&!password&&!captcha&&body.length>30&&authenticated&&(!loginWords||strong));");
            return result is bool authenticated && authenticated;
        }
        catch { return false; }
    }
    ...
    private static string CanonicalSisbolSpecificSubject(string? subject)
    {
        ...
        // Espaçamos somente o traço que já funciona como separador. Um hífen colado
        // pertence à palavra (AUXÍLIO-NATALIDADE, SALÁRIO-FAMÍLIA etc.) e deve ser
        // preservado, pois a pesquisa do SisBol também diferencia essas grafias.
        text = Regex.Replace(text, @"\s*[–—]\s*", " - ");
        text = Regex.Replace(text, @"(?:\s+-\s*|\s*-\s+)", " - ").Trim();
        // Também houve modelo salvo com TRASNPORTE digitado errado; normalizamos
        ...
        if (OperatingSystem.IsWindows())
        {
            try
            {
                ...
                foreach (var handle in handles)
                {
                    ...
                    // Alterna TOPMOST apenas para atravessar a proteção de foco do
                    // Windows; imediatamente devolve a janela ao nível normal.
    ...
    private List<IntPtr> FindBrowserWindows(bool visibleOnly)
    {
        ...
        foreach (var pid in pids)
        {
            try
            {
                ...
                if (IsPrimaryBrowserWindow(handle, requireTitle: false)
                    && (!visibleOnly || IsWindowVisible(handle))) result.Add(handle);
            }
            catch { }
        }
        ...
        EnumWindows((handle, _) =>
        {
            try
            {
                ...
                if (IsPrimaryBrowserWindow(handle, requireTitle: true)) result.Add(handle);
            }
            catch { }
            return true;
        }, IntPtr.Zero);
    ...
    private bool IsPrimaryBrowserWindow(IntPtr handle, bool requireTitle)
    {
        if (handle == IntPtr.Zero || !IsWindow(handle) || GetParent(handle) != IntPtr.Zero) return false;
        if (!GetWindowRect(handle, out var rect) || rect.Right - rect.Left < 400 || rect.Bottom - rect.Top < 250) return false;

        var classBuffer = new StringBuilder(128);
        if (GetClassName(handle, classBuffer, classBuffer.Capacity) <= 0) return false;
        var className = classBuffer.ToString();
        var expectedClass = NormalizeBrowser(_browser) == "firefox" ? "MozillaWindowClass" : "Chrome_WidgetWin_1";
        if (!className.Equals(expectedClass, StringComparison.OrdinalIgnoreCase)) return false;

        if (!requireTitle) return true;
        var titleLength = GetWindowTextLength(handle);
        if (titleLength <= 0) return false;
        var title = new StringBuilder(titleLength + 1);
        return GetWindowText(handle, title, title.Capacity) > 0 && !string.IsNullOrWhiteSpace(title.ToString());
    }
