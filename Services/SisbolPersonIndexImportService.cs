...
public sealed class SisbolPersonIndexImportService
{
    ...
    public async Task EnsureDatabaseAsync(CancellationToken cancellationToken = default)
    {
        ...
        command.CommandText = """
            ...
            CREATE INDEX IF NOT EXISTS ix_sisbol_person_index_date_person ON sisbol_person_index(bulletin_date, person_name);
            CREATE INDEX IF NOT EXISTS ix_sisbol_person_index_date_subject ON sisbol_person_index(bulletin_date, main_subject, sub_subject);
            """;
    ...
    public async Task<SisbolPersonIndexImportResult> ImportAsync(string sourcePdfPath, IProgress<string>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var imported = await SaveImportedRowsAsync(storedPath, hash, parsed, cancellationToken);
            ...
            var result = new SisbolPersonIndexImportResult
            {
                ...
                Imported = imported,
                Linked = parsed
                    .DistinctBy(BuildImportRowKey, StringComparer.OrdinalIgnoreCase)
                    .Count(x => x.MilitaryId.HasValue)
            };
    ...
    /// <summary>
    /// Retorna as linhas individuais do índice (antes do agrupamento visual), necessárias
    /// para reconciliar cada publicação do BI com a pessoa, página e nota corretas.
    /// </summary>
    public async Task<IReadOnlyList<SisbolPersonIndexItem>> LoadRowsForReconciliationAsync(CancellationToken cancellationToken = default)
    {
        await EnsureDatabaseAsync(cancellationToken);
        return await LoadCachedRowsAsync(cancellationToken);
    }
    ...
    private async Task<int> SaveImportedRowsAsync(string sourcePath, string hash, IReadOnlyList<SisbolPersonIndexItem> rows, CancellationToken cancellationToken)
    {
        var uniqueRows = rows
            .GroupBy(BuildImportRowKey, StringComparer.OrdinalIgnoreCase)
            .Select(group => group.First())
            .ToList();
        var rangeStart = uniqueRows.Select(x => x.IndexStartDate).FirstOrDefault(x => x is not null)
                         ?? uniqueRows.Select(x => x.BulletinDate).Min();
        var rangeEnd = uniqueRows.Select(x => x.IndexEndDate).FirstOrDefault(x => x is not null)
                       ?? uniqueRows.Select(x => x.BulletinDate).Max();
        ...
        await using (var delete = connection.CreateCommand())
        {
            ...
            // Cada PDF é a fonte oficial apenas para o período que ele cobre. Apagar a
            // tabela inteira fazia o download de 2026 eliminar 2025, 2024 etc. Substituímos
            // somente o intervalo importado e preservamos todo o restante do histórico.
            delete.CommandText = rangeStart is not null && rangeEnd is not null
                ? """
                  DELETE FROM sisbol_person_index
                  WHERE source_pdf_hash = $hash
                     OR (bulletin_date IS NOT NULL AND bulletin_date >= $start_date AND bulletin_date <= $end_date);
                  """
                : """
                  DELETE FROM sisbol_person_index
                  WHERE source_pdf_hash = $hash OR source_pdf_path = $source_path;
                  """;
            ...
            {
                delete.Parameters.AddWithValue("$start_date", ToIsoDate(rangeStart));
                delete.Parameters.AddWithValue("$end_date", ToIsoDate(rangeEnd));
            }
        ...
        insert.CommandText = """
            ...
            ($source_pdf_path, $source_pdf_hash, $index_start_date, $index_end_date, $rank, $person_name, $military_id,
             ...
             $note_number, $sisbol_user, $search_text, $created_at, $updated_at);
            """;
        CreateParameters(insert);
        foreach (var item in uniqueRows)
        {
            SetParameterValues(insert, item);
    ...
    private static string BuildImportRowKey(SisbolPersonIndexItem item)
        => string.Join('|',
            item.BulletinType,
            item.BulletinNumber,
            item.BulletinDate?.ToString("yyyyMMdd", CultureInfo.InvariantCulture),
            item.BulletinPage?.ToString(CultureInfo.InvariantCulture),
            item.NoteNumber,
            NormalizeForSearch(item.PersonName),
            NormalizeForSearch(item.MainSubject),
            NormalizeForSearch(item.SubSubject));

    private static void CreateParameters(SqliteCommand command)
    {
        foreach (var name in new[]
                 {
                     "$source_pdf_path", "$source_pdf_hash", "$index_start_date", "$index_end_date", "$rank", "$person_name",
                     "$military_id", "$main_subject", "$sub_subject", "$subject_note", "$bulletin_type", "$bulletin_number",
                     "$bulletin_date", "$bulletin_page", "$note_number", "$sisbol_user", "$search_text", "$created_at", "$updated_at"
                 })
            command.Parameters.Add(new SqliteParameter(name, DBNull.Value));
    }

    private static void SetParameterValues(SqliteCommand command, SisbolPersonIndexItem item)
    {
        command.Parameters["$source_pdf_path"].Value = item.SourcePdfPath;
        command.Parameters["$source_pdf_hash"].Value = item.SourcePdfHash;
        command.Parameters["$index_start_date"].Value = ToIsoDate(item.IndexStartDate);
        command.Parameters["$index_end_date"].Value = ToIsoDate(item.IndexEndDate);
        command.Parameters["$rank"].Value = item.Rank;
        command.Parameters["$person_name"].Value = item.PersonName;
        command.Parameters["$military_id"].Value = item.MilitaryId.HasValue ? item.MilitaryId.Value : DBNull.Value;
        command.Parameters["$main_subject"].Value = item.MainSubject;
        command.Parameters["$sub_subject"].Value = item.SubSubject;
        command.Parameters["$subject_note"].Value = item.SubjectNote;
        command.Parameters["$bulletin_type"].Value = item.BulletinType;
        command.Parameters["$bulletin_number"].Value = item.BulletinNumber;
        command.Parameters["$bulletin_date"].Value = ToIsoDate(item.BulletinDate);
        command.Parameters["$bulletin_page"].Value = item.BulletinPage.HasValue ? item.BulletinPage.Value : DBNull.Value;
        command.Parameters["$note_number"].Value = item.NoteNumber;
        command.Parameters["$sisbol_user"].Value = item.SisbolUser;
        command.Parameters["$search_text"].Value = item.SearchText;
        command.Parameters["$created_at"].Value = item.CreatedAt.ToString("O", CultureInfo.InvariantCulture);
        command.Parameters["$updated_at"].Value = item.UpdatedAt.ToString("O", CultureInfo.InvariantCulture);
    }
