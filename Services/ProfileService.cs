...
public sealed class ProfileService
{
    ...
    public ProfileService(JsonFileService json, SecurityService security, string? profileStoreDirectory = null)
    {
        _json = json;
        _security = security;
        _profileStoreDirectory = string.IsNullOrWhiteSpace(profileStoreDirectory)
            ? ProfileStoreDirectory
            : Path.GetFullPath(profileStoreDirectory);
    }
    ...
    /// <summary>
    /// Remove somente o vínculo deste computador. Dados locais, perfil sincronizado
    /// e backups permanecem intactos e podem ser vinculados novamente.
    /// </summary>
    public async Task<bool> RemoveLocalProfileAsync(SigfurProfileConfig config)
    {
        ArgumentNullException.ThrowIfNull(config);
        var profiles = await LoadLocalProfilesAsync();
        var removed = profiles.RemoveAll(profile =>
            profile.ProfileName.Equals(config.ProfileName, StringComparison.OrdinalIgnoreCase)
            && SamePath(profile.SyncFolderPath, config.SyncFolderPath)) > 0;
    ...
    private string LocalProfilesPath()
    {
        Directory.CreateDirectory(_profileStoreDirectory);
        return Path.Combine(_profileStoreDirectory, LocalProfilesFileName);
    }
