...
public sealed class MilitaryPreferenceService
{
    ...
    {
        // Arquivos independentes: lê todos juntos para não somar quatro esperas
        // de disco sempre que o Listar Militares é aberto ou atualizado.
        var favoritesTask = LoadBooleanMapAsync(FavoritesPath);
        var attachedTask = LoadBooleanMapAsync(AttachedPath);
        var notesTask = LoadStringMapAsync(NotesPath);
        var colorsTask = LoadStringMapAsync(ColorsPath);
        await Task.WhenAll(favoritesTask, attachedTask, notesTask, colorsTask);
        var favorites = favoritesTask.Result;
        var attached = attachedTask.Result;
    ...
    public async Task SetColorsAsync(IEnumerable<MilitaryRecord> records, string? color)
    {
        var selected = records
            ...
            .DistinctBy(x => x.Id > 0
                ...
                : $"TMP:{MilitaryFormatting.Digits(x.Cpf)}|{MilitaryFormatting.Digits(x.PrecCp)}|{x.Name}" )
            .ToList();
        if (selected.Count == 0) return;

        var normalizedColor = NormalizeColor(color);

        // Atualiza os objetos imediatamente, inclusive registros sem ID válido.
        // Antes, registros filtrados por ID<=0 simplesmente não mudavam na tela.
        foreach (var record in selected) record.CustomColor = normalizedColor;

        var persisted = selected.Where(x => x.Id > 0).DistinctBy(x => x.Id).ToList();
        ...
        try
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(normalizedColor)) map.Remove(record.Id);
                else map[record.Id] = normalizedColor;
            }

            // Não chamar SaveMapAsync aqui: ele também entra no mesmo SemaphoreSlim,
            // causando deadlock e fazendo a janela parecer que aplicou a cor sem mudar nada.
            var payload = map.ToDictionary(x => x.Key.ToString(CultureInfo.InvariantCulture), x => x.Value);
            await _json.SaveAsync(ColorsPath, payload);
        }
        finally
        {
            _gate.Release();
    ...
    private static string NormalizeColor(string? color)
    {
        ...
        if (text.Length is not (7 or 9)) return string.Empty;
        return text.Skip(1).All(Uri.IsHexDigit) ? text.ToUpperInvariant() : string.Empty;
    }
