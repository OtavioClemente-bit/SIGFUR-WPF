...
public sealed class AppPaths
{
    public AppPaths(string? dataDirectory = null)
    {
        ...
        ConsolidateLegacyTemplateFolders();
    }
    ...
    public string MilitaryPhotosDirectory => Path.Combine(MilitaryDocumentsDirectory, "Fotos");
    ...
    public string SippesDirectory => Path.Combine(DataDirectory, "SIPPES");
    public string SippesDadosMaDirectory => Path.Combine(SippesDirectory, "Dados_Militar_Ativa");
    public string SippesEspelhoContrachequeOmDirectory => Path.Combine(SippesDirectory, "Espelho_Contracheque_OM");
    public string SippesLastConferenceFile => Path.Combine(SippesDirectory, "conferencia_efetivo_sippes_ultima.json");
    public string SippesLastMirrorConferenceFile => Path.Combine(SippesDirectory, "conferencia_espelho_contracheque_om_ultima.json");
    /// <summary>
    /// Única pasta oficial de modelos do SIGFUR. Todos os módulos apontam para
    /// este endereço para impedir colisões entre arquivos genéricos como
    /// CAPA_TEMPLATE.docx.
    /// </summary>
    public string UnifiedTemplatesDirectory => Path.Combine(DataDirectory, "Templates");
    public string DocumentTemplatesDirectory => UnifiedTemplatesDirectory;
    ...
    public string GratificationTemplatesDirectory => UnifiedTemplatesDirectory;
    public string GratificationSpedAttachmentsDirectory => Path.Combine(DataDirectory, "gratificacao_representacao", "anexos_sped");
    public string GratificationProcessesDirectory => Path.Combine(DataDirectory, "gratificacao_representacao", "processos");
    public string GratificationProcessesFile => Path.Combine(DataDirectory, "gratificacao_representacao", "processos.json");
    public string SpedDirectory => Path.Combine(DataDirectory, "SPED");
    public string SpedSettingsFile => Path.Combine(SpedDirectory, "sped_config_wpf.json");
    public string SpedProcessSettingsFile => Path.Combine(SpedDirectory, "sped_processos_config_wpf.json");
    public string SpedMappingDirectory => Path.Combine(SpedDirectory, "mapeamentos");
    public string SpedProcessesDirectory => Path.Combine(SpedDirectory, "processos_salvos");
    public string ExercisePreviousDirectory => Path.Combine(DataDirectory, "EA");
    public string ExercisePreviousTemplatesDirectory => UnifiedTemplatesDirectory;
    ...
    public string PaystubAuditCacheDirectory => Path.Combine(CacheDirectory, "PaystubAudit");
    public string PaymentConferenceCacheDirectory => Path.Combine(CacheDirectory, "PaymentConference");
    public string AssistantInterpretationCacheDirectory => Path.Combine(CacheDirectory, "AssistantInterpretation");
    ...
    public string RedDaysFile => Path.Combine(DataDirectory, "calendar_red_days_wpf.json");
    ...
    public string PhpmTemplatesDirectory => UnifiedTemplatesDirectory;
    ...
    public string LegislationBuiltInDirectory => Path.Combine(AppContext.BaseDirectory, "Resources", "Legislacao", "Fontes");
    ...
    public string JudicialPensionTemplatesDirectory => UnifiedTemplatesDirectory;
    ...
    public string MeasuresTakenProcessesDirectory => Path.Combine(MeasuresTakenOutputDirectory, "Processos");
    ...
    public string SisbolSubmissionHistoryFile => Path.Combine(DataDirectory, "sisbol", "historico_envios.json");
    ...
    public string EbMailSettingsFile => Path.Combine(DataDirectory, "ebmail_envio_config.json");
    public string EbMailBrowserProfileDirectory => Path.Combine(DataDirectory, "ebmail_browser_profile_wpf");
    public string WhatsAppSettingsFile => Path.Combine(DataDirectory, "whatsapp_compartilhamento_config.json");
    public string WhatsAppBrowserProfileDirectory => Path.Combine(DataDirectory, "whatsapp_browser_profile_wpf");
    ...
    private void ConsolidateLegacyTemplateFolders()
    {
        Directory.CreateDirectory(UnifiedTemplatesDirectory);
        var legacyFolders = new[]
        {
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "SIGFUR", "templates", "docs"),
            Path.Combine(DataDirectory, "gratificacao_representacao", "templates"),
            Path.Combine(DataDirectory, "EA", "templates"),
            Path.Combine(DataDirectory, "phpm", "templates"),
            Path.Combine(DataDirectory, "pensao_judicial", "templates"),
            Path.Combine(DataDirectory, "templates", "phpm")
        };

        foreach (var folder in legacyFolders
                     .Where(Directory.Exists)
                     .Where(x => !Path.GetFullPath(x).Equals(Path.GetFullPath(UnifiedTemplatesDirectory), StringComparison.OrdinalIgnoreCase))
                     .Distinct(StringComparer.OrdinalIgnoreCase))
        {
            try
            {
                foreach (var source in Directory.EnumerateFiles(folder, "*.*", SearchOption.AllDirectories))
                {
                    var module = folder.Contains($"{Path.DirectorySeparatorChar}EA{Path.DirectorySeparatorChar}", StringComparison.OrdinalIgnoreCase) ? "EA"
                        : folder.Contains("phpm", StringComparison.OrdinalIgnoreCase) ? "PHPM"
                        : folder.Contains("gratificacao", StringComparison.OrdinalIgnoreCase) ? "GRAT_REP"
                        : folder.Contains("pensao_judicial", StringComparison.OrdinalIgnoreCase) ? "PENSAO_JUDICIAL"
                        : "LEGADO";
                    var fileName = Path.GetFileName(source);
                    var destination = Path.Combine(UnifiedTemplatesDirectory, fileName);
                    if (File.Exists(destination) && !FilesEqual(source, destination))
                        destination = Path.Combine(UnifiedTemplatesDirectory, $"{module}_{fileName}");
            ...
            {
                // Nunca impede a abertura do SIGFUR por causa de um arquivo
                // antigo bloqueado. O conteúdo já copiado permanece preservado.
            }
        }
    }
    ...
    private static bool FilesEqual(string first, string second)
    {
        ...
        while (true)
        {
            var count = left.Read(x, 0, x.Length);
            if (count != right.Read(y, 0, y.Length)) return false;
