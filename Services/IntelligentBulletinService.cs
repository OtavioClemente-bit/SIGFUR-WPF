...
public sealed class IntelligentBulletinService
{
    ...
    public const int CurrentParserVersion = 17;
        ...
        try
        {
            ...
            for (var index = 0; index < files.Count; index++)
            {
                ...
                try
                {
                    ...
                    {
                        MigrateReviews(existing.Findings, parsed.Findings, reviews);
    ...
    public async Task ReindexAsync(string fileId, IProgress<string>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var reviews = await ReadReviewsUnsafeAsync(cancellationToken);
            MigrateReviews(item.Findings, reparsed.Findings, reviews);
    ...
    public async Task<IntelligentBulletinReconciliationResult> ReconcileFurrielWithPersonIndexAsync(
        IReadOnlyList<SisbolPersonIndexItem> indexRows,
        IProgress<string>? progress = null,
        CancellationToken cancellationToken = default)
    {
        var result = new IntelligentBulletinReconciliationResult();
        var publications = BuildIndexPublications(indexRows);
        var publicationsByNumber = publications
            .GroupBy(publication => BulletinNumeric(publication.Row.BulletinNumber))
        ...
        {
            var store = await ReadStoreUnsafeAsync(cancellationToken);
            result.FilesScanned = store.Items.Count;
            var changedStore = false;

            for (var fileIndex = 0; fileIndex < store.Items.Count; fileIndex++)
            {
                cancellationToken.ThrowIfCancellationRequested();
                var file = store.Items[fileIndex];
                var findings = file.Findings.Where(IsFurrielFinding).ToList();
                if (findings.Count == 0) continue;

                progress?.Report($"Conferindo consequências {fileIndex + 1}/{store.Items.Count}: {file.FileName}");
                var fileChanged = false;
                var bulletinNumber = BulletinNumeric(file.BulletinNumber);
                publicationsByNumber.TryGetValue(bulletinNumber, out var bulletinPublications);
                bulletinPublications ??= [];
                var fileDate = ParseReconciliationDate(file.DateIso, file.BulletinDate);
                var dateCandidates = NarrowPublicationsByDate(bulletinPublications, fileDate, file.BulletinNumber);

                foreach (var finding in findings)
                {
                    result.FurrielFindings++;
                    var match = FindBestIndexPublication(finding, fileDate, dateCandidates);
                    if (match is null)
                    {
                        result.Unmatched++;
                        continue;
                    ...
                    var row = match.Publication.Row;
                    var display = ComposeIndexSubjectNote(row);
                    var subject = FirstNonEmpty(row.MainSubject, row.SubSubject, row.SubjectNote, "Assunto não identificado");
                    var noteTitle = FirstNonEmpty(row.SubSubject, row.SubjectNote);
                    var detail = BuildIndexMatchDetail(row, match.Reason);
                    var coreChanged = !finding.IsPersonIndexMatch
                                      || !string.Equals(finding.Subject, subject, StringComparison.Ordinal)
                                      || !string.Equals(finding.SubjectNoteDisplay, display, StringComparison.Ordinal)
                                      || !string.Equals(finding.NoteTitle, noteTitle, StringComparison.Ordinal)
                                      || !string.Equals(finding.PersonIndexNoteNumber, row.NoteNumber, StringComparison.Ordinal)
                                      || !string.Equals(finding.PersonIndexMatchDetail, detail, StringComparison.Ordinal);

                    finding.Subject = subject;
                    finding.SubjectNoteDisplay = display;
                    finding.NoteTitle = noteTitle;
                    finding.IsPersonIndexMatch = true;
                    finding.PersonIndexNoteNumber = row.NoteNumber;
                    finding.PersonIndexMatchDetail = detail;
                    if (coreChanged)
                        finding.PersonIndexUpdatedAt = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss", PtBr);
                ...
                changedStore = true;
                await WriteParseCacheUnsafeAsync(file, cancellationToken);
            }
    ...
    public async Task RemoveAsync(string fileId, bool deletePdf, CancellationToken cancellationToken = default)
        => _ = await RemoveManyAsync([fileId], deletePdf, cancellationToken);

    public async Task<int> RemoveManyAsync(IEnumerable<string> fileIds, bool deletePdf, CancellationToken cancellationToken = default)
    {
        var ids = fileIds
        ...
        {
            var store = await ReadStoreUnsafeAsync(cancellationToken);
            var items = store.Items.Where(item => ids.Contains(item.Id)).ToList();
            if (items.Count == 0) return 0;

            store.Items.RemoveAll(item => ids.Contains(item.Id));
            var reviews = await ReadReviewsUnsafeAsync(cancellationToken);
            foreach (var finding in items.SelectMany(item => item.Findings))
            {
                reviews.Remove(finding.Id);
                reviews.Remove(finding.ReviewStorageKey);
            }
            await WriteStoreUnsafeAsync(store, cancellationToken);
            await WriteReviewsUnsafeAsync(reviews, cancellationToken);
            if (deletePdf)
            {
                foreach (var path in items
                             .SelectMany(item => new[] { item.PdfPath, item.TextCachePath, item.ParseCachePath })
                             .Where(path => !string.IsNullOrWhiteSpace(path))
                             .Distinct(StringComparer.OrdinalIgnoreCase))
                {
                    try { if (File.Exists(path)) File.Delete(path); }
                    catch (Exception ex) { await _log.WriteAsync("Falha ao apagar arquivo removido da biblioteca: " + path, ex); }
                }
            }
            return items.Count;
        }
    ...
    public async Task SetReviewedAsync(IntelligentBulletinFinding finding, bool reviewed, string note = "", CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var reviews = await ReadReviewsUnsafeAsync(cancellationToken);
            if (reviewed)
                reviews[finding.ReviewStorageKey] = new IntelligentBulletinReviewEntry { Ok = true, Category = finding.Category, Bulletin = finding.Bulletin, File = finding.FileName, Military = finding.DisplayMilitary, Summary = finding.Detail, Note = note, ReviewedAt = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss", PtBr) };
            else
            {
                reviews.Remove(finding.ReviewStorageKey);
                reviews.Remove(finding.Id);
    ...
    public async Task<IReadOnlyList<PaymentAmbiguousBlock>> FindAiRescueCandidatesAsync(IEnumerable<string> fileIds,
        CancellationToken cancellationToken = default)
    {
        var wanted = fileIds.ToHashSet(StringComparer.OrdinalIgnoreCase);
        var store = await LoadAsync(cancellationToken);
        var result = new List<PaymentAmbiguousBlock>();
        var relevant = new Regex(@"\b(FURRIEL|PAGAMENTO|SAQUE|ATRASAD[OA]S?|AUXILIO|FERIAS|GRATIFICACAO|ADICIONAL|IMPLANTACAO|EXCLUSAO|ALTERACAO|PENSAO|DESCONTO|RESTITUICAO)\b", RegexOptions.IgnoreCase);
        foreach (var file in store.Items.Where(file => wanted.Count == 0 || wanted.Contains(file.Id)))
        {
            cancellationToken.ThrowIfCancellationRequested();
            var textPath = ResolveDataPath(file.TextCachePath);
            if (!File.Exists(textPath)) continue;
            var pages = (await File.ReadAllTextAsync(textPath, cancellationToken)).Split('\f');
            for (var pageIndex = 0; pageIndex < pages.Length; pageIndex++)
            {
                var text = pages[pageIndex];
                if (!relevant.IsMatch(Normalize(text))) continue;
                var findings = file.Findings.Where(item => item.Page == pageIndex + 1).ToList();
                if (findings.Count > 0 && findings.All(item => item.IsDatabaseMatch
                    && !string.IsNullOrWhiteSpace(item.Subject) && !item.Subject.Equals("Furriel", StringComparison.OrdinalIgnoreCase)
                    && !item.Category.Equals("Outros", StringComparison.OrdinalIgnoreCase))) continue;
                result.Add(new PaymentAmbiguousBlock
                {
                    SourceFile = file.PdfPath, BulletinNumber = file.BulletinNumber, BulletinDate = file.BulletinDate,
                    Page = pageIndex + 1, Text = text,
                    Reason = findings.Count == 0 ? "Página relevante sem achado estruturado." : "Achado sem militar/assunto seguro ou categoria genérica."
                });
            }
        }
        return result;
    ...
    public async Task<int> MergeAiRescueFindingsAsync(IEnumerable<AiRescuePublication> rescued,
        CancellationToken cancellationToken = default)
    {
        await _gate.WaitAsync(cancellationToken);
        try
        {
            var store = await ReadStoreUnsafeAsync(cancellationToken);
            var added = 0;
            foreach (var source in rescued)
            {
                var file = store.Items.FirstOrDefault(item => item.PdfPath.Equals(source.SourceFile, StringComparison.OrdinalIgnoreCase)
                    || Path.GetFileName(item.PdfPath).Equals(Path.GetFileName(source.SourceFile), StringComparison.OrdinalIgnoreCase));
                if (file is null) continue;
                var key = string.Join('|', file.HashSha256, source.Page, Normalize(source.SourceExcerpt), Normalize(source.MilitaryName), Normalize(source.Subject));
                var id = ShortHash("ai-rescue|" + key);
                if (file.Findings.Any(item => item.Id == id)) continue;
                file.Findings.Add(new IntelligentBulletinFinding
                {
                    Id = id, Category = string.IsNullOrWhiteSpace(source.FinancialEffect) ? "Outros" : "Pagamento",
                    Type = source.FinancialEffect, Bulletin = file.BulletinNumber, BulletinDate = file.BulletinDate,
                    Military = source.MilitaryName, FullName = source.MilitaryName, MentionedCpf = source.Cpf,
                    MentionedPrecCp = source.PrecCp, Subject = source.Subject, NoteTitle = source.Subject,
                    SubjectNoteDisplay = source.Subject, Detail = source.ReasoningSummary, Context = source.SourceExcerpt,
                    NoteText = source.SourceExcerpt, Page = source.Page, FileName = file.FileName, PdfPath = file.PdfPath,
                    PdfSearchTerm = source.MilitaryName.Length > 0 ? source.MilitaryName : source.SourceExcerpt,
                    HasConsequence = true, IsFurrielConsequence = true, InterpretationOrigin = "IA_RESCUE",
                    AiConfidence = source.Confidence, RequiresHumanReview = true, Reviewed = false
                });
                added++;
            }
            foreach (var file in store.Items)
            {
                file.Findings = file.Findings.OrderBy(item => item.Page).ThenBy(item => item.DisplaySubject).ToList();
                await WriteParseCacheUnsafeAsync(file, cancellationToken);
            }
            await WriteStoreUnsafeAsync(store, cancellationToken);
            return added;
    ...
    private IntelligentBulletinFile ParsePdf(IReadOnlyList<string> pages, string sourcePath, IReadOnlyList<IntelligentBulletinMilitaryOption> people)
    {
        ...
        var file = new IntelligentBulletinFile
        {
            ...
            FileName = Path.GetFileName(sourcePath), Pages = pages.Count, Mentions = [], ParserVersion = CurrentParserVersion
        };

        // Uma ocorrência representa a nota/consequência, não cada militar citado.
        // A tela escolhe o responsável (Furriel, 1ª Seção etc.) sobre este índice completo.
        file.Findings = BuildConsequenceFindings(pages, number, dateText, parsedDate, sourcePath)
            .OrderBy(x => x.Page)
    ...
    public static bool ConsequenceMentions(IntelligentBulletinFinding finding, string? keyword)
    {
        if (string.IsNullOrWhiteSpace(keyword) || keyword.Equals("Todos os responsáveis", StringComparison.OrdinalIgnoreCase))
            return true;
        var text = Normalize(finding.ConsequenceText);
        var phrase = Normalize(keyword);
        if (text.Length == 0 || phrase.Length == 0) return false;
        return Regex.IsMatch(text, $@"(?:^|\s){Regex.Escape(phrase)}(?:$|\s)");
    }

    public static IReadOnlyList<IntelligentBulletinFinding> ExtractConsequencesFromPages(
        IReadOnlyList<string> pages,
        string bulletinNumber = "—",
        string bulletinDate = "—",
        string sourcePath = "boletim.pdf")
        => BuildConsequenceFindings(pages, bulletinNumber, bulletinDate, null, sourcePath);

    private static List<IntelligentBulletinFinding> BuildConsequenceFindings(
        IReadOnlyList<string> pages,
        string bulletinNumber,
        ...
        string sourcePath)
    {
        ...
        for (var pageIndex = 0; pageIndex < pages.Count; pageIndex++)
        {
            ...
            for (var lineIndex = 0; lineIndex < lines.Count; lineIndex++)
            {
                ...
                var start = FindConsequenceNoteStart(lines, lineIndex);
                var consequenceHasList = Regex.IsMatch(lines[lineIndex], @":\s*$");
                var end = FindConsequenceEnd(lines, lineIndex, consequenceHasList);
                var noteLines = lines.Skip(start).Take(end - start + 1).ToList();
                var consequenceLines = lines.Skip(lineIndex).Take(end - lineIndex + 1).ToList();
                if (end == lines.Count - 1 && pageIndex + 1 < pages.Count)
                {
                    var continuation = ExtractConsequencePageContinuation(CleanFurrielPageLines(pages[pageIndex + 1]), consequenceHasList);
                    consequenceLines.AddRange(continuation);
                    noteLines.AddRange(continuation);
                }
                var noteText = OneLine(string.Join(" ", noteLines));
                var consequenceText = OneLine(string.Join(" ", consequenceLines));
                if (!ContainsConsequenceWord(consequenceText)) continue;

                var (subject, noteTitle) = DetectConsequenceSubject(lines, start, lineIndex);
                var displaySubject = BuildSubjectNoteDisplay(subject, noteTitle);
                var detectedNote = ExtractDetectedNoteNumber(noteText);
                var pageNumber = pageIndex + 1;
                var key = string.Join('|', "consequencia", bulletinNumber, pageNumber.ToString(CultureInfo.InvariantCulture), lineIndex.ToString(CultureInfo.InvariantCulture), Normalize(consequenceText));
                if (!seen.Add(key)) continue;
                var reviewKey = string.Join('|', "ciencia-consequencia", bulletinNumber, pageNumber.ToString(CultureInfo.InvariantCulture), Normalize(detectedNote), Normalize(noteText), Normalize(consequenceText));

                results.Add(new IntelligentBulletinFinding
                {
                    Id = ShortHash(key),
                    ReviewKey = ShortHash(reviewKey),
                    Category = "Consequência",
                    Type = "Consequência",
                    ...
                    DetectedNoteNumber = detectedNote,
                    Preview = TruncateText(noteText, 280),
                    Detail = noteText,
                    Context = noteText,
                    NoteText = noteText,
                    HasConsequence = true,
                    IsFurrielConsequence = ContainsFurrielWord(consequenceText),
                    ConsequenceText = consequenceText,
                    Page = pageNumber,
                    FileName = Path.GetFileName(sourcePath),
                    PdfPath = sourcePath,
                    PdfSearchTerm = string.IsNullOrWhiteSpace(detectedNote) ? "Em consequência" : $"Nota {detectedNote}"
                });
            }
        }

        return results
            .GroupBy(item => item.ReviewStorageKey, StringComparer.OrdinalIgnoreCase)
            .Select(group => group.First())
            .ToList();
    }

    private static int FindConsequenceNoteStart(IReadOnlyList<string> lines, int consequenceIndex)
    {
        var lowerLimit = Math.Max(0, consequenceIndex - 70);
        for (var index = consequenceIndex - 1; index >= lowerLimit; index--)
        {
            if (IsConsequenceLine(lines[index])) return index + 1;
            if (IsFurrielBlockBoundary(lines[index]) || IsProbableFurrielSubject(lines[index])) return index;
        }
        return lowerLimit;
    }

    private static int FindConsequenceEnd(IReadOnlyList<string> lines, int consequenceIndex, bool consequenceHasList)
    {
        var limit = Math.Min(lines.Count - 1, consequenceIndex + (consequenceHasList ? 40 : 12));
        var end = consequenceIndex;
        for (var index = consequenceIndex + 1; index <= limit; index++)
        {
            if (IsConsequenceLine(lines[index])) break;
            if (IsMajorBulletinBoundary(lines[index])) break;
            if (IsFurrielBlockBoundary(lines[index]) &&
                (!consequenceHasList || !IsConsequenceListContinuation(lines[index]))) break;
            if (!consequenceHasList && index > consequenceIndex + 1 && IsProbableFurrielSubject(lines[index])) break;
            end = index;
            if (IsConsequenceClosingLine(lines[index])) break;
        }
        return end;
    }

    private static IReadOnlyList<string> ExtractConsequencePageContinuation(IReadOnlyList<string> nextPageLines, bool consequenceHasList)
    {
        var continuation = new List<string>();
        foreach (var line in nextPageLines.Take(consequenceHasList ? 40 : 12))
        {
            if (IsConsequenceLine(line) || IsMajorBulletinBoundary(line)) break;
            if (IsFurrielBlockBoundary(line) &&
                (!consequenceHasList || !IsConsequenceListContinuation(line))) break;
            if (!consequenceHasList && continuation.Count > 0 && IsProbableFurrielSubject(line)) break;
            continuation.Add(line);
            if (IsConsequenceClosingLine(line)) break;
        }
        return continuation;
    }

    private static bool IsMajorBulletinBoundary(string line)
        => Regex.IsMatch(line, @"^\d+[ªa]?\s+PARTE\b", RegexOptions.IgnoreCase);

    private static bool IsConsequenceListContinuation(string line)
    {
        var content = CleanFurrielHeading(line);
        return line.Contains(':') || (content.Length > 0 && char.IsLower(content[0]));
    }

    private static bool IsConsequenceClosingLine(string line)
    {
        var text = Normalize(line);
        return text.Contains("interessad", StringComparison.Ordinal) &&
               (text.Contains("providenc", StringComparison.Ordinal) || text.Contains("conhecimento", StringComparison.Ordinal));
    }

    private static (string Subject, string NoteTitle) DetectConsequenceSubject(
        IReadOnlyList<string> lines, int start, int consequenceIndex)
    {
        var first = start >= 0 && start < lines.Count ? lines[start] : string.Empty;
        var noteTitle = IsFurrielBlockBoundary(first) ? CleanFurrielHeading(first) : string.Empty;
        var subject = string.Empty;

        for (var index = consequenceIndex - 1; index >= Math.Max(0, start - 55); index--)
        {
            if (index == start && IsFurrielBlockBoundary(lines[index])) continue;
            if (!IsProbableFurrielSubject(lines[index])) continue;
            subject = CleanFurrielHeading(lines[index]);
            break;
        }

        if (string.IsNullOrWhiteSpace(subject) && IsProbableFurrielSubject(first))
            subject = CleanFurrielHeading(first);
        if (string.IsNullOrWhiteSpace(noteTitle) && !string.IsNullOrWhiteSpace(subject))
            noteTitle = subject;
        if (string.IsNullOrWhiteSpace(subject)) subject = "Assunto não identificado";
        if (string.IsNullOrWhiteSpace(noteTitle)) noteTitle = "Nota com consequência";
        return (subject, noteTitle);
    }

    private static string BuildSubjectNoteDisplay(string subject, string noteTitle)
    {
        if (string.IsNullOrWhiteSpace(subject)) return noteTitle;
        if (string.IsNullOrWhiteSpace(noteTitle) || Normalize(subject) == Normalize(noteTitle)) return subject;
        return $"{subject} — {noteTitle}";
    }

    private static string ExtractDetectedNoteNumber(string text)
    {
        var match = Regex.Match(text ?? string.Empty, @"\bNota\s+(?:n[ºo°]?|nr\.?)?\s*[:#-]?\s*(\d{2,10})\b", RegexOptions.IgnoreCase);
    ...
    private static bool IsFurrielBlockBoundary(string line)
        => Regex.IsMatch(line, @"^(?:\d+[ªa]?\s+PARTE\b|[A-Za-z][.)]\s+|\d+[.)-]\s+)", RegexOptions.IgnoreCase)
    ...
    private static bool IsProbableFurrielSubject(string line)
    {
        ...
        if (Regex.IsMatch(text, @"^(?:transcrito|assina|portaria|despacho|publicado|confere|em cumprimento|belo horizonte)\b", RegexOptions.IgnoreCase)) return false;
    ...
    private sealed class IndexPublication
    {
        public required SisbolPersonIndexItem Row { get; init; }
        public required IReadOnlyList<SisbolPersonIndexItem> People { get; init; }
    }

    private sealed class IndexPublicationMatch
    {
        public required IndexPublication Publication { get; init; }
        public required int Score { get; init; }
        public required string Reason { get; init; }
        public bool Ambiguous { get; init; }
    }

    private static List<IndexPublication> BuildIndexPublications(IReadOnlyList<SisbolPersonIndexItem>? rows)
        => (rows ?? [])
            .Where(row => BulletinNumeric(row.BulletinNumber) > 0)
            .GroupBy(row => string.Join('|',
                BulletinNumeric(row.BulletinNumber).ToString(CultureInfo.InvariantCulture),
                row.BulletinDate?.ToString("yyyyMMdd") ?? string.Empty,
                row.BulletinPage?.ToString(CultureInfo.InvariantCulture) ?? string.Empty,
                Normalize(row.NoteNumber), Normalize(row.MainSubject), Normalize(row.SubSubject), Normalize(row.SubjectNote)),
                StringComparer.OrdinalIgnoreCase)
            .Select(group => new IndexPublication { Row = group.First(), People = group.ToList() })
            .ToList();

    private static IReadOnlyList<IndexPublication> NarrowPublicationsByDate(
        IReadOnlyList<IndexPublication> publications,
        DateTime? bulletinDate,
        string bulletinNumber)
    {
        if (publications.Count == 0) return publications;
        if (bulletinDate.HasValue)
        {
            var exact = publications.Where(publication => publication.Row.BulletinDate?.Date == bulletinDate.Value.Date).ToList();
            if (exact.Count > 0) return exact;
        }

        var year = bulletinDate?.Year ?? ExtractBulletinYear(bulletinNumber);
        if (year > 0)
        {
            var sameYear = publications.Where(publication => publication.Row.BulletinDate?.Year == year).ToList();
            if (sameYear.Count > 0) return sameYear;
        }
        return publications;
    }

    private static IndexPublicationMatch? FindBestIndexPublication(
        IntelligentBulletinFinding finding,
        DateTime? bulletinDate,
        IReadOnlyList<IndexPublication> publications)
    {
        if (publications.Count == 0) return null;
        var context = Normalize(string.Join(' ', finding.Context, finding.ConsequenceText, finding.Detail, finding.NoteText));
        // "Furriel" é o responsável citado na consequência, não o nome da pessoa da nota.
        // Só usamos identidade nominal quando o parser realmente encontrou o militar.
        var findingName = Normalize(finding.FullName);
        var scored = new List<(IndexPublication Publication, int Score, int Person, int Page, int Note, string Reason)>();

        foreach (var publication in publications)
        {
            var row = publication.Row;
            var dateScore = bulletinDate.HasValue && row.BulletinDate.HasValue
                ? row.BulletinDate.Value.Date == bulletinDate.Value.Date ? 300 : row.BulletinDate.Value.Year == bulletinDate.Value.Year ? 80 : 0
                : 0;
            var pageDifference = row.BulletinPage.HasValue ? Math.Abs(row.BulletinPage.Value - finding.Page) : int.MaxValue;
            var pageScore = pageDifference switch { 0 => 500, 1 => 260, 2 => 120, _ => 0 };
            var noteScore = ExplicitNoteNumberMatches(context, row.NoteNumber) ? 760 : 0;

            var personScore = 0;
            foreach (var person in publication.People)
            {
                var score = 0;
                if (finding.MilitaryId.HasValue && person.MilitaryId == finding.MilitaryId) score = 1000;
                var candidateNames = new[] { person.LinkedFullName, person.PersonName, person.LinkedWarName }
                    .Select(Normalize)
                    .Where(name => name.Length >= 3)
                    .Distinct(StringComparer.Ordinal)
                    .ToList();
                if (findingName.Length >= 5 && candidateNames.Any(name => name == findingName)) score = Math.Max(score, 920);
                else if (findingName.Length >= 8 && candidateNames.Any(name => name.Contains(findingName, StringComparison.Ordinal) || findingName.Contains(name, StringComparison.Ordinal))) score = Math.Max(score, 820);
                if (candidateNames.Where(name => name.Length >= 7).Any(name => ContainsWholeNormalizedPhrase(context, name))) score = Math.Max(score, 780);
                personScore = Math.Max(personScore, score);
            }

            var subjectTokens = NameTokens(string.Join(' ', row.MainSubject, row.SubSubject, row.SubjectNote))
                .Where(token => token.Length >= 4 && token is not "nota" and not "boletim" and not "interno" and not "militar")
                .Distinct(StringComparer.Ordinal)
                .Count(token => ContainsWholeNormalizedPhrase(context, token));
            var textScore = Math.Min(180, subjectTokens * 35);
            var scoreTotal = dateScore + pageScore + noteScore + personScore + textScore;
            if (findingName.Length >= 5 && personScore == 0) scoreTotal -= 180;
            var reason = string.Join(", ", new[]
            {
                personScore > 0 ? "militar" : string.Empty,
                noteScore > 0 ? "número da nota" : string.Empty,
                pageScore >= 500 ? "página" : pageScore > 0 ? "página próxima" : string.Empty,
                dateScore >= 300 ? "data" : dateScore > 0 ? "ano" : string.Empty,
                textScore > 0 ? "contexto" : string.Empty
            }.Where(value => value.Length > 0));
            scored.Add((publication, scoreTotal, personScore, pageScore, noteScore, reason));
        }

        var ordered = scored.OrderByDescending(candidate => candidate.Score).ToList();
        var best = ordered[0];
        var hasStrongIdentity = best.Person >= 780 || best.Note >= 760;
        var hasExactLocation = best.Page >= 500 && (bulletinDate is null || best.Score >= 800);
        if (best.Score < 700 || (!hasStrongIdentity && !hasExactLocation)) return null;

        var second = ordered.Skip(1).FirstOrDefault();
        var ambiguous = second.Publication is not null
                        && second.Score >= 700
                        && best.Score - second.Score < (hasStrongIdentity ? 80 : 150);
        return new IndexPublicationMatch
        {
            Publication = best.Publication,
            Score = best.Score,
            Reason = string.IsNullOrWhiteSpace(best.Reason) ? "BI e publicação" : best.Reason,
            Ambiguous = ambiguous
        };
    }

    private static bool ExplicitNoteNumberMatches(string normalizedContext, string? noteNumber)
    {
        var digits = Digits(noteNumber);
        if (digits.Length < 2 || string.IsNullOrWhiteSpace(normalizedContext)) return false;
        return Regex.IsMatch(normalizedContext, $@"(?:\bnota\b|\bnr\b|\bnumero\b)[^0-9]{{0,12}}0*{Regex.Escape(digits)}\b", RegexOptions.IgnoreCase);
    }

    private static string ComposeIndexSubjectNote(SisbolPersonIndexItem row)
    {
        var subject = FirstNonEmpty(row.MainSubject, row.SubSubject, row.SubjectNote, "Assunto não identificado");
        var note = FirstNonEmpty(row.SubjectNote, row.SubSubject);
        if (string.IsNullOrWhiteSpace(note) || Normalize(note) == Normalize(subject)) return subject;
        if (Normalize(note).StartsWith(Normalize(subject), StringComparison.Ordinal)) return note;
        return $"{subject} — {note}";
    }

    private static string BuildIndexMatchDetail(SisbolPersonIndexItem row, string reason)
    {
        var reference = new List<string> { $"BI {row.BulletinNumber}" };
        if (row.BulletinDate.HasValue) reference.Add(row.BulletinDate.Value.ToString("dd/MM/yyyy"));
        if (row.BulletinPage.HasValue) reference.Add($"página {row.BulletinPage.Value}");
        if (!string.IsNullOrWhiteSpace(row.NoteNumber)) reference.Add($"nota {row.NoteNumber}");
        return $"Conferido no Índice por Pessoa ({string.Join(" · ", reference)}). Vínculo: {reason}.";
    }

    private static DateTime? ParseReconciliationDate(string? dateIso, string? dateText)
    {
        if (DateTime.TryParseExact(dateIso, "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None, out var iso)) return iso;
        if (DateTime.TryParse(dateText, PtBr, DateTimeStyles.AllowWhiteSpaces, out var parsed)) return parsed;
        return null;
    }

    private static int ExtractBulletinYear(string? bulletinNumber)
        => int.TryParse(Regex.Match(bulletinNumber ?? string.Empty, @"\b20\d{2}\b").Value, out var year) ? year : 0;

    private static bool IsFurrielFinding(IntelligentBulletinFinding finding)
        => finding.HasConsequence
           || finding.IsFurrielConsequence
           || finding.Category.Equals("Furriel", StringComparison.OrdinalIgnoreCase)
           || ContainsFurrielWord(string.Join(' ', finding.Type, finding.Context, finding.ConsequenceText));
    ...
    private async Task<bool> UpgradeStaleParserItemsUnsafeAsync(IntelligentBulletinStore store, Dictionary<string, IntelligentBulletinReviewEntry> reviews, CancellationToken cancellationToken)
    {
        ...
        foreach (var item in staleItems)
        {
            ...
            {
                IReadOnlyList<string> pages;
                if (!string.IsNullOrWhiteSpace(item.TextCachePath) && File.Exists(item.TextCachePath))
                    pages = (await File.ReadAllTextAsync(item.TextCachePath, cancellationToken)).Split('\f');
                else
                    pages = await ExtractPdfPagesAsync(item.PdfPath, cancellationToken);
                var reparsed = ParsePdf(pages, item.PdfPath, people);
                reparsed.Id = item.Id;
                reparsed.HashSha256 = item.HashSha256;
                reparsed.PdfPath = item.PdfPath;
    ...
    private static void ApplyReviews(IntelligentBulletinStore store, IReadOnlyDictionary<string, IntelligentBulletinReviewEntry> reviews)
    {
        foreach (var item in store.Items)
        foreach (var finding in item.Findings)
            finding.Reviewed = (reviews.TryGetValue(finding.ReviewStorageKey, out var review)
                                || reviews.TryGetValue(finding.Id, out review)) && review.Ok;
    }

    private static void MigrateReviews(
        IReadOnlyList<IntelligentBulletinFinding> oldFindings,
        IReadOnlyList<IntelligentBulletinFinding> newFindings,
        Dictionary<string, IntelligentBulletinReviewEntry> reviews)
    {
        foreach (var current in newFindings)
        {
            if (reviews.ContainsKey(current.ReviewStorageKey)) continue;
            var prior = oldFindings
                .Where(old => old.Page == current.Page &&
                              (reviews.ContainsKey(old.ReviewStorageKey) || reviews.ContainsKey(old.Id)))
                .OrderByDescending(old => ReviewEvidenceSimilarity(old, current))
                .FirstOrDefault(old => ReviewEvidenceSimilarity(old, current) >= 0.42);
            if (prior is null) continue;
            if (!reviews.TryGetValue(prior.ReviewStorageKey, out var entry) &&
                !reviews.TryGetValue(prior.Id, out entry)) continue;
            reviews[current.ReviewStorageKey] = entry;
        }

        var liveKeys = newFindings
            .SelectMany(finding => new[] { finding.Id, finding.ReviewStorageKey })
            .ToHashSet(StringComparer.OrdinalIgnoreCase);
        foreach (var old in oldFindings)
        {
            if (!liveKeys.Contains(old.Id)) reviews.Remove(old.Id);
            if (!liveKeys.Contains(old.ReviewStorageKey)) reviews.Remove(old.ReviewStorageKey);
        }
    }

    private static double ReviewEvidenceSimilarity(IntelligentBulletinFinding left, IntelligentBulletinFinding right)
    {
        var leftText = Normalize(string.Join(' ', left.ConsequenceText, left.Subject, left.NoteTitle));
        var rightText = Normalize(string.Join(' ', right.ConsequenceText, right.Subject, right.NoteTitle));
        if (leftText.Length == 0 || rightText.Length == 0) return 0;
        if (leftText == rightText) return 1;
        if (leftText.Contains(rightText, StringComparison.Ordinal) || rightText.Contains(leftText, StringComparison.Ordinal)) return 0.85;
        var leftTokens = leftText.Split(' ', StringSplitOptions.RemoveEmptyEntries).Where(token => token.Length >= 4).ToHashSet();
        var rightTokens = rightText.Split(' ', StringSplitOptions.RemoveEmptyEntries).Where(token => token.Length >= 4).ToHashSet();
    ...
    private async Task WriteParseCacheUnsafeAsync(IntelligentBulletinFile item, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(item.ParseCachePath))
            item.ParseCachePath = Path.Combine(_paths.IntelligentBulletinParseDirectory, Path.GetFileNameWithoutExtension(item.FileName) + "_" + item.Id + ".json");
        Directory.CreateDirectory(Path.GetDirectoryName(item.ParseCachePath)!);
        await File.WriteAllTextAsync(item.ParseCachePath, JsonSerializer.Serialize(item, JsonOptions), new UTF8Encoding(false), cancellationToken);
    }
