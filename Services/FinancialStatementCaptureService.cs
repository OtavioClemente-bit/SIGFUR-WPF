...
public sealed class FinancialStatementCaptureService
{
    ...
    public const string LegacyPortalUrl = "https://cpex-intranet.eb.mil.br/ficha_financeira.asp";
    public const int ModernPortalStartYear = 2021;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var precCp = MilitaryFormatting.Digits(military.PrecCp);
        var usesLegacyPortal = year < ModernPortalStartYear;
        var identifier = cpf.Length == 11 ? cpf : usesLegacyPortal ? precCp : string.Empty;
        if (string.IsNullOrWhiteSpace(identifier))
            throw new InvalidOperationException(usesLegacyPortal
                ? "O militar não possui CPF ou PREC-CP válido para pesquisar a ficha financeira antiga."
                : "O militar não possui CPF válido para pesquisar a ficha financeira.");
        ...
        progress?.Report($"{(identifier == cpf ? "CPF" : "PREC-CP")} sem pontuação copiado. Abrindo o portal da ficha financeira…");
        ShellService.OpenPath(GetPortalUrl(year));
    ...
    public static string GetPortalUrl(int year)
        => year >= ModernPortalStartYear ? PortalUrl : LegacyPortalUrl;
