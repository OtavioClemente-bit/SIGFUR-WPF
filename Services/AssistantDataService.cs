...
public sealed class AssistantDataService
{
    ...
    public JsonArray BuildToolDefinitions()
    {
        ...
        [
            ...
            Tool("conferir_pagamento_ia",
                "Executa ou reutiliza uma conferência determinística de TODOS os aditamentos do Furriel indexados existentes contra contracheques da competência. Retorna evidências paginadas, não um parecer IA. Sempre declare escopo/cobertura; rubrica não localizada não prova não pagamento. Leia as páginas restantes para cobrir todos os itens.",
                new JsonObject
                {
                    ...
                    ["inicio"] = IntegerProperty("Deslocamento da página, começando em 0. Use proximo_inicio retornado para continuar.", 0, 1000000),
                    ["salvar_relatorio"] = BooleanProperty("Salvar CSV da conferência na pasta oficial de relatórios.")
                }, ["ano", "mes", "limite_linhas", "inicio", "salvar_relatorio"]),

            Tool("consultar_pendencias_operacionais",
                "Consulta somente leitura a visão consolidada de pendências, prazos registrados, lembretes, corrida de pagamento e processos de exercícios anteriores em andamento. Distingue prazos cadastrados de sugestões; não inventa datas legais. Leia avisos e paginação.",
                new JsonObject
                {
                    ["inicio"] = IntegerProperty("Deslocamento da página, começando em 0.", 0, 1000000),
                    ["limite"] = IntegerProperty("Quantidade máxima de itens desta página.", 1, 100)
                }, ["inicio", "limite"]),

            Tool("criar_lembrete_operacional",
                "Prepara um lembrete operacional para revisão antes de salvar no SIGFUR quando o operador pedir para lembrar, cobrar ou não esquecer uma providência.",
                new JsonObject
                {
    ...
    public JsonArray BuildToolDefinitions(IReadOnlySet<string> allowedNames)
    {
        if (allowedNames is null || allowedNames.Count == 0) return [];

        var selected = new JsonArray();
        foreach (var tool in BuildToolDefinitions().OfType<JsonObject>())
        {
    ...
    public async Task<AssistantToolExecutionResult> ExecuteAsync(string name, JsonObject arguments, AssistantSettings settings, CancellationToken cancellationToken)
    {
        ...
        if (!settings.EnableLocalDataTools)
            return Result(new { success = false, error = "Consultas aos módulos desativadas nas configurações." }, "Ferramentas indisponíveis");
        try
        {
            var result = name switch
            {
                ...
                "consultar_pendencias_operacionais" => await GetOperationalPendingItemsAsync(arguments, settings, cancellationToken),
            ...
            var explicitSensitiveAccess = (name is "consulta_operacional_militar" or "buscar_militar" or "consultar_resumo_militar" or "buscar_licenciados_transferidos")
        ...
        IReadOnlyList<LegislationSearchHit> legislationHits)
    {
        ...
        return $"{title}\n\n{legal} seja providenciado o lançamento administrativo referente a {subject.Trim().ToUpperInvariant()} {action.Trim().ToLowerInvariant()}, em favor do militar abaixo identificado, conforme documentação de amparo correspondente.\n\nMilitar: {identification}.\n{required}\n{OptionalLine("Informações complementares", extra)}\n\nEm consequência, solicito que o Ch SSPP/Cmdo 4ª RM processe o lançamento correspondente.";
    }
    ...
    private static string BuildVacationBulletinBody(string action, MilitaryRecord? military, int year, string period, string extra, IReadOnlyList<LegislationSearchHit> legislationHits)
    {
        ...
        {
            return $"{title}\n\n{legal}, seja concedido o gozo de férias ao militar {identification}, relativo ao ano de referência {referenceYear}, no período de {periodText}, totalizando [QUANTIDADE DE DIAS] dia(s), conforme planejamento da OM.\n\nDocumento de amparo/publicação anterior: [BI/DIEx/Plano de Férias, se houver].\n{OptionalLine("Informações complementares", extra)}\n\nEm consequência, sejam atualizados os registros funcionais correspondentes.";
        }

        if (title.Contains("Atrasados", StringComparison.OrdinalIgnoreCase))
        {
            return $"{title}\n\n{legal}, seja realizado o saque de atrasados do adicional de férias em favor do militar {identification}, relativo ao ano de referência {referenceYear}, tendo em vista que o direito não foi pago na competência própria.\n\nPeríodo de férias: {periodText}.\nQuantidade de dias: [QUANTIDADE DE DIAS].\nValor solicitado: [VALOR EXIGIDO PELO SIPPES].\nCompetência em que deveria ter sido pago: [PAGAMENTO {paymentYear}/MÊS CORRETO].\nDocumento de amparo: [BI Nr ___, de ___, da OM].\n{OptionalLine("Informações complementares", extra)}\n\nEm consequência, solicito que o Ch SSPP/Cmdo 4ª RM processe o saque de atrasado correspondente.";
        }

        return $"{title}\n\n{legal}, seja realizado o saque do adicional de férias em favor do militar {identification}, relativo ao ano de referência {referenceYear}, em virtude de férias previstas para o período de {periodText}.\n\nQuantidade de dias: [QUANTIDADE DE DIAS].\nPeríodo aquisitivo/ano de referência: {referenceYear}.\nDocumento de amparo: [BI Nr ___, de ___, da OM].\n{OptionalLine("Informações complementares", extra)}\n\nEm consequência, solicito que o Ch SSPP/Cmdo 4ª RM processe o lançamento correspondente.";
    }
    ...
    private async Task<AssistantToolExecutionResult> SearchLegislationAsync(JsonObject args, AssistantSettings settings, CancellationToken ct)
    {
        ...
        var evidence = await App.AssistantKnowledge.BuildContextAsync(query, Math.Min(limit, 6), 9500, ct);
        var rows = evidence.Sources.Select(x => new { referencia = x.Reference, titulo = x.Title, pagina = x.Page, trecho = x.Excerpt, versao = x.Version, identificador_fonte = x.SourceFingerprint }).ToList();
        var actions = evidence.Sources
            .Where(x => !string.IsNullOrWhiteSpace(x.Path) && File.Exists(x.Path))
            .GroupBy(x => x.Path + "|" + x.Page.ToString(CultureInfo.InvariantCulture), StringComparer.OrdinalIgnoreCase)
            .Select(group =>
            {
                var hit = group.First();
                var action = AssistantActionRegistry.OpenFile($"{hit.Title} — pág. {hit.Page}", hit.Path, $"Abrir a fonte local citada: {hit.FileName}, página {hit.Page}.");
                action.Payload["display"] = $"{hit.Title} — pág. {hit.Page}";
                action.Payload["page"] = hit.Page.ToString(CultureInfo.InvariantCulture);
                return action;
            })
            .Take(8)
            .ToList();
        return Result(new
        {
            ...
            quantidade = rows.Count,
            resultados = rows,
            cache_reutilizado = evidence.CacheHit,
            versao_manual = evidence.ManualVersion,
            identificador_acervo = evidence.CorpusFingerprint,
            lacunas = evidence.Gaps,
            aviso = rows.Count == 0 ? "Não há base suficiente. Não invente norma ou artigo; oriente importar/indexar a norma em Legislação." : "Trechos documentais, não parecer pronto. Cite referência/página e confira a vigência para a época do fato."
        }, $"Legislação: {rows.Count} referência(s)", actions);
    }
    ...
    private async Task<AssistantToolExecutionResult> RunPaymentConferenceForAssistantAsync(JsonObject args, CancellationToken ct)
    {
        ...
        var offset = GetInt(args, "inicio", 0, 0, 1000000);
        ...
        var cached = await _paymentConference.LoadValidCacheAsync(selected, settings, ct);
        var result = cached?.Result ?? await _paymentConference.RunAsync(selected, settings, null, ct);
        if (cached is null) await _paymentConference.SaveCacheAsync(result, selected, settings, ct);
        string csv = string.Empty;
        if (saveReport) csv = await _paymentConference.ExportCsvAsync(result, ct);
        var ordered = result.Rows
            .OrderBy(x => x.Severity == "success" ? 1 : 0)
            .ThenBy(x => x.Status, StringComparer.CurrentCultureIgnoreCase)
            .ThenBy(x => x.Military, StringComparer.CurrentCultureIgnoreCase)
            .ToList();
        // Character budget is applied between complete evidence items; never truncate a source excerpt.
        var rows = new List<object>();
        var characters = 0;
        foreach (var row in ordered.Skip(offset).Take(limit))
        {
            var evidence = AssistantWorkflowService.PaymentEvidence(row, $"ITEM {offset + rows.Count + 1:000000}");
            var size = JsonSerializer.Serialize(evidence).Length;
            if (characters + size > 10_500 && rows.Count > 0) break;
            if (size > 10_500)
                return Result(new { success = false, error = $"Item {offset + 1} excede o limite de evidência. Abra a Conferência de Pagamento para revisar o documento integral; nenhum trecho foi cortado.", total_linhas = ordered.Count, inicio = offset }, "Conferência: evidência extensa requer revisão do documento");
            rows.Add(evidence); characters += size;
        }
        return Result(new
        {
            success = true,
            ...
            escopo = "Todos os aditamentos do Furriel indexados com arquivo disponível; contracheques somente da competência configurada.",
            aditamentos_indexados = bulletins.Count,
            aditamentos_indisponiveis = bulletins.Count - selected.Count,
            resultado_reutilizado = cached is not null,
            assinatura_entradas = result.InputSignature,
            resumo = result.Summary,
            avisos = result.Warnings.Take(10).ToList(),
            avisos_omitidos = Math.Max(0, result.Warnings.Count - 10),
            total_linhas = ordered.Count,
            inicio = offset,
            linhas_retornadas = rows.Count,
            proximo_inicio = offset + rows.Count < ordered.Count ? (int?)(offset + rows.Count) : null,
            cobertura_completa_nesta_resposta = offset == 0 && rows.Count == ordered.Count,
            orientacao = "Leia páginas restantes antes de resumir todos. Resultado do extrator é evidência a revisar. Ausência de rubrica/PDF não é prova de não pagamento, e presença não certifica quitação. Não altere valores nem verificação humana.",
            pendencias_e_resultados = rows,
            relatorio_csv = string.IsNullOrWhiteSpace(csv) ? null : csv
        }, $"Conferência de pagamento {settings.Month:00}/{settings.Year}: {rows.Count}/{ordered.Count} itens retornados, {result.Summary.MissingRubric} sem rubrica localizada, {result.Summary.MissingPaystub} sem contracheque");
    }

    private async Task<AssistantToolExecutionResult> GetOperationalPendingItemsAsync(JsonObject args, AssistantSettings settings, CancellationToken ct)
    {
        if (!settings.EnableLocalDataTools) return Result(new { success = false, error = "Consultas aos dados do SIGFUR estão desativadas." }, "Pendências: consulta desativada");
        var snapshot = await App.AssistantOperations.BuildSnapshotAsync(ct);
        var offset = GetInt(args, "inicio", 0, 0, 1000000);
        var limit = GetInt(args, "limite", 30, 1, 100);
        var items = snapshot.Items.Skip(offset).Take(limit).Select(x => new
        {
            id = x.Id, modulo = x.Module, titulo = x.Title, detalhe = x.Detail,
            prazo_registrado = x.DueDate?.ToString("yyyy-MM-dd"), urgente = x.Urgent, situacao = x.Status
        }).ToList();
        return Result(new
        {
            success = true, consultado_em = snapshot.GeneratedAt, total = snapshot.Items.Count,
            inicio = offset, quantidade = items.Count,
            proximo_inicio = offset + items.Count < snapshot.Items.Count ? (int?)(offset + items.Count) : null,
            pendencias = items, avisos = snapshot.Warnings,
            orientacao = "Prazos são os registrados ou calculados pelas regras operacionais do sistema; não são confirmação de prazo legal. Não invente data quando prazo_registrado for null."
        }, $"Pendências operacionais: {items.Count}/{snapshot.Items.Count}");
    }
    ...
    private async Task<AssistantToolExecutionResult> CreateOperationalReminderAsync(JsonObject args, CancellationToken ct)
    {
        ...
        var record = new ReminderRecord
        {
            ...
            Urgent = priority.Equals("Urgente", StringComparison.OrdinalIgnoreCase) || priority.Equals("Urgentíssimo", StringComparison.OrdinalIgnoreCase),
            Recurrence = "Nenhuma",
            AutoReschedule = true,
            Completed = false
        };
        ...
        {
            Type = "create_reminder", Title = "Revisar e salvar lembrete", Description = $"{record.Title} • {record.FormattedDate}",
            RequiresConfirmation = true, Payload = new() { ["record"] = JsonSerializer.Serialize(record) }
        };
        return Result(new
        {
            success = true,
            salvo = false,
            orientacao = "Minuta preparada. O operador deve clicar em Revisar e salvar lembrete, conferir o formulário e salvar. Não afirme que o lembrete já foi criado.",
            titulo = record.Title,
            data = record.FormattedDate,
            prazo = record.DaysText,
            prioridade = record.EffectivePriority,
            descricao = record.Body
        }, $"Lembrete preparado para revisão: {record.Title}", [action]);
    }
    ...
    private static List<AssistantPendingAction> BuildOpenActionsFromBulletinRows(IReadOnlyList<Dictionary<string, object?>> rows, int limit)
    {
        ...
        foreach (var row in rows)
        {
            ...
            if (int.TryParse(page, NumberStyles.Integer, CultureInfo.InvariantCulture, out var pageNumber) && pageNumber > 0)
                action.Payload["page"] = pageNumber.ToString(CultureInfo.InvariantCulture);
            actions.Add(action);
        }
