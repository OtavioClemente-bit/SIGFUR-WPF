...
public sealed class MilitaryDocumentGenerationService
{
    ...
    public async Task<string?> ResolveTemplateAsync(GeneratedDocumentType type)
    {
        ...
        var roots = new[] { _paths.UnifiedTemplatesDirectory };
    ...
    public static IReadOnlyList<(string Key, string Label, string Default)> FieldsFor(GeneratedDocumentType type)
    {
        ...
        return type switch
        {
            ...
            [
                ...
                ("OM_NOME", "Nome da OM", OrganizationIdentity.Name),
                ("CMT_OM", "Cmt da OM / autoridade", OrganizationIdentity.CommandRole),
            ...
            [
                ...
                ("PUBLICACAO_LICENCIAMENTO", "Publicação do licenciamento — BI ou Adt Furr", ""),
            ...
            [
                ...
                ("REMETENTE_NOME", "Remetente", OrganizationIdentity.CommandRole),
                ("OM_NOME", "Nome da OM remetente", OrganizationIdentity.Name),
                ("CMT_OM", "Cmt da OM remetente", OrganizationIdentity.CommandRole),
            ...
            [
                ...
                ("PUBLICACAO_FERIAS", "Publicação das férias — BI ou Adt Furr", ""),
                ("ORDENADOR", "Ordenador de Despesas — nome editável", ""),
                ("UNIDADE_SERVINDO", "Unidade em que serve", OrganizationIdentity.Name),
            ...
            [
                ...
                ("OM_NOME", "OM/lotação", $"servindo na {OrganizationIdentity.Name}"),
    ...
    private static string EnsureOdtWarNameBoldStyle(string xml)
    {
        ...
        const string style = "<style:style style:name=\"SIGFURWarNameBold\" style:family=\"text\"><style:text-properties fo:font-weight=\"bold\" style:font-weight-asian=\"bold\" style:font-weight-complex=\"bold\"/></style:style>";
    ...
    private static string ReplaceOdtWarNameBold(string xml, IReadOnlyDictionary<string, string> mapping)
    {
        ...
        var fullName = mapping.GetValueOrDefault("NOME_COMPLETO", string.Empty);
        var segments = SIGFUR.Wpf.Controls.NameHighlightHelper.BuildSegments(fullName, warName);
        var fragmentBuilder = new StringBuilder();
        foreach (var segment in segments)
        {
            var text = (segment.Text ?? string.Empty).ToUpper(culture);
            if (string.IsNullOrEmpty(text)) continue;
            var encoded = EncodeOdtText(text);
            if (segment.IsBold)
                fragmentBuilder.Append("<text:span text:style-name=\"SIGFURWarNameBold\">").Append(encoded).Append("</text:span>");
            else
                fragmentBuilder.Append(encoded);
        }
        var fragment = fragmentBuilder.ToString();
    ...
    private static string EncodeOdtText(string value)
    {
        ...
        {
            if (index > 0) encoded.Append("<text:s/>");
            encoded.Append(SecurityElement.Escape(parts[index]) ?? string.Empty);
        }
    ...
    private static Dictionary<string, string> BuildMapping(GeneratedDocumentType type, MilitaryRecord m, IReadOnlyDictionary<string, string> fields)
    {
        ...
        if (map.TryGetValue("PUBLICACAO_LICENCIAMENTO", out var licensingPublication))
        {
            map["BI_LICENCIAMENTO"] = licensingPublication;
            map["BI_LIC"] = licensingPublication;
        }
        if (map.TryGetValue("PUBLICACAO_FERIAS", out var vacationPublication))
            map["BI_REFERENCIA"] = vacationPublication;
    ...
    {
        GeneratedDocumentType.TransportAid => ["LISTAR_AUXILIO_TRANSPORTE_TEMPLATE.odt"],
        GeneratedDocumentType.PecuniaryCompensation => ["LISTAR_PECUNIARIA_TEMPLATE.docx"],
        GeneratedDocumentType.AuthenticPaymentCopy => ["LISTAR_COPIA_AUTENTICA_PAGAMENTO_TEMPLATE.docx"],
        GeneratedDocumentType.AdvanceChristmasBonus => ["LISTAR_ADIANTAMENTO_NATALINO_TEMPLATE.odt"],
        GeneratedDocumentType.PostalLabel => ["LISTAR_CORREIOS_TEMPLATE.odt"],
        GeneratedDocumentType.CoverSheet => ["PHPM_CAPA_TEMPLATE.docx"],
        GeneratedDocumentType.ExercisePreviousRequest => ["EA_REQUERIMENTO_TEMPLATE.docx"],
        GeneratedDocumentType.JudicialPensionWorksheet => ["PENSAO_JUDICIAL_TEMPLATE.docx"],
        GeneratedDocumentType.RemissiveIndex => ["PHPM_INDICE_REMISSIVO_TEMPLATE.odt"],
        GeneratedDocumentType.GratificationDiex => ["GRAT_REP_DIEX_TEMPLATE.docx"],
        GeneratedDocumentType.GratificationMap => ["GRAT_REP_MAPA_TEMPLATE.docx"],
        _ => []
    };
