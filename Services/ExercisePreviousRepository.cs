...
public sealed class ExercisePreviousRepository
{
    ...
    [
        ...
        (nameof(ExercisePreviousProcess.CpexNotes), "cpex_obs"),
        (nameof(ExercisePreviousProcess.CpexStatusYear), "cpex_consulta_ano"),
        (nameof(ExercisePreviousProcess.CpexStatusCheckedAt), "cpex_consultado_em"),
        (nameof(ExercisePreviousProcess.CpexApprovalStatus), "cpex_situacao_relatorio"),
        (nameof(ExercisePreviousProcess.CpexApprovalReason), "cpex_motivo_relatorio"),
        (nameof(ExercisePreviousProcess.BankOrderPath), "ordem_bancaria_arquivo"),
        (nameof(ExercisePreviousProcess.BankOrderAttachedAt), "ordem_bancaria_anexada_em"),
        (nameof(ExercisePreviousProcess.Paid), "pago"),
    ...
    public async Task EnsureSchemaAsync(CancellationToken cancellationToken = default)
    {
        ...
        CREATE TABLE IF NOT EXISTS ea_processos (
            ...
            cpex_consulta_ano INTEGER, cpex_consultado_em TEXT,
            cpex_situacao_relatorio TEXT, cpex_motivo_relatorio TEXT,
            ordem_bancaria_arquivo TEXT, ordem_bancaria_anexada_em TEXT,
        ...
        {
            "militar_id" or "ano_processo" or "pago" or "cpex_consulta_ano" => "INTEGER",
            _ => "TEXT"
    ...
    public async Task<ExercisePreviousProcess?> GetLatestForMilitaryAsync(MilitaryRecord military, CancellationToken ct = default)
        => await GetLatestForMilitaryIdentifiersAsync(military.Id, military.Cpf, military.PrecCp, military.MilitaryId, ct);

    public async Task<ExercisePreviousProcess?> GetLatestForMilitaryIdentifiersAsync(
        int militaryId,
        string? cpf,
        string? precCp,
        string? identity,
        CancellationToken ct = default)
    {
        ...
        var precDigits = Digits(precCp);
        var identityDigits = Digits(identity);
        await using var cn = Open();
        await using var cmd = cn.CreateCommand();
        var cpfExpr = "REPLACE(REPLACE(REPLACE(REPLACE(COALESCE(cpf,''),'.',''),'-',''),'/',''),' ','')";
        var precExpr = "REPLACE(REPLACE(REPLACE(REPLACE(COALESCE(prec_cp,''),'.',''),'-',''),'/',''),' ','')";
        var identityExpr = "REPLACE(REPLACE(REPLACE(REPLACE(COALESCE(idt,''),'.',''),'-',''),'/',''),' ','')";
        cmd.CommandText = $"""
            SELECT id
              FROM ea_processos
             WHERE ($id<>0 AND militar_id=$id)
                OR ($cpf<>'' AND {cpfExpr}=$cpf)
                OR ($prec<>'' AND {precExpr}=$prec)
                OR ($idt<>'' AND {identityExpr}=$idt)
             ORDER BY CASE
                    WHEN $id<>0 AND militar_id=$id THEN 0
                    WHEN $cpf<>'' AND {cpfExpr}=$cpf THEN 1
                    WHEN $prec<>'' AND {precExpr}=$prec THEN 2
                    WHEN $idt<>'' AND {identityExpr}=$idt THEN 3
                    ELSE 4 END,
        ...
        cmd.Parameters.AddWithValue("$prec", precDigits);
        cmd.Parameters.AddWithValue("$idt", identityDigits);
        var value = await cmd.ExecuteScalarAsync(ct);
        return value is null or DBNull ? null : await GetAsync(Convert.ToInt32(value, CultureInfo.InvariantCulture), ct);
    }
    ...
    public async Task SaveAutomaticCpexCaptureAsync(
        int processId,
        string pdfPath,
        string? protocol,
        string? protocolledAt,
        CancellationToken ct = default)
    {
        if (processId <= 0) throw new ArgumentOutOfRangeException(nameof(processId));
        if (string.IsNullOrWhiteSpace(pdfPath)) throw new ArgumentException("O caminho do PDF é obrigatório.", nameof(pdfPath));
        ...
        cmd.CommandText = """
            UPDATE ea_processos
               SET cpex_pagina_impressao=$pdf,
                   cpex_protocolo=CASE WHEN $protocolo<>'' THEN $protocolo ELSE cpex_protocolo END,
                   cpex_protocolado_em=CASE WHEN $data<>'' THEN $data ELSE cpex_protocolado_em END,
                   cpex_status=CASE WHEN $protocolo<>'' THEN 'OK' ELSE 'PDF SALVO - CONFERIR PROTOCOLO' END,
                   updated_at=datetime('now')
             WHERE id=$id
            """;
        cmd.Parameters.AddWithValue("$id", processId);
        cmd.Parameters.AddWithValue("$pdf", Path.GetFullPath(pdfPath));
        cmd.Parameters.AddWithValue("$protocolo", CleanDbText(protocol));
        cmd.Parameters.AddWithValue("$data", CleanDbText(protocolledAt));
        if (await cmd.ExecuteNonQueryAsync(ct) != 1)
            throw new InvalidOperationException($"O processo EA {processId:0000} não foi encontrado para vincular o PDF do CPEx.");
    }

    public async Task SaveCpexStatusCheckAsync(int processId, CpexExerciseStatusResult result, CancellationToken ct = default)
    {
        if (processId <= 0) throw new ArgumentOutOfRangeException(nameof(processId));
        ...
        cmd.CommandText = """
            UPDATE ea_processos
               SET cpex_consulta_ano=$ano,
                   cpex_consultado_em=$consultado,
                   cpex_situacao_relatorio=$situacao,
                   cpex_motivo_relatorio=$motivo,
                   updated_at=datetime('now')
             WHERE id=$id
            """;
        cmd.Parameters.AddWithValue("$id", processId);
        cmd.Parameters.AddWithValue("$ano", result.Year);
        cmd.Parameters.AddWithValue("$consultado", CleanDbText(result.CheckedAt));
        cmd.Parameters.AddWithValue("$situacao", CleanDbText(result.Situation));
        cmd.Parameters.AddWithValue("$motivo", CleanDbText(result.Explanation));
        if (await cmd.ExecuteNonQueryAsync(ct) != 1)
            throw new InvalidOperationException($"O processo EA {processId:0000} não foi encontrado para salvar a consulta do CPEx.");
    }
    ...
    public async Task<int> DuplicateAsync(int id, CancellationToken ct = default)
    {
        ...
        source.BankOrderPath = source.BankOrderAttachedAt = string.Empty;
        source.CpexStatusYear = null;
        source.CpexStatusCheckedAt = source.CpexApprovalStatus = source.CpexApprovalReason = string.Empty;
    ...
    public async Task ReplaceIpcaAsync(
        IReadOnlyCollection<(string Competence, double Percentage, double Factor)> rows,
        CancellationToken ct = default)
    {
        if (rows.Count == 0) throw new InvalidDataException("A planilha não contém índices IPCA-E válidos.");
        await using var cn = Open();
        ...
        await using (var delete = cn.CreateCommand())
        {
            ...
            delete.CommandText = "DELETE FROM ipca_e";
        ...
        await using (var insert = cn.CreateCommand())
        {
            ...
            insert.CommandText = "INSERT INTO ipca_e(competencia,percentual,fator) VALUES($c,$p,$f)";
            var competence = insert.Parameters.Add("$c", SqliteType.Text);
            var percentage = insert.Parameters.Add("$p", SqliteType.Real);
            var factor = insert.Parameters.Add("$f", SqliteType.Real);
            foreach (var row in rows.OrderBy(x => x.Competence, StringComparer.Ordinal))
            {
                ct.ThrowIfCancellationRequested();
                competence.Value = row.Competence;
    ...
    public async Task<ExercisePreviousSummary> CalculateSummaryAsync(ExercisePreviousProcess process, CancellationToken ct = default)
    {
        ...
        foreach (var entry in process.Entries)
        {
            ...
            var type = process.Codes.FirstOrDefault(x => x.Order == entry.CodeOrder)?.Type;
            var isExpense = string.Equals(type, "Despesa", StringComparison.OrdinalIgnoreCase);
            var sign = isExpense ? -1m : 1m;

            // Mesma álgebra da planilha oficial: os totais de recebido/devido são
            // RECEITAS - DESPESAS e cada diferença continua sendo DEVIDO - RECEBIDO.
            summary.Received += sign * entry.Received;
            summary.Due += sign * entry.Due;
            summary.CorrectedReceived += sign * entry.CorrectedReceived;
            summary.CorrectedDue += sign * entry.CorrectedDue;

            if (isExpense)
            {
                summary.ExpenseNet += entry.Net;
                summary.CorrectedExpenseNet += entry.CorrectedNet;
            }
            else
            {
                summary.RevenueNet += entry.Net;
                summary.CorrectedRevenueNet += entry.CorrectedNet;
            }
        }
        summary.Net = summary.RevenueNet - summary.ExpenseNet;
        summary.CorrectedNet = summary.CorrectedRevenueNet - summary.CorrectedExpenseNet;
        return summary;
    }
    ...
    private static void Normalize(ExercisePreviousProcess p)
    {
        ...
        p.Account = NormalizeCpexAccount(p.Account);
    ...
    private static void UpgradeNarrativeDefaults(ExercisePreviousProcess p)
    {
        if (SameText(p.RightMaterializationDocument, ExercisePreviousDefaults.RightMaterializationGuide))
            p.RightMaterializationDocument = ExercisePreviousDefaults.ProfessionalRightMaterializationGuide;
        if (SameText(p.NonPaymentExplanation, ExercisePreviousDefaults.NonPaymentGuide))
            p.NonPaymentExplanation = ExercisePreviousDefaults.ProfessionalNonPaymentGuide;
    }

    private static bool SameText(string? left, string? right)
        => CleanDbText(left).Equals(CleanDbText(right), StringComparison.Ordinal);

    public static string Digits(string? value) => new((value ?? string.Empty).Where(char.IsDigit).ToArray());

    /// <summary>
    /// Normaliza contas destinadas ao CPEx sem descartar algarismos significativos.
    /// Importações de contracheque podem preencher a conta à esquerda com zeros;
    /// eles só são removidos quando o valor ultrapassa o limite de 12 posições.
    /// Uma conta que continue maior que 12 é preservada para que a validação do
    /// envio possa bloqueá-la, em vez de truncar silenciosamente o número correto.
    /// </summary>
    public static string NormalizeCpexAccount(string? value)
    {
        var digits = Digits(value);
        if (digits.Length <= 12) return digits;

        var withoutPadding = digits.TrimStart('0');
        return withoutPadding.Length == 0 ? "0" : withoutPadding;
    }
