...
public static class ProfessionalBulletinParser
{
        ...
        IReadOnlyList<BulletinMilitaryIdentity> military)
    {
        ...
        for (var blockIndex = 0; blockIndex < starts.Count; blockIndex++)
        {
            ...
            var noteNumber = noteMatch.Success ? noteMatch.Groups["n"].Value : string.Empty;
            var noteTitle = noteMatch.Success ? $"Nota nº {noteNumber}" : splitHeading.Note;
            ...
            foreach (var person in matches)
            {
                ...
                var idSeed = string.Join('|', bulletinType, bulletinNumber, pageNumber, start, noteNumber, Normalize(structured.Subject), Normalize(structured.NoteTitle), Normalize(person.Name), Digits(person.Cpf), Digits(person.PrecCp));
                result.Add(new BulletinMentionItem
                {
                    ...
                    NoteNumber = noteNumber, SubjectNoteDisplay = display, NoteText = noteText, NoteExcerpt = Excerpt(noteText),
    ...
    {
        var match = Regex.Match(OneLine(value), @"^(?<mark>[a-z]{1,2})\.\s+(?<rest>\S)", RegexOptions.IgnoreCase);
    ...
    {
        var text = OneLine(value);
        if (IsLetteredNoteHeading(text))
            text = Regex.Replace(text, @"^[a-z]{1,2}\.\s*", string.Empty, RegexOptions.IgnoreCase);
        text = Regex.Replace(text, @"^\d+[.)-]\s+", string.Empty).Trim(' ', '-', ':');
