...
public static partial class AssistantIntentDetector
{
    ...
    public static AssistantIntent Detect(string prompt)
    {
        ...
        if (string.IsNullOrWhiteSpace(normalized) || ContainsAny(normalized, "ajuda", "o que voce faz", "o que você faz", "como funciona"))
        ...
        else if (ContainsAny(normalized, "tudo sobre o militar", "tudo sobre a militar", "resumo do militar", "o que tem salvo do militar", "painel do militar", "dossie do militar", "dossiê do militar"))
        ...
        else if (LooksLikeExplicitLegislationQuestion(normalized))
            ...
            intent.Kind = AssistantIntentKind.LegislationResearch;
        else if (LooksLikePersonQuestion(normalized))
            ...
            // Texto livre, cumprimento e pedidos de redação não podem ser tratados como
            // nome de militar. A pesquisa de pessoa só é ativada por sinais explícitos.
            intent.Kind = AssistantIntentKind.GeneralQuestion;
    ...
    private static bool LooksLikeExplicitLegislationQuestion(string normalized)
    {
        if (string.IsNullOrWhiteSpace(normalized)) return false;
        if (ContainsAny(normalized, "boletim", "adt", "aditamento", "nota", "carteira", "contracheque", "ficha financeira"))
            return false;
        return ContainsAny(normalized,
            "legislacao", "lei", "decreto", "portaria", "norma", "artigo", "inciso",
            "amparo legal", "base legal", "fundamento legal", "quem tem direito", "faz jus",
            "pode receber", "deve receber", "como funciona", "regulamento", "estatuto",
            "instrucao normativa", "prazo legal", "requisito legal", "licenca", "afastamento",
            "indenizacao", "beneficio");
    }
    ...
    private static bool LooksLikePersonQuestion(string normalized)
    {
        if (string.IsNullOrWhiteSpace(normalized)) return false;
        if (ContainsAny(normalized,
                "militar", "nome de guerra", "cpf", "prec cp", "preccp", "identidade",
                "dados cadastrais", "cadastro do", "ficha do", "ficha da", "quem e o", "quem e a"))
            return true;

        if (ContainsAny(normalized, "dados do", "dados da", "informacoes do", "informacoes da")
            && !ContainsAny(normalized, "pagamento", "boletim", "relatorio", "documento", "sistema", "legislacao", "processo"))
            return true;

        var asksToFind = ContainsAny(normalized, "localize", "localizar", "procure", "procurar", "busque", "buscar", "encontre", "encontrar");
        var mentionsPerson = ContainsAny(normalized,
            "soldado", "sd", "cabo", "cb", "sargento", "sgt", "subtenente", "subten", "tenente", "capitao", "major", "coronel");
        return asksToFind && mentionsPerson;
    }
