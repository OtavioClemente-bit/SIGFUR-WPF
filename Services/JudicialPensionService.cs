...
public sealed class JudicialPensionService
{
    ...
    private void EnsureTemplateInstalled()
    {
        try
        {
            ...
            var fileName = "PENSAO_JUDICIAL_TEMPLATE.docx";
            var candidates = new[]
            {
    ...
    public static string BuildBulletinText(MilitaryRecord? military, JudicialPensionSettings settings, JudicialPensionCalculationResult result)
    {
        ...
        static string Trim250(string? text)
        {
            var value = Required(text);
            return value.Length > 250 ? value[..250] : value;
        }

        void AppendIdentity(StringBuilder target)
        {
            target.AppendLine("DADOS DO MILITAR / ALIMENTANTE");
            target.AppendLine($"{rank} {displayName}".Trim());
            target.AppendLine($"Prec-CP {MilitaryFormatting.Digits(prec)} CPF {cpf}".Trim());
            target.AppendLine();
        }
        ...
        {
            sb.AppendLine("Seja implantada a pensão alimentícia na remuneração do militar abaixo identificado, conforme decisão judicial/ofício recebido e dados necessários ao lançamento no SIPPES.");
            sb.AppendLine();
            AppendIdentity(sb);
            sb.AppendLine("DADOS DO PROCESSO");
            sb.AppendLine($"Número do processo: {Required(b.ProcessNumber)}.");
            sb.AppendLine($"Origem da decisão: {Required(b.DecisionOrigin)}.");
            ...
            sb.AppendLine($"Resumo da decisão: {Trim250(b.DecisionSummary)}.");
            ...
            sb.AppendLine($"Ofício de recebimento: {Required(b.ReceivingLetter)} | Origem: {Required(b.ReceivingLetterOrigin)} | Data: {Required(b.ReceivingLetterDate)}.");
            sb.AppendLine();
            sb.AppendLine("DADOS DA IMPLANTAÇÃO DA PENSÃO");
            sb.AppendLine($"Alimentante: {rank} {displayName}, CPF {cpf}.");
            ...
            sb.AppendLine($"Valor fixo, fórmula ou percentual da pensão: {Required(b.CalculationRule)}.");
            ...
            sb.AppendLine();
            AppendBank(sb, b);
            if (IsJudicialDeposit(b.Representation))
                sb.AppendLine($"Depósito em juízo: {b.Representation.Trim()}.");
        }
        else if (mode is "ATUALIZAÇÃO")
        {
            sb.AppendLine("Seja atualizada a pensão alimentícia do militar abaixo identificado, conforme dados necessários ao lançamento no SIPPES.");
            sb.AppendLine();
            AppendIdentity(sb);
            sb.AppendLine($"Alimentado(a): {Required(b.BeneficiaryName)}, CPF {Required(b.BeneficiaryCpf)}.");
            sb.AppendLine($"Detentor(a)/representante: {Required(b.GuardianName)}, CPF {Required(b.GuardianCpf)}.");
            if (!string.IsNullOrWhiteSpace(b.ProcessNumber)) sb.AppendLine($"Processo: {b.ProcessNumber.Trim()}.");
            sb.AppendLine($"Item/motivo da atualização: {Required(b.Observation)}.");
            sb.AppendLine($"Dados atualizados: {Required(b.CalculationRule)}.");
            sb.AppendLine($"Resumo da decisão/ofício: {Trim250(b.DecisionSummary)}.");
        }
        else if (mode is "DESCONTO DE ATRASADOS")
        {
            sb.AppendLine("Seja realizado o desconto de atrasados de pensão alimentícia do militar abaixo identificado, conforme dados necessários ao lançamento no SIPPES.");
            sb.AppendLine();
            AppendIdentity(sb);
            sb.AppendLine($"Valor total do desconto: {Required(arrearsValue)}.");
            sb.AppendLine($"Mês de referência: {Required(b.ReferenceMonth)}.");
            sb.AppendLine($"Ano de referência: {Required(b.ReferenceYear)}.");
            sb.AppendLine($"Alimentado(a): {Required(b.BeneficiaryName)}.");
            sb.AppendLine($"Representante/detentor da guarda: {Required(b.GuardianName)}.");
        }
        else
        {
            sb.AppendLine("Seja excluída a pensão alimentícia do militar abaixo identificado, conforme dados necessários ao lançamento no SIPPES.");
            sb.AppendLine();
            AppendIdentity(sb);
            ...
            sb.AppendLine($"Justificativa da exclusão: {Trim250(b.ExclusionJustification)}.");
        ...
        static void AppendBank(StringBuilder sb, JudicialPensionBulletin b)
        {
            ...
            var bank = $"Banco {Required(b.BankCode)} — {Required(b.BankName)}, Agência {Required(b.Agency)}, Conta Corrente {Required(b.Account)}";
            if (isCaixa) bank += $", Operação CAIXA {Required(b.BankOperation)}";
            sb.AppendLine(bank + ".");
        }

        static bool IsJudicialDeposit(string? value)
            => !string.IsNullOrWhiteSpace(value) && value.Contains("dep", StringComparison.OrdinalIgnoreCase)
                                             && value.Contains("ju", StringComparison.OrdinalIgnoreCase);
    }
    ...
    {
        var template = Path.Combine(_paths.JudicialPensionTemplatesDirectory, "PENSAO_JUDICIAL_TEMPLATE.docx");
        ...
        {
            await _log.WriteAsync("Falha ao preencher PENSAO_JUDICIAL_TEMPLATE.docx; usando geração nativa como fallback.", ex);
    ...
    private static Dictionary<string, string> BuildTemplateMap(MilitaryRecord? military, JudicialPensionSettings settings, JudicialPensionCalculationResult result)
    {
        ...
        map["OFICIO_RECEBIMENTO"] = S(b.ReceivingLetter);
        map["ORIGEM_OFICIO"] = S(b.ReceivingLetterOrigin);
        map["DATA_OFICIO"] = S(b.ReceivingLetterDate);
        return map;
    }
