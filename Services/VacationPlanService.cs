...
public sealed class VacationPlanService
{
    ...
    public VacationPlanService(AppPaths paths, JsonFileService json, MilitaryRepository military, PaystubService paystubs, PdfTextService pdf, LogService log)
    {
        _paths = paths;
        _json = json;
        _military = military;
        _paystubs = paystubs;
        _paystubAudit = new PaystubAuditService(paystubs, pdf, log);
        _log = log;
    }
    ...
    public async Task SetPaidAsync(IEnumerable<int> allocationIds, bool paid, bool foodAidConfirmed = false, CancellationToken cancellationToken = default)
    {
        ...
        await using var connection = OpenConnection();
        await using var tx = await connection.BeginTransactionAsync(cancellationToken);
        var annualPayments = new HashSet<(int Year, int MilitaryId)>();
        foreach (var id in ids)
        {
            await using var lookup = connection.CreateCommand();
            lookup.Transaction = (SqliteTransaction)tx;
            lookup.CommandText = "SELECT ano,militar_id FROM ferias_alocacoes_wpf WHERE id=$id LIMIT 1;";
            lookup.Parameters.AddWithValue("$id", id);
            await using var reader = await lookup.ExecuteReaderAsync(cancellationToken);
            if (await reader.ReadAsync(cancellationToken))
                annualPayments.Add((reader.GetInt32(0), reader.GetInt32(1)));
        }

        // O adicional de férias e o auxílio-alimentação de Cb/Sd são pagos uma única
        // vez pelo direito anual completo. Se as férias foram divididas em 10+20 ou
        // 15+15 dias, a baixa financeira precisa alcançar todos os períodos do ano.
        foreach (var payment in annualPayments)
        {
            ...
            cmd.Parameters.AddWithValue("$aux_em", paid && foodAidConfirmed ? (object)now : DBNull.Value);
            cmd.Parameters.AddWithValue("$agora", now);
            cmd.Parameters.AddWithValue("$ano", payment.Year);
            cmd.Parameters.AddWithValue("$militar", payment.MilitaryId);
            await cmd.ExecuteNonQueryAsync(cancellationToken);
        }
        await tx.CommitAsync(cancellationToken);
    }
    ...
    public async Task<Dictionary<string, VacationFinancialProfile>> LoadFinancialProfilesAsync()
    {
        ...
        foreach (var pair in root)
        {
            ...
            var profile = new VacationFinancialProfile
            {
                ...
                QualificationCode = LegacyString(item, "QualificationCode", string.Empty),
            ...
            if (result.TryGetValue(pair.Key, out var current))
            {
                profile.PaystubSalary = current.PaystubSalary;
                profile.PaystubQualificationAdditional = current.PaystubQualificationAdditional;
                profile.PaystubQualificationPercent = current.PaystubQualificationPercent;
                profile.PaystubMilitaryAdditional = current.PaystubMilitaryAdditional;
                profile.PaystubAvailabilityAdditional = current.PaystubAvailabilityAdditional;
                profile.PaystubMilitaryAdditionalPercent = current.PaystubMilitaryAdditionalPercent;
                profile.PaystubAvailabilityPercent = current.PaystubAvailabilityPercent;
                profile.PaystubPnrDiscount = current.PaystubPnrDiscount;
                profile.IncludePnr = current.IncludePnr;
                profile.PnrPercent = current.PnrPercent;
                profile.SalaryOverride = current.SalaryOverride;
                profile.QualificationAdditionalOverride = current.QualificationAdditionalOverride;
                profile.MilitaryAdditionalOverride = current.MilitaryAdditionalOverride;
                profile.AvailabilityAdditionalOverride = current.AvailabilityAdditionalOverride;
                profile.PaystubPath = current.PaystubPath;
                profile.PaystubReference = current.PaystubReference;
                profile.PaystubModifiedAt = current.PaystubModifiedAt;
                profile.PaystubParserVersion = current.PaystubParserVersion;
                profile.PaystubImportError = current.PaystubImportError;
            }
    ...
    public VacationFinancialProfile CreateDefaultFinancialProfile(MilitaryRecord military, int year)
    {
        var profile = new VacationFinancialProfile
        {
            ...
            Year = year,
            Type = VacationProfileType(military),
            IncludePnr = MilitaryRecord.IsYes(military.HasPnr)
        };
        NormalizeFinancialProfile(military, profile);
        return profile;
    }
    ...
    public async Task<VacationFinancialProfile> GetFinancialProfileAsync(MilitaryRecord military, int year)
    {
        ...
        if (profiles.TryGetValue(FinancialKey(military.Id, year), out var profile))
        {
            NormalizeFinancialProfile(military, profile);
            if (await ImportLatestPaystubAsync(military, profile)) await SaveFinancialProfilesAsync(profiles);
            return profile;
        }
        if (profiles.TryGetValue(military.Id.ToString(CultureInfo.InvariantCulture), out profile))
        {
            ...
            profile.Year = year;
            NormalizeFinancialProfile(military, profile);
            if (await ImportLatestPaystubAsync(military, profile))
            {
                profiles[FinancialKey(military.Id, year)] = profile;
                await SaveFinancialProfilesAsync(profiles);
            }
            return profile;
        }
        profile = CreateDefaultFinancialProfile(military, year);
        if (await ImportLatestPaystubAsync(military, profile))
        {
            profiles[FinancialKey(military.Id, year)] = profile;
            await SaveFinancialProfilesAsync(profiles);
    ...
    public async Task<VacationFinancialResult> CalculateFinancialAsync(MilitaryRecord military, int year, VacationFinancialProfile? profile = null, CancellationToken cancellationToken = default)
    {
        ...
        NormalizeFinancialProfile(military, profile);
        var salary = profile.SalaryOverride
                     ?? profile.PaystubSalary
                     ?? await _military.GetSalaryByRankAsync(military.Rank, cancellationToken);
        ...
        if (!profile.HasPaystubValues && !string.IsNullOrWhiteSpace(profile.PaystubPath) && !string.IsNullOrWhiteSpace(profile.PaystubImportError))
            return new VacationFinancialResult { Error = $"O último contracheque ({profile.PaystubReference}) foi localizado, mas seus valores não puderam ser usados: {profile.PaystubImportError}" };
        if (profile.HasPaystubValues)
        {
            decimal PaystubAdd(decimal percent) => Math.Round(salary * percent / 100m, 2, MidpointRounding.AwayFromZero);
            var qualificationFromPaystub = profile.QualificationAdditionalOverride
                                           ?? (profile.PaystubQualificationAdditional is > 0m
                                               ? profile.PaystubQualificationAdditional.Value
                                               : profile.PaystubQualificationPercent is { } qualificationPercent
                                                   ? PaystubAdd(qualificationPercent)
                                                   : 0m);
            var militaryFromPaystub = profile.MilitaryAdditionalOverride
                                      ?? (profile.PaystubMilitaryAdditionalPercent is { } militaryPercent
                                          ? PaystubAdd(militaryPercent)
                                          : profile.PaystubMilitaryAdditional ?? 0m);
            var availabilityFromPaystub = profile.AvailabilityAdditionalOverride
                                          ?? (profile.PaystubAvailabilityPercent is { } availabilityPercent
                                              ? PaystubAdd(availabilityPercent)
                                              : profile.PaystubAvailabilityAdditional ?? 0m);
            var totalFromPaystub = salary + qualificationFromPaystub + militaryFromPaystub + availabilityFromPaystub;
            var pnrFromPaystub = CalculatePnr(profile, salary, out var pnrError);
            if (!string.IsNullOrWhiteSpace(pnrError)) return new VacationFinancialResult { Error = pnrError };
            return new VacationFinancialResult
            {
                Success = true, Salary = salary, QualificationAdditional = qualificationFromPaystub,
                MilitaryAdditional = militaryFromPaystub, AvailabilityAdditional = availabilityFromPaystub,
                BaseTotal = totalFromPaystub,
                VacationAdditional = Math.Round(totalFromPaystub / 3m, 2, MidpointRounding.AwayFromZero),
                PnrDiscount = pnrFromPaystub
            };
        }
        if (profile.QualificationPercent is null || profile.MilitaryAdditionalPercent is null || profile.AvailabilityPercent is null)
            return new VacationFinancialResult { Error = "Selecione o curso de habilitação aplicável; adicional militar e disponibilidade já são definidos pelo P/Grad." };
        decimal Add(decimal percent) => Math.Round(salary * percent / 100m, 2, MidpointRounding.AwayFromZero);
        var qualification = profile.QualificationAdditionalOverride ?? Add(profile.QualificationPercent.Value);
        var militaryAdditional = profile.MilitaryAdditionalOverride ?? Add(profile.MilitaryAdditionalPercent.Value);
        var availability = profile.AvailabilityAdditionalOverride ?? Add(profile.AvailabilityPercent.Value);
        var baseTotal = salary + qualification + militaryAdditional + availability;
        var pnr = CalculatePnr(profile, salary, out var pnrCalculationError);
        if (!string.IsNullOrWhiteSpace(pnrCalculationError)) return new VacationFinancialResult { Error = pnrCalculationError };
        return new VacationFinancialResult
        {
    ...
    public static MilitaryRemunerationProfile VacationRemunerationProfile(string? rank)
        => MilitaryRemunerationCatalog.Resolve(rank,
            MilitaryRankService.Canonicalize(rank) == "Soldado Efetivo Variável"
                ? AdjustmentSituationKind.RecruitSpecialExtension
                : MilitaryRemunerationCatalog.SuggestedSituation(rank));

    private static void NormalizeFinancialProfile(MilitaryRecord military, VacationFinancialProfile profile)
    {
        var legal = VacationRemunerationProfile(military.Rank);
        profile.MilitaryId = military.Id;
        var savedType = MilitaryCareerTypeService.Normalize(profile.Type);
        profile.Type = string.IsNullOrWhiteSpace(savedType) ? VacationProfileType(military) : savedType;
        profile.MilitaryAdditionalPercent = legal.MilitaryAdditionalPercent;
        profile.AvailabilityPercent = legal.AvailabilityPercent;

        if (string.IsNullOrWhiteSpace(profile.QualificationCode))
            profile.QualificationCode = QualificationCodeFromCurrentPercentage(profile.QualificationPercent);

        if (legal.QualificationMode == QualificationSelectionMode.NotDue)
        {
            profile.QualificationCode = QualificationCatalog.NoneCode;
            profile.QualificationPercent = 0m;
        }
        else if (legal.QualificationMode == QualificationSelectionMode.FixedFormation)
        {
            profile.QualificationCode = QualificationCatalog.FormationCode;
            profile.QualificationPercent = 12m;
        }
        else
        {
            var selected = QualificationCatalog.Resolve(profile.QualificationCode);
            if (!legal.IsQualificationAllowed(selected.Code) || selected.IsNone)
            {
                profile.QualificationCode = QualificationCatalog.NoneCode;
                profile.QualificationPercent = null;
            }
            else
            {
                profile.QualificationCode = selected.Code;
                profile.QualificationPercent = selected.Percentage;
            }
        }
    }

    private static string QualificationCodeFromCurrentPercentage(decimal? percentage)
        => QualificationCatalog.All.FirstOrDefault(x => percentage.HasValue && x.Percentage == percentage.Value)?.Code
           ?? QualificationCatalog.NoneCode;

    private static string VacationProfileType(MilitaryRecord military)
        => MilitaryCareerTypeService.Normalize(military.CareerType) is { Length: > 0 } stored
            ? stored
            : MilitaryCareerTypeService.SuggestForRank(military.Rank) ?? MilitaryCareerTypeService.Career;

    private static decimal CalculatePnr(VacationFinancialProfile profile, decimal salary, out string error)
    {
        error = string.Empty;
        if (!profile.IncludePnr) return 0m;
        if (profile.PnrPercent is not (3.5m or 5m))
        {
            error = "Selecione a taxa de uso do PNR: 3,5% para imóvel com despesas condominiais ou 5% para imóvel sem despesas condominiais.";
            return 0m;
        }
        return Math.Round(salary * profile.PnrPercent.Value / 100m, 2, MidpointRounding.AwayFromZero);
    }

    private async Task<bool> ImportLatestPaystubAsync(MilitaryRecord military, VacationFinancialProfile profile, CancellationToken cancellationToken = default)
    {
        var latest = (await _paystubs.FindForMilitaryAsync(military, cancellationToken))
            .OrderByDescending(x => ReferenceSortKey(x.Reference))
            .ThenByDescending(x => x.ModifiedAt)
            .FirstOrDefault();
        if (latest is null)
        {
            if (profile.PaystubImportError == "Nenhum contracheque salvo foi localizado.") return false;
            profile.PaystubImportError = "Nenhum contracheque salvo foi localizado.";
            return true;
        }
        if (string.Equals(profile.PaystubPath, latest.Path, StringComparison.OrdinalIgnoreCase)
            && profile.PaystubModifiedAt == latest.ModifiedAt
            && profile.PaystubParserVersion.Equals(PaystubAuditService.ParserVersion, StringComparison.Ordinal)
            && (profile.HasPaystubValues || !string.IsNullOrWhiteSpace(profile.PaystubImportError)))
            return false;

        var snapshot = await _paystubAudit.ReadRemunerationAsync(military, latest.Path, cancellationToken);
        profile.PaystubPath = latest.Path;
        profile.PaystubReference = latest.Reference;
        profile.PaystubModifiedAt = latest.ModifiedAt;
        profile.PaystubImportError = snapshot.Success ? string.Empty : snapshot.Error;
        profile.PaystubSalary = snapshot.Success ? snapshot.Salary : null;
        profile.PaystubQualificationAdditional = snapshot.Success ? snapshot.QualificationAdditional : null;
        profile.PaystubQualificationPercent = snapshot.Success ? snapshot.QualificationPercent : null;
        profile.PaystubMilitaryAdditional = snapshot.Success ? snapshot.MilitaryAdditional : null;
        profile.PaystubAvailabilityAdditional = snapshot.Success ? snapshot.AvailabilityAdditional : null;
        profile.PaystubMilitaryAdditionalPercent = snapshot.Success ? snapshot.MilitaryAdditionalPercent : null;
        profile.PaystubAvailabilityPercent = snapshot.Success ? snapshot.AvailabilityPercent : null;
        profile.PaystubPnrDiscount = snapshot.Success ? snapshot.PnrDiscount : null;
        profile.PaystubParserVersion = PaystubAuditService.ParserVersion;
        if (snapshot.Success && snapshot.PnrDiscount > 0m)
        {
            profile.IncludePnr = true;
            var inferred = snapshot.Salary > 0m ? snapshot.PnrDiscount / snapshot.Salary * 100m : 0m;
            profile.PnrPercent = Math.Abs(inferred - 3.5m) <= 0.15m ? 3.5m
                : Math.Abs(inferred - 5m) <= 0.15m ? 5m
                : profile.PnrPercent;
        }
        profile.UpdatedAt = DateTime.Now;
        return true;
    ...
    private static int ReferenceSortKey(string? reference)
    {
        var match = Regex.Match(reference ?? string.Empty, @"(?<month>0?[1-9]|1[0-2])\D+(?<year>20\d{2})");
        return match.Success ? int.Parse(match.Groups["year"].Value, CultureInfo.InvariantCulture) * 100
                               + int.Parse(match.Groups["month"].Value, CultureInfo.InvariantCulture) : 0;
    }
    ...
    public async Task<string> GeneratePreviewAsync(
        ...
        int year,
        bool includeLateValues,
        IReadOnlyDictionary<string, string>? formFields = null,
        string? modelName = null,
        IReadOnlyCollection<MilitaryRecord>? selectedMilitary = null,
        int selectedDays = 30,
        CancellationToken cancellationToken = default)
    {
        var allocations = (await ResolveNoteAllocationsAsync(period, year, selectedMilitary, selectedDays, cancellationToken))
            .OrderBy(x => MilitaryRankService.GetOrder(x.Military.Rank))
        ...
        var isVacationAnnualEntitlement = !isVacationFoodAid
        ...
        const int vacationAnnualPaymentDays = 30;
        var isLateVacationFoodAid = isVacationFoodAid
                                    && modelType.Contains("ATRASAD", StringComparison.OrdinalIgnoreCase);
        var foodAidReason = formFields is null
            ? VacationFoodAidSippesReason
            : FirstValue(formFields, "FUNDAMENTO_AUX_ALIMENTACAO_FERIAS", "FUNDAMENTO_AR0058");
        if (string.IsNullOrWhiteSpace(foodAidReason)) foodAidReason = VacationFoodAidSippesReason;
        ...
        foreach (var allocation in allocations)
        {
            ...
            if (isVacationFoodAid)
            {
                ...
                block += Environment.NewLine + $"Quantidade de dias: {vacationAnnualPaymentDays:00}";
                if (isLateVacationFoodAid)
                {
                    var lateFoodAidCode = AuxilioAlimentacaoRubricRule.Resolve(1, AuxilioAlimentacaoTipoLancamento.Atrasado);
                    block += Environment.NewLine + $"Motivo do saque — {lateFoodAidCode}: férias regulamentares ({foodAidReason})";
                    block += Environment.NewLine + $"Código: {lateFoodAidCode} - Aux Alim comum/B atrasado";
                }
                else
                {
                    block += Environment.NewLine + "Motivo do saque: férias regulamentares";
                    block += Environment.NewLine + $"Fundamento legal: {foodAidReason}";
                }
            }
            else if (mustShowIndividualVacationValue)
            {
                ...
                var paymentDays = isVacationAnnualEntitlement ? vacationAnnualPaymentDays : allocation.Days;
                block += Environment.NewLine + $"Período: {period.Name} ({FormatDate(period.StartDate)} a {FormatDate(period.EndDate)})";
                block += Environment.NewLine + $"Ano de referência: {year - 1}";
                block += Environment.NewLine + $"Quantidade: {VacationMonthsText(paymentDays)} · {paymentDays:00} dia(s) de férias";
                if (result.Success)
                {
        ...
        var daysText = isVacationAnnualEntitlement
            ? vacationAnnualPaymentDays.ToString(CultureInfo.InvariantCulture)
            : isVacationFoodAid
                ? vacationAnnualPaymentDays.ToString(CultureInfo.InvariantCulture)
            : distinctDays.Count == 1 ? distinctDays[0].ToString(CultureInfo.InvariantCulture) : "variados";
    ...
    private async Task<List<VacationAllocation>> ResolveNoteAllocationsAsync(
        VacationPeriod period,
        int year,
        IReadOnlyCollection<MilitaryRecord>? selectedMilitary,
        int selectedDays,
        CancellationToken cancellationToken)
    {
        var existing = (await GetAllocationsAsync(year, period.Id, cancellationToken)).ToList();
        if (selectedMilitary is null)
            return existing.Where(x => !x.IsPaid).ToList();

        var byMilitary = existing
            .GroupBy(x => x.MilitaryId)
        ...
        return selectedMilitary
            ...
            .Select(group =>
            {
                var military = group.First();
                return byMilitary.TryGetValue(military.Id, out var allocation)
                    ? allocation
                    : new VacationAllocation
                    {
                        Year = year,
                        PeriodId = period.Id,
                        MilitaryId = military.Id,
                        Military = military,
                        Days = NormalizeDays(selectedDays)
                    };
            })
            .ToList();
    }

    // Title/SisbolSubject identificam e pesquisam o assunto no SisBol.
    // Text deve conter exclusivamente o corpo publicável, sem repetir o título.
    ...
    public const string VacationFoodAidSippesReason = "Art 69 do Dec Nr 4.307/02";

    private const string VacationFoodAidLegalBasis =
        "com fundamento no art. 2º, inciso I, alínea “e”, e na alínea “c” da Tabela III do Anexo IV da Medida Provisória nº 2.215-10, de 31 AGO 01; " +
        "no caput do art. 69 e no § 3º do art. 71 do Decreto nº 4.307, de 18 JUL 02; " +
        "e observadas as condições estabelecidas pela Portaria nº 419-Cmt Ex, de 21 AGO 02";
    ...
    {
        const int annualVacationDays = 30;
        var sippesReason = FirstValue(values, "FUNDAMENTO_AUX_ALIMENTACAO_FERIAS", "FUNDAMENTO_AR0058");
        if (string.IsNullOrWhiteSpace(sippesReason)) sippesReason = VacationFoodAidSippesReason;
        var list = string.Join(Environment.NewLine + Environment.NewLine, allocations.Select(x =>
        {
            var identification = $"{x.Military.ShortRank} {x.Military.Name.ToUpperInvariant()}".Trim() + Environment.NewLine +
                                 $"Prec-CP {Digits(x.Military.PrecCp)} CPF {FormatCpf(x.Military.Cpf)}" + Environment.NewLine;
            if (late)
            {
                var lateFoodAidCode = AuxilioAlimentacaoRubricRule.Resolve(1, AuxilioAlimentacaoTipoLancamento.Atrasado);
                return identification +
                       $"Motivo do saque — {lateFoodAidCode}: férias regulamentares ({sippesReason})" + Environment.NewLine +
                       $"Código: {lateFoodAidCode} - Aux Alim comum/B atrasado | Quantidade de dias: {annualVacationDays:00}";
            }

            // Na ordem de saque normal não há rubrica de atrasados. A publicação
            // informa apenas o motivo, o fundamento legal e a quantidade de dias.
            return identification +
                   "Motivo do saque: férias regulamentares" + Environment.NewLine +
                   $"Fundamento legal: {sippesReason}" + Environment.NewLine +
                   $"Quantidade de dias: {annualVacationDays:00}";
        }));
        var daysText = annualVacationDays.ToString(CultureInfo.InvariantCulture);
        var daysWords = NumberToWordsService.Convert(annualVacationDays, false);
        ...
        return $"{request} do auxílio-alimentação referente a {daysText} ({daysWords}) dia(s) de férias {timing}, em favor exclusivamente do(s) Cabo(s) e Soldado(s) abaixo nominado(s), no mês/período de {monthText}, no {period.Name} ({startText} a {endText}), férias relativas ao ano de {year - 1}, conforme publicado no {biReference}, {VacationFoodAidLegalBasis}." + Environment.NewLine + Environment.NewLine +
    ...
    {
        const int transportWorkingDays = 22;
        ...
        var list = string.Join(Environment.NewLine + Environment.NewLine, allocations.Select(x =>
        {
            ...
            if (monthly <= 0 && x.Military.TransportGrossTotal.GetValueOrDefault() > 0)
                monthly = NormalizeTransportMonthlyValue((decimal)x.Military.TransportGrossTotal.GetValueOrDefault());
            // O valor mensal de AT já é formado sobre 22 dias úteis. Nas férias
            // parceladas a despesa a anular não é fracionada pelo trecho de 10/15 dias:
            // considera-se uma única competência integral de 22 dias úteis.
            var value = monthly;
            return $"{x.Military.ShortRank} {x.Military.Name.ToUpperInvariant()}".Trim() + Environment.NewLine +
                   ...
                   "Motivo: férias" + Environment.NewLine +
                   $"Quantidade de dias úteis a anular: {transportWorkingDays:00}" + Environment.NewLine +
                   $"Valor total a ser descontado: R$ {value.ToString("N2", PtBr)} ({NumberToWordsService.Convert(value, true)})";
        }));
        return $"Seja realizada a despesa a anular do auxílio-transporte, por motivo de férias relativas ao ano de {year - 1}, conforme dados abaixo." + Environment.NewLine + Environment.NewLine +
               list.TrimEnd() + Environment.NewLine;
    }
