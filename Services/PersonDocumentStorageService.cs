...
public static class PersonDocumentStorageService
{
    ...
    {
        Directory.CreateDirectory(paths.PaystubsDirectory);
        return paths.PaystubsDirectory;
    }

    public static string ResolveConfiguredRoot(AppPaths paths)
        => DefaultRoot(paths);

    public static string EnsureWritableRoot(AppPaths paths, string? root)
        => DefaultRoot(paths);
    ...
    public static string PrepareRegisteredFolder(
        ...
        string? prec)
    {
        root = EnsureWritableRoot(paths, root);
        ...
        {
            root = EnsureWritableRoot(paths, paths.PaystubsDirectory);
            destination = BuildFolder(root, rank, name, cpf, prec, external: false);
            Directory.CreateDirectory(destination);
        }
        catch (IOException)
        {
