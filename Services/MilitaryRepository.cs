...
public sealed class MilitaryRepository
{
    ...
    {
        EnsureWritableDatabaseFile(_paths.DatabaseFile);
    ...
    private void EnsureWritableDatabaseFile(string path)
    {
        try
        {
            ...
            if (!full.StartsWith(dataRoot, StringComparison.OrdinalIgnoreCase))
                throw new InvalidOperationException($"Banco fora da pasta de dados do SIGFUR: {full}");
            ...
            if (File.Exists(full))
            {
                ...
                if (attributes.HasFlag(FileAttributes.ReadOnly))
                {
                    ...
                    _ = _log.WriteAsync($"Atributo somente leitura removido do banco oficial: {full}");
                }
            }
        ...
        {
            _ = _log.WriteAsync($"Falha ao validar escrita no banco SQLite: {path}", ex);
            throw new InvalidOperationException($"O banco SQLite não está gravável.\n\nCaminho: {path}\n\nVerifique permissões da pasta de dados do SIGFUR.", ex);
        }
    }
    ...
    public async Task<IReadOnlyList<string>> GetRanksAsync(CancellationToken cancellationToken = default)
    {
        await EnsureSchemaAsync(cancellationToken);
        await using var connection = OpenConnection();
        var list = new List<string>(MilitaryRankService.AllRanks);
        if (await TableExistsAsync(connection, "postos", cancellationToken))
        {
            ...
            while (await reader.ReadAsync(cancellationToken))
            {
                ...
                if (MilitaryRankService.GetOrder(rank) < 999) list.Add(rank);
            }
        }
        list.AddRange((await GetAllAsync(cancellationToken))
            .Select(x => MilitaryRankService.Canonicalize(x.Rank))
    ...
    public async Task<IReadOnlyList<string>> GetBanksAsync(CancellationToken cancellationToken = default)
    {
        ...
        // Mesmo que a tabela de catálogo esteja vazia, o editor precisa mostrar
        // os bancos que já estão salvos nos militares ativos e nos licenciados/transferidos.
        foreach (var table in new[] { "militares", "lt_militares" })
        {
            if (!await TableExistsAsync(connection, table, cancellationToken)) continue;
            var columns = await GetTableColumnsAsync(connection, table, cancellationToken);
            if (!columns.Contains("banco")) continue;
            await using var cmd = connection.CreateCommand();
            cmd.CommandText = $"SELECT DISTINCT TRIM(COALESCE(banco,'')) FROM {table} WHERE TRIM(COALESCE(banco,''))<>'';";
            await using var reader = await cmd.ExecuteReaderAsync(cancellationToken);
            while (await reader.ReadAsync(cancellationToken)) list.Add(reader.GetString(0));
        }

        return MilitaryBankService.Consolidate(list)
            .OrderBy(MilitaryBankService.SortCode)
    ...
    public async Task<int> SaveAsync(MilitaryRecord military, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            military.CareerType = MilitaryCareerTypeService.Normalize(military.CareerType) is { Length: > 0 } storedType
                ? storedType
            ...
            var values = new Dictionary<string, object?>
            {
                ...
                ["foto"] = military.PhotoPath.Trim(), ["ano"] = military.FormationYear.Trim(), ["tipo_carreira"] = MilitaryCareerTypeService.Normalize(military.CareerType), ["data_nascimento"] = MilitaryFormatting.NormalizeDateText(military.BirthDate),
    ...
    public async Task<Dictionary<string, string>> GetLatestCertificateKeysAsync(int militaryId, CancellationToken cancellationToken = default)
    {
        ...
        return ParseDocumentKeys(json);
    }

    public async Task<Dictionary<string, string>> GetLatestCertificateKeysAsync(MilitaryRecord military, CancellationToken cancellationToken = default)
    {
        var direct = await GetLatestCertificateKeysAsync(military.Id, cancellationToken);
        ...
        var identity = MilitaryFormatting.Digits(military.MilitaryId);
        var activeCpf = CleanSqlDigits("m.cpf");
        var activePrec = CleanSqlDigits("m.prec_cp");
        var activeIdentity = CleanSqlDigits("m.idt");
        var ltCpf = CleanSqlDigits("lt.cpf");
        var ltPrec = CleanSqlDigits("lt.prec_cp");
        var ltIdentity = CleanSqlDigits("lt.idt");
        command.CommandText = $"""
            SELECT d.chaves_json
              FROM militar_documentos d
              LEFT JOIN militares m ON m.id=d.militar_id
              LEFT JOIN lt_militares lt ON lt.id=d.militar_id
             WHERE UPPER(d.tipo)='CERTIDAO_NASCIMENTO'
               AND COALESCE(d.chaves_json,'')<>''
               AND (
                    ($cpf<>'' AND ({activeCpf}=$cpf OR {ltCpf}=$cpf))
                 OR ($prec<>'' AND ({activePrec}=$prec OR {ltPrec}=$prec))
                 OR ($idt<>'' AND ({activeIdentity}=$idt OR {ltIdentity}=$idt))
               )
             ORDER BY d.data_salvo DESC,d.id DESC
             LIMIT 1;
            """;
        command.Parameters.AddWithValue("$cpf", cpf);
        command.Parameters.AddWithValue("$prec", prec);
        command.Parameters.AddWithValue("$idt", identity);
        var json = Convert.ToString(await command.ExecuteScalarAsync(cancellationToken)) ?? string.Empty;
        return ParseDocumentKeys(json);
    }

    private static Dictionary<string, string> ParseDocumentKeys(string? json)
    {
    ...
    private static string CleanSqlDigits(string expression)
    ...
    public async Task<bool> UpdateAddressIfMissingAsync(int militaryId, string address, string? zipCode = null,
        CancellationToken cancellationToken = default)
    {
        await EnsureSchemaAsync(cancellationToken);
        var cleanAddress = address?.Trim() ?? string.Empty;
        if (militaryId <= 0 || string.IsNullOrWhiteSpace(cleanAddress)) return false;
        ...
        command.CommandText = """
            UPDATE militares
               SET endereco=$endereco,
                   cep=CASE WHEN TRIM(COALESCE(cep,''))='' THEN $cep ELSE cep END
             WHERE id=$id AND TRIM(COALESCE(endereco,''))='';
            """;
        command.Parameters.AddWithValue("$endereco", cleanAddress);
    ...
    {
        Id = GetInt(reader, "id"),
        Rank = MilitaryRankService.Canonicalize(GetFirstString(reader, string.Empty, "posto", "posto_grad", "posto_graduacao", "posto_graduação", "p_g", "pg")),
        Name = UpperName(GetFirstString(reader, string.Empty, "nome", "nome_completo", "full_name")),
        WarName = UpperName(GetFirstString(reader, string.Empty, "nome_guerra", "guerra", "nome_de_guerra")),
        Cpf = GetString(reader, "cpf"), PrecCp = GetFirstString(reader, string.Empty, "prec_cp", "preccp", "prec"), MilitaryId = GetFirstString(reader, string.Empty, "idt", "identidade", "id_militar"),
        Bank = GetFirstString(reader, string.Empty, "banco", "domicilio_bancario", "domicílio_bancário", "banco_credito"),
        Agency = GetFirstString(reader, string.Empty, "agencia", "agência"), Account = GetFirstString(reader, string.Empty, "conta", "conta_corrente"),
        PhotoPath = GetString(reader, "foto"), FormationYear = GetFirstString(reader, string.Empty, "ano", "ano_formacao", "ano_formação"),
        CareerType = GetFirstString(reader, string.Empty, "tipo_carreira", "carreira_temporario", "vinculo"),
        BirthDate = GetString(reader, "data_nascimento"), EnlistmentDate = GetString(reader, "data_praca"), Address = GetFirstString(reader, string.Empty, "endereco", "endereço"), ZipCode = GetFirstString(reader, string.Empty, "cep", "zip", "zipcode"),
    ...
    private static string GetString(SqliteDataReader reader, string name, string fallback = "")
    {
        ...
        catch (Exception ex) when (ex is IndexOutOfRangeException or ArgumentOutOfRangeException) { return fallback; }
    }

    private static string GetFirstString(SqliteDataReader reader, string fallback, params string[] names)
    {
    ...
    private static int GetInt(SqliteDataReader reader, string name)
    {
        ...
        catch (Exception ex) when (ex is IndexOutOfRangeException or ArgumentOutOfRangeException) { return 0; }
    }
    private static double? GetNullableDouble(SqliteDataReader reader, string name)
    {
        ...
        catch (Exception ex) when (ex is IndexOutOfRangeException or ArgumentOutOfRangeException) { return null; }
    }
