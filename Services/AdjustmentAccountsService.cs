...
public static partial class AdjustmentAccountsService
{
    ...
    {
        "AR0018", "AR0077", "AR0078", "AR0092", "AR0094", "AR0096", "AR0070", "AR0083", "AR0099", "AR0100",
    ...
    public static IReadOnlyList<AdjustmentRubric> CreateDefaultRubrics(AdjustmentAccountsSettings settings)
        ...
        [
            ...
            New("AR0096", "ADICIONAL DE FÉRIAS", "R", "+", "MES", "FORM_FER_PROP", 0m),
    ...
    public static AdjustmentCalculationResult Calculate(AdjustmentAccountsSettings settings, IEnumerable<AdjustmentRubric> sourceRubrics)
    {
        ...
        {
            var vacationCodes = new HashSet<string>(StringComparer.OrdinalIgnoreCase) { "AR0096", "AR0094" };
    ...
    public static IReadOnlyList<AdjustmentBizuRule> DefaultBizuRules()
        ...
        [
            Rule("Licenciamento Ex-Officio — Término de prorrogação Tp Sv", true, true, true, true,
                ...
                "Faz jus às verbas de férias, adicional natalino e compensação pecuniária. Informe separadamente as cotas de férias, de 13º e da compensação."),
            Rule("Licenciamento a pedido", true, true, true, false,
                "Quadro Formas de Licenciamento — 08 ABR 25.",
                "Faz jus às verbas de férias e ao adicional natalino; não faz jus à compensação pecuniária."),
            Rule("Anulação de Incorporação", false, false, false, false,
                "Quadro Formas de Licenciamento — 08 ABR 25.",
                "Não lançar férias, indenização, adicional natalino ou compensação pecuniária."),
            Rule("Desincorporação", false, false, true, false,
                "Quadro Formas de Licenciamento — 08 ABR 25.",
                "O quadro prevê apenas adicional natalino entre estes quatro direitos."),
            Rule("Licenciamento a Bem da Disciplina (EP)", false, false, false, false,
                "Quadro Formas de Licenciamento — 08 ABR 25.",
                "Não lançar férias, indenização, adicional natalino ou compensação pecuniária."),
            ...
            Rule("Deserção", false, false, false, false,
                "Quadro Formas de Licenciamento — 08 ABR 25.",
                "Não lançar estes direitos. Confira separadamente eventual devolução de adicional natalino já recebido."),
            Rule("Falecimento", true, true, false, false,
                "Art. 9º da MP nº 2.215/2001; quadro Formas de Licenciamento — 08 ABR 25.",
                "Férias e indenização são proporcionais aos meses no Exército; o quadro não prevê adicional natalino nem compensação pecuniária."),
            ...
            Rule("Revogação", true, true, true, null,
                "§ 1º do art. 80 do Decreto nº 4.307/2022; DIEx nº 460/2023 e DIEx nº 794-ASSE1/SSEF/SEF, de 23 NOV 22.",
                "As verbas principais são devidas. A pecuniária é caso condicional e não é habilitada automaticamente; confirme o cumprimento do tempo de serviço em documento próprio."),
            ...
            Rule("Militar gestante acima de 8º ano", true, true, true, true,
                ...
                "O quadro indica as verbas principais e 9 compensações pecuniárias. Confira a publicação e os meses de referência.", 9),
            Rule("Sv Mil Inicial — arrimo/nascimento de filho", false, false, false, false,
                "NT 026-ASSE1/SSEF/SEF, de 10 AGO 21.",
                "Caso especial: conferir Auxílio-Natalidade e Pré-Escolar. Estes quatro direitos não são aplicados automaticamente."),
            Rule("LTIP / LAC", null, null, null, null,
                "Fluxo especial — exige publicação e documentação individual.",
                "Não herda automaticamente direitos de licenciamento. Informe somente parcelas com fundamento específico no caso concreto."),
            Rule("Transferência para reserva / inatividade", null, null, null, null,
                "Fluxo especial — exige ato de transferência e conferência individual.",
                "Não herda automaticamente a matriz de licenciamento."),
            Rule("Reforma", null, null, null, null,
                "Fluxo especial — exige ato de reforma e conferência individual.",
                "Não herda automaticamente a matriz de licenciamento."),
            Rule("Demissão", null, null, null, null,
                "Fluxo especial — exige ato de demissão e conferência individual.",
                "Não herda automaticamente a matriz de licenciamento."),
            Rule("Missão no exterior / retorno", null, null, null, null,
                "Fluxo especial — exige publicação e conferência individual.",
                "Não herda automaticamente a matriz de licenciamento."),
            Rule("Nova convocação / incorporação", null, null, null, null,
                "Fluxo especial — exige publicação e conferência individual.",
                "Não herda automaticamente a matriz de licenciamento."),
            Rule("Reintegração", null, null, null, null,
                "Fluxo especial — exige decisão e conferência individual.",
                "Não herda automaticamente a matriz de licenciamento."),
            Rule("Outros / editar manualmente", null, null, null, null,
                "Motivo não classificado automaticamente.",
                "Por segurança, nenhum direito é presumido. Selecione somente após conferir o fundamento individual.")
        ];

    public static IReadOnlyList<string> AdjustmentReasonOptions()
        => DefaultBizuRules().Select(x => x.Title).ToList().AsReadOnly();

    public static IReadOnlyList<AdjustmentBizuRule> AvailableBizuRules(IEnumerable<AdjustmentBizuRule>? customRules)
    {
        var rules = DefaultBizuRules().Select(x => x.Clone()).ToList();
        foreach (var custom in customRules ?? [])
        {
            if (string.IsNullOrWhiteSpace(custom.Title)) continue;
            var copy = custom.Clone();
            copy.IsCustom = true;
            var index = rules.FindIndex(x => x.Title.Equals(copy.Title, StringComparison.CurrentCultureIgnoreCase));
            if (index >= 0) rules[index] = copy;
            else rules.Add(copy);
        }
        return rules.AsReadOnly();
    }

    public static AdjustmentBizuRule? ResolveBizuRule(string? reason)
        => DefaultBizuRules().FirstOrDefault(x =>
            x.Title.Equals((reason ?? string.Empty).Trim(), StringComparison.CurrentCultureIgnoreCase));

    public static AdjustmentBizuRule? ResolveBizuRule(string? reason, IEnumerable<AdjustmentBizuRule>? customRules)
        => AvailableBizuRules(customRules).FirstOrDefault(x =>
            x.Title.Equals((reason ?? string.Empty).Trim(), StringComparison.CurrentCultureIgnoreCase));
    ...
    public static void ApplyBizu(AdjustmentAccountsSettings settings, AdjustmentBizuRule rule)
    {
        ...
        settings.VacationAdditionalEntitlement = rule.VacationAdditional == true;
        settings.VacationIndemnityEntitlement = rule.VacationIndemnity == true;
        settings.ChristmasEntitlement = rule.ChristmasAdditional == true;
        settings.PecuniaryEntitlement = rule.Pecuniary == true;
        settings.PecuniaryQuotas = settings.PecuniaryEntitlement ? rule.PecuniaryQuotas ?? 0 : 0;
    }

    public static void ApplyBizu(AdjustmentDraft draft, AdjustmentBizuRule rule)
    {
        draft.AdjustmentReason = rule.Title;
        draft.VacationAdditionalEntitlement = rule.VacationAdditional == true;
        draft.VacationIndemnityEntitlement = rule.VacationIndemnity == true;
        draft.ChristmasEntitlement = rule.ChristmasAdditional == true;
        draft.PecuniaryEntitlement = rule.Pecuniary == true;
        draft.PecuniaryQuotas = draft.PecuniaryEntitlement ? rule.PecuniaryQuotas ?? 0 : 0;
    }
    ...
    public static string BulletinIntroductionForReason(string? reason)
    {
        ...
        string Base(string publicationReason)
            => $"Seja realizado o ajuste de contas do militar abaixo relacionado, por motivo de {publicationReason}, sendo excluído e desligado do estado efetivo da {{OM}}, a contar de {{CORTE}}, conforme publicado no BI Nr {{BI}}, de {{BI_DATA}}, da {{OM}}.";
        ...
        if (normalized.Contains("LTIP") || normalized.Contains("LAC")) return Base("LTIP/LAC");
        if (normalized.Contains("CONCURSO")) return Base("aprovação em concurso público");
        if (normalized.Contains("GESTANTE")) return Base("licenciamento de militar gestante");
        if (normalized.Contains("RESERVA") || normalized.Contains("INATIV")) return Base("transferência para a inatividade");
        if (normalized.Contains("REFORMA")) return Base("reforma");
        if (normalized.Contains("DEMIS")) return Base("demissão");
        if (normalized.Contains("MISSAO") || normalized.Contains("EXTERIOR")) return Base("missão no exterior ou retorno");
        if (normalized.Contains("CONVOC") || normalized.Contains("NOVA INCORPOR")) return Base("nova convocação ou incorporação");
        if (normalized.Contains("REINTEGR")) return Base("reintegração");
        if (normalized.Contains("OUTRO"))
            return "Seja realizado o ajuste de contas do militar abaixo relacionado, pelo motivo informado nesta abertura, a contar de {CORTE}, conforme publicado no BI Nr {BI}, de {BI_DATA}, da {OM}.";
        return Base("licenciamento ex officio do serviço ativo do Exército");
    }

    public static string SimplifyBulletinIntroduction(string? introduction)
    {
        var value = (introduction ?? string.Empty).Trim();
        if (string.IsNullOrWhiteSpace(value)) return string.Empty;

        value = Regex.Replace(
            value,
            @"\s*O\s+c[aá]lculo\s+observa\s+as\s+rubricas\s+obrigat[oó]rias\s+do\s+SIPPES\b[^.]*\.?",
            string.Empty,
    ...
    public static string ProfessionalDescription(string description, string code, bool simplify)
    {
        var map = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["AR0170"] = "ADICIONAL DE DISPONIBILIDADE MILITAR", ["AD0001"] = "CONTRIBUIÇÃO FUSEx", ["AD0003"] = "OCUPAÇÃO DE PNR",
            ["AD0011"] = "FUSEx - DEPENDENTE", ["AD0013"] = "FUSEx - DESPESA MÉDICA", ["AD0014"] = "PENSÃO ALIMENTÍCIA",
            ["AD0039"] = "PENSÃO MILITAR", ["AR0092"] = "ADICIONAL DE FÉRIAS (LEGADO)", ["AR0096"] = "ADICIONAL DE FÉRIAS", ["AR0094"] = "INDENIZAÇÃO DE FÉRIAS",
            ["AR0070"] = "ADICIONAL NATALINO", ["AR0066"] = "COMPENSAÇÃO PECUNIÁRIA", ["AR0083"] = "1ª PARCELA DO ADICIONAL NATALINO",
            ["DR0083"] = "DEVOLUÇÃO DA 1ª PARCELA DO ADICIONAL NATALINO", ["AD0010"] = "IMPOSTO DE RENDA",
            ["AD0015"] = "IMPOSTO DE RENDA SOBRE FÉRIAS", ["AD0028"] = "IMPOSTO DE RENDA SOBRE O ADICIONAL NATALINO"
        };
    ...
    public static string FormatBulletinMilitaryName(MilitaryRecord military)
    {
        ...
        var name = BulletinFullName(military);
        return string.Join(" ", new[] { rank, name }.Where(x => !string.IsNullOrWhiteSpace(x))).Trim();
    }

    public static string BulletinFullName(MilitaryRecord military)
    {
        ...
        if (!string.IsNullOrWhiteSpace(war))
            name = Regex.Replace(name, $@"\s*[-–—]\s*{Regex.Escape(war)}\s*$", string.Empty,
    ...
    private static AdjustmentBizuRule Rule(string title, bool? vacationAdditional, bool? vacationIndemnity, bool? christmas, bool? pecuniary, string basis, string observation, int? pecuniaryQuotas = null)
        => new() { Title = title, VacationAdditional = vacationAdditional, VacationIndemnity = vacationIndemnity, ChristmasAdditional = christmas, Pecuniary = pecuniary, PecuniaryQuotas = pecuniaryQuotas, LegalBasis = basis, Observation = observation };
    ...
    private static decimal CalculateMonthlyIncomeTax(
        ...
        decimal dependents)
    {
        var legalDeductions = Round(Math.Max(0m, militaryPension + fusex + medical + dependentFusex + alimony + pnr + dependents));
        var bestDeduction = Math.Max(legalDeductions, SimplifiedMonthlyDiscount2026);
        var taxableBase = Round(Math.Max(0m, taxableIncome - bestDeduction));
