...
public sealed class LicensedTransferredRepository
{
        ...
        try
        {
            ...
            await using (var create = connection.CreateCommand())
            {
                create.CommandText = """
                    ...
                    CREATE INDEX IF NOT EXISTS idx_lt_visivel_nome ON lt_militares(visivel, nome COLLATE NOCASE);
                    CREATE INDEX IF NOT EXISTS idx_lt_prec_cp ON lt_militares(prec_cp);
                    CREATE INDEX IF NOT EXISTS idx_lt_idt ON lt_militares(idt);
                    CREATE INDEX IF NOT EXISTS idx_recrutas_extra_cpf ON recrutas_extra(cpf);
                    """;
            ...
            // Não normalizamos todos os nomes na abertura para não travar a tela.
            // A normalização continua ocorrendo no SaveAsync e no ReadRecord.
            _schemaReady = true;
        }
        catch (Exception ex)
        {
    ...
    public async Task<List<LicensedTransferredRecord>> GetAllAsync(bool includeHidden = false, string? filter = null, CancellationToken cancellationToken = default)
    {
        ...
        var digits = MilitaryFormatting.Digits(filter ?? string.Empty);
        var like = $"%{(filter ?? string.Empty).Trim()}%";
        ...
        command.CommandText = """
            SELECT lt.*,
             ...
             WHERE ($hidden=1 OR COALESCE(lt.visivel,1)=1)
               AND (
                    $hasFilter=0
                    OR lt.posto LIKE $like COLLATE NOCASE
                    OR lt.nome LIKE $like COLLATE NOCASE
                    OR lt.nome_guerra LIKE $like COLLATE NOCASE
                    OR lt.motivo LIKE $like COLLATE NOCASE
                    OR lt.destino LIKE $like COLLATE NOCASE
                    OR lt.ano LIKE $like COLLATE NOCASE
                    OR lt.endereco LIKE $like COLLATE NOCASE
                    OR lt.cep LIKE $like COLLATE NOCASE
                    OR COALESCE(NULLIF(lt.telefone,''),NULLIF(r.telefone,''),'') LIKE $like COLLATE NOCASE
                    OR COALESCE(NULLIF(lt.email,''),NULLIF(r.email,''),'') LIKE $like COLLATE NOCASE
                    OR COALESCE(NULLIF(lt.escolaridade,''),NULLIF(r.escolaridade,''),'') LIKE $like COLLATE NOCASE
                    OR ($digits<>'' AND (
                        REPLACE(REPLACE(REPLACE(COALESCE(lt.cpf,''),'.',''),'-',''),' ','') LIKE $digitLike
                        OR REPLACE(REPLACE(REPLACE(COALESCE(lt.prec_cp,''),'.',''),'-',''),' ','') LIKE $digitLike
                        OR REPLACE(REPLACE(REPLACE(COALESCE(lt.idt,''),'.',''),'-',''),' ','') LIKE $digitLike
                        OR REPLACE(REPLACE(REPLACE(COALESCE(lt.cep,''),'.',''),'-',''),' ','') LIKE $digitLike
                        OR REPLACE(REPLACE(REPLACE(COALESCE(lt.telefone,''),'.',''),'-',''),' ','') LIKE $digitLike
                        OR REPLACE(REPLACE(REPLACE(COALESCE(r.telefone,''),'.',''),'-',''),' ','') LIKE $digitLike
                    ))
               )
             ORDER BY lt.nome COLLATE NOCASE, lt.id;
            """;
        command.Parameters.AddWithValue("$hidden", includeHidden ? 1 : 0);
        command.Parameters.AddWithValue("$hasFilter", hasFilter ? 1 : 0);
        command.Parameters.AddWithValue("$like", like);
        command.Parameters.AddWithValue("$digits", digits);
        command.Parameters.AddWithValue("$digitLike", digitLike);
        await using (var reader = await command.ExecuteReaderAsync(cancellationToken))
        {
            while (await reader.ReadAsync(cancellationToken)) result.Add(ReadRecord(reader));
        }

        // A consulta SQL é rápida, mas o LIKE do SQLite não ignora acentos em todos os casos.
        // Quando a busca textual não encontra nada, fazemos uma segunda passada leve ignorando acentos.
        if (hasFilter && result.Count == 0 && digits.Length == 0)
        {
            ...
            fallback.CommandText = """
                SELECT lt.*,
                 ...
                 ORDER BY lt.nome COLLATE NOCASE, lt.id;
                """;
            fallback.Parameters.AddWithValue("$hidden", includeHidden ? 1 : 0);
            ...
            {
                var item = ReadRecord(fallbackReader);
                var text = Normalize($"{item.Rank} {item.Name} {item.WarName} {item.Cpf} {item.PrecCp} {item.MilitaryId} {item.Reason} {item.Destination} {item.FormationYear} {item.Phone} {item.Email} {item.Address} {item.ZipCode}");
                if (text.Contains(query)) result.Add(item);
            }
        }
        return result;
    }

    public async Task<int> CountAsync(bool includeHidden = false, CancellationToken cancellationToken = default)
    {
        await EnsureSchemaAsync(cancellationToken);
        await using var connection = OpenConnection();
        await using var command = connection.CreateCommand();
        command.CommandText = "SELECT COUNT(1) FROM lt_militares WHERE ($hidden=1 OR COALESCE(visivel,1)=1);";
        command.Parameters.AddWithValue("$hidden", includeHidden ? 1 : 0);
