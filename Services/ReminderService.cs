...
public sealed class ReminderService
{
    ...
    public async Task<List<ReminderRecord>> LoadAsync(bool normalizeRecurring = true, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var changed = normalizeRecurring && NormalizeRecurring(records);
            changed |= NormalizeOrder(records);
            if (changed) await WriteListAsync(_paths.RemindersFile, records, cancellationToken);
            return records.OrderBy(x => x.Completed).ThenBy(x => x.Completed ? int.MaxValue : x.Order).ThenBy(x => x.Title, StringComparer.CurrentCultureIgnoreCase).ToList();
        }
    ...
    public async Task SaveAsync(ReminderRecord record, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            records.RemoveAll(x => x.Id == record.Id);

            var saved = record.Clone();
            if (saved.Completed)
            {
                saved.Order = 0;
                records.Add(saved);
            ...
            {
                NormalizeOrder(records);
                var active = records.Where(x => !x.Completed).OrderBy(x => x.Order).ToList();
                var completed = records.Where(x => x.Completed).ToList();
                var insertAt = saved.Order <= 0 ? active.Count : Math.Clamp(saved.Order - 1, 0, active.Count);
                active.Insert(insertAt, saved);
                for (var i = 0; i < active.Count; i++) active[i].Order = i + 1;
    ...
    public async Task SetCompletedAsync(int id, bool completed, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (item.Completed) item.Order = 0;
            NormalizeOrder(records);
            await WriteListAsync(_paths.RemindersFile, records, cancellationToken);
        }
        finally { _gate.Release(); }
    ...
    public async Task RestoreFromHistoryAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            restored.Order = records.Count(x => !x.Completed) + 1;
            history.Remove(item);
    ...
    public async Task<List<ReminderRecord>> GetUrgentAsync(int upcomingDays = 2, CancellationToken cancellationToken = default)
        => (await LoadAsync(true, cancellationToken))
            .Where(x => !x.Completed && (x.Urgent || x.DaysRemaining is <= 0))
    ...
    public async Task MoveAsync(int id, int direction, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (index < 0) return;
            var target = Math.Clamp(index + Math.Sign(direction), 0, active.Count - 1);
            if (target == index) return;
            (active[index], active[target]) = (active[target], active[index]);
            for (var i = 0; i < active.Count; i++) active[i].Order = i + 1;
            await WriteListAsync(_paths.RemindersFile, [.. active, .. records.Where(x => x.Completed)], cancellationToken);
        }
        finally { _gate.Release(); }
        Changed?.Invoke(this, EventArgs.Empty);
    }

    public async Task SetUrgentAsync(int id, bool urgent, CancellationToken cancellationToken = default)
    {
        await _gate.WaitAsync(cancellationToken);
        try
        {
    ...
    public async Task ExportTextAsync(string path, IEnumerable<ReminderRecord> records, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var item in records.OrderBy(x => x.Completed).ThenBy(x => x.Order))
        {
            builder.AppendLine($"[{item.OrderText}] {(item.Urgent ? "[URGENTE] " : string.Empty)}{item.Title}");
    ...
    private static bool NormalizeOrder(List<ReminderRecord> records)
    {
        var changed = false;
        var active = records.Where(x => !x.Completed)
            .OrderBy(x => x.Order <= 0 ? int.MaxValue : x.Order)
            .ThenBy(x => x.DueDate ?? DateTime.MaxValue)
            .ThenBy(x => PriorityOrder(x.EffectivePriority))
            .ThenBy(x => x.Title, StringComparer.CurrentCultureIgnoreCase)
            .ToList();
        for (var i = 0; i < active.Count; i++)
        {
            if (active[i].Order == i + 1) continue;
            active[i].Order = i + 1;
            changed = true;
        }
        foreach (var item in records.Where(x => x.Completed && x.Order != 0))
        {
            item.Order = 0;
            changed = true;
        }
