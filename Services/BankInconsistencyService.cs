...
public sealed class BankInconsistencyService : IDisposable
{
    ...
    private IWebDriver GetDriver(string downloadDirectory, bool headless, bool keepOpen, CancellationToken ct)
    {
        ...
        try
        {
            ...
            foreach (var arg in new[] { $"--user-data-dir={_paths.BankAutomationProfileDirectory}", "--ignore-certificate-errors", "--allow-running-insecure-content", "--disable-notifications", "--disable-popup-blocking", "--disable-gpu", "--disable-features=CalculateNativeWinOcclusion", "--window-size=1500,1050", "--no-first-run", "--no-default-browser-check" }) options.AddArgument(arg);
...
""");
    ...
    private void CloseBrowserForShutdown()
    {
        if (!_driverGate.Wait(TimeSpan.FromSeconds(2))) return;
        try
        {
            var driver = _driver;
            _driver = null;
            ...
            try
            {
                ...
                quit.Wait(TimeSpan.FromSeconds(2));
            }
            catch { }
    ...
    public void Dispose()
    {
        try { CloseBrowserForShutdown(); } catch { }
        _driverGate.Dispose();
