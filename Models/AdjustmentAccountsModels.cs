...
public sealed class AdjustmentAccountsSettings : INotifyPropertyChanged
{
    ...
    public int PecuniaryQuotas { get => _pecuniaryQuotas; set => Set(ref _pecuniaryQuotas, Math.Clamp(value, 0, 50)); }
    public string SelectedBizuTitle { get => _selectedBizuTitle; set => Set(ref _selectedBizuTitle, value ?? string.Empty); }

    // Compatibilidade com arquivos antigos. O fluxo novo usa a opção estruturada
    // selecionada e, quando informada, a data de vigência dentro da competência.
...
public sealed class AdjustmentBizuRule
{
    ...
    public AdjustmentBizuRule Clone()
        => new()
        {
            ...
            Pecuniary = Pecuniary,
            PecuniaryQuotas = PecuniaryQuotas,
...
public sealed class AdjustmentAccountsStore
{
    ...
    public AdjustmentDraft? CurrentDraft { get; set; }
    public List<AdjustmentDraft> Drafts { get; set; } = [];
    public List<AdjustmentAuditEntry> AuditTrail { get; set; } = [];
}
...
public sealed class BulletinMilitarySelection
{
    ...
    public string Source => string.IsNullOrWhiteSpace(Military.BulletinSource) ? "Ativo" : Military.BulletinSource;
}
