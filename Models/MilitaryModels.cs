...
public sealed class MilitaryRecord : INotifyPropertyChanged
{
    ...
    private string _careerType = string.Empty;
    public string CareerType
    {
        get => _careerType;
        set
        {
            var normalized = MilitaryCareerTypeService.Normalize(value);
            if (string.Equals(_careerType, normalized, StringComparison.Ordinal)) return;
            _careerType = normalized;
            OnPropertyChanged(nameof(CareerType));
            OnPropertyChanged(nameof(CareerTypeDisplay));
        }
    }
    ...
    public string BulletinSource { get; set; } = "Ativo";
    public string CustomColor
    {
        ...
        set
        {
            ...
            if (string.Equals(_customColor, normalized, StringComparison.Ordinal)) return;
            _customColor = normalized;
            OnPropertyChanged(nameof(CustomColor));
            OnPropertyChanged(nameof(RowColor));
        }
    }
    public string RowColor => IsValidColor(CustomColor)
        ? CustomColor.Trim()
    ...
    public bool IsMarkedForBatch
    {
        ...
        set
        {
            ...
            OnPropertyChanged(nameof(IsMarkedForBatch));
        }
    }
    ...
    public string CareerTypeDisplay => MilitaryCareerTypeService.Display(CareerType);
    public string RankSortKey => $"{MilitaryRankService.GetOrder(Rank):000}|{MilitaryRankService.Canonicalize(Rank)}|{Name}";
...
{
    public string SortMode { get; set; } = "Ordem salva em Listar Militares";
    public bool OpenAfterGenerate { get; set; } = true;
    public List<int> OrderedMilitaryIds { get; set; } = [];
    /// <summary>
    /// Sistema de pagamento escolhido para cada militar na relação SIAPPES/SIPPES.
    /// Cadastros ainda não classificados entram como SIPPES, que é o sistema atual.
    /// </summary>
    public Dictionary<int, string> PaymentSystemByMilitaryId { get; set; } = [];
}

public sealed class PersonnelSystemRelationItem
{
    public required MilitaryRecord Military { get; init; }
    public string PaymentSystem { get; init; } = "SIPPES";
}
