...
public sealed class GratificationSettings
{
    ...
    public List<string> RequestNatureHistory { get; set; } = [];
    public List<string> RequestDescriptionHistory { get; set; } = [];
    public string RequestAuthorizingDocument { get; set; } = "DIEx nº 29183-DivOpLog.6/COpLog/COLOG, de 31 OUT 25";
    public string RequestAuthorizingAttachmentFile { get; set; } = string.Empty;
    ...
    public string RequestBulletin { get; set; } = string.Empty;
    public string RequestBulletinAttachmentFile { get; set; } = string.Empty;
    public string RequestContact { get; set; } = string.Empty;
    public string RequestRitex { get; set; } = "804-9833";
    public string RequestEmail { get; set; } = string.Empty;
    public string RequestAuthority { get; set; } = string.Empty;
    public string RequestOrganization { get; set; } = string.Empty;
    public string RequestCity { get; set; } = "Belo Horizonte";
    public int RequestSalaryReferenceYear { get; set; } = DateTime.Today.Year;
    ...
    public List<GratificationMissionLot> RequestMissionLots { get; set; } = [];
    public List<string> RequestAttachmentFiles { get; set; } = [];
    public string CurrentProcessId { get; set; } = string.Empty;
    public string SpedSender { get; set; } = "PHABLLO";
    public string SpedRecipient { get; set; } = "E1";
    public string SpedClassification { get; set; } = "085.612 - GRATIFICAÇÕES";
    public string SpedDocumentPurpose { get; set; } = "Geral";
    public string SpedSubject { get; set; } = "Gratificação de Representação (2%)";
}

public sealed class OfficialSalarySnapshot
{
    public DateTime ReferenceDate { get; init; }
    public IReadOnlyDictionary<string, decimal> Salaries { get; init; } = new Dictionary<string, decimal>(StringComparer.OrdinalIgnoreCase);
    public string EffectivePeriod { get; init; } = string.Empty;
    public string LegalBasis { get; init; } = string.Empty;
    public string SourceUrl { get; init; } = string.Empty;
    public bool RetrievedOnline { get; init; }
    public string RetrievalStatus => RetrievedOnline ? "Fonte oficial consultada online" : "Referência oficial local (site indisponível)";
}

public sealed class GratificationSpedAttachment
{
...
public sealed class GratificationEffectiveRow : INotifyPropertyChanged
{
    ...
    public int PersonDays
    {
        ...
        {
            var safe = Math.Max(0, value);
            if (_personDays == safe) return;
            _personDays = safe;
            Notify(nameof(PersonDays));
            Notify(nameof(Subtotal));
            Notify(nameof(SubtotalText));
        }
    }
    public string DaysBreakdown { get; set; } = string.Empty;
    public string DailyRateBreakdown { get; set; } = string.Empty;
    public decimal? SubtotalOverride { get; set; }
    public decimal DailyRate => Salary * 0.02m;
    public int PayablePersonDays => PersonDays > 0 ? PersonDays : Days * Quantity;
    public decimal Subtotal => SubtotalOverride ?? DailyRate * PayablePersonDays;
    public string SalaryText => Salary.ToString("C2", PtBr);
    public string DailyRateText => string.IsNullOrWhiteSpace(DailyRateBreakdown) ? DailyRate.ToString("C2", PtBr) : DailyRateBreakdown;
...
public sealed class GratificationMissionLot
{
    public string Id { get; set; } = Guid.NewGuid().ToString("N");
    public string Name { get; set; } = "1ª leva";
    ...
    [JsonIgnore]
    public int EffectiveTotal => EffectiveByRank?.Values.Sum(value => Math.Max(0, value)) ?? 0;

    [JsonIgnore]
    public int IndemnifiableDays
    {
        get
        {
            if (!TryGetPeriod(out var start, out var end) || end <= start) return 0;
            var duration = end - start;
            var fullDays = (int)Math.Floor(duration.TotalHours / 24d);
            var remainder = duration - TimeSpan.FromDays(fullDays);
            return fullDays + (remainder >= TimeSpan.FromHours(8) ? 1 : 0);
        }
    }

    [JsonIgnore]
    public int PersonDays => EffectiveTotal * IndemnifiableDays;

    [JsonIgnore]
    public string SummaryText => $"{StartDate:dd/MM/yyyy} {StartTime} a {EndDate:dd/MM/yyyy} {EndTime}  •  {IndemnifiableDays} dia(s)  •  {EffectiveTotal} militar(es)  •  {PersonDays} homens-dia";

    public bool TryGetPeriod(out DateTime start, out DateTime end)
    {
        start = default;
        end = default;
        return TryCombine(StartDate, StartTime, out start) && TryCombine(EndDate, EndTime, out end);
    }
    ...
    private static bool TryCombine(DateTime date, string? time, out DateTime value)
    {
        ...
        if (!TimeSpan.TryParse(raw, CultureInfo.GetCultureInfo("pt-BR"), out var parsed)) return false;
        value = date.Date + parsed;
        return true;
    }
}
