...
public sealed class PlanCallRecord : INotifyPropertyChanged
{
    ...
    public string ReferenceAddress
    {
        get
        {
            var wallet = BaseAddress.Trim();
            var transport = TransportAddress.Trim();
            if (string.IsNullOrWhiteSpace(wallet)) return transport;
            if (string.IsNullOrWhiteSpace(transport)) return wallet;
            return Services.PlanCallService.AddressEquivalent(wallet, transport)
                ? transport
                : $"Carteira: {wallet} | Auxílio-Transporte: {transport}";
        }
    }
    public string ReferenceAddressDetails
    {
        get
        {
            var wallet = string.IsNullOrWhiteSpace(BaseAddress) ? "não informado" : BaseAddress;
            var transport = string.IsNullOrWhiteSpace(TransportAddress) ? "não informado" : TransportAddress;
            return $"Carteira: {wallet}\nAuxílio-Transporte: {transport}";
        }
    }
    public string Region => Services.PlanCallService.RegionFor(FirstAddress(PlanAddress, TransportAddress, BaseAddress), CityState);
    public int RegionDistanceOrder => Services.PlanCallService.RegionDistanceOrder(Region);
    ...
    public string WalletAddressStatus => SourceAddressStatus(BaseAddress);
    public string TransportAddressStatus => SourceAddressStatus(TransportAddress);
    public string DifferenceStatus
    {
        get
        {
            if (!HasMilitaryMatch) return "SEM COMPARAÇÃO";
            if (!HasPlanAddress) return "SEM ENDEREÇO NO PLANO";
            var sources = new[] { BaseAddress, TransportAddress }.Where(x => !string.IsNullOrWhiteSpace(x)).ToList();
            if (sources.Count == 0) return "SEM ENDEREÇO DE REFERÊNCIA";
            var statuses = sources.Select(x => Services.PlanCallService.CompareAddresses(PlanAddress, x)).ToList();
            if (statuses.Any(x => x == "DIVERGENTE")) return "DIVERGENTE";
            return statuses.Any(x => x == "COMPATÍVEL") ? "COMPATÍVEL" : "CONFERE";
        }
    }
    ...
    public bool CanCopyAddressToMilitary => HasMilitaryMatch && string.IsNullOrWhiteSpace(BaseAddress) && HasPlanAddress;
    public bool CanCompleteMilitary => CanCopyPhoneToMilitary || CanCopyAddressToMilitary;
    public string SearchText => $"{Rank} {ShortRank} {Name} {WarName} {Cpf} {PrecCp} {Phone} {AlternatePhone} {PlanAddress} {ReferenceAddress} {ZipCode} {Region} {DifferenceStatus}";
    ...
    private bool Set<T>(ref T field, T value, [CallerMemberName] string? name = null)
    {
        ...
        if (name is nameof(Phone))
        {
            ...
            PropertyChanged?.Invoke(this, new PropertyChangedEventArgs(nameof(CanCompleteMilitary)));
        }
        return true;
    }
    ...
    {
        foreach (var name in new[] { nameof(MatchStatus), nameof(ReferenceAddress), nameof(ReferenceAddressDetails), nameof(DifferenceStatus), nameof(WalletAddressStatus), nameof(TransportAddressStatus), nameof(PhoneStatus), nameof(CanCopyPhoneToMilitary), nameof(CanCopyAddressToMilitary), nameof(CanCompleteMilitary), nameof(Region), nameof(RegionDistanceOrder), nameof(SearchText) })
            PropertyChanged?.Invoke(this, new PropertyChangedEventArgs(name));
    }

    private void NotifyAddress()
    {
        foreach (var name in new[] { nameof(PlanAddress), nameof(HasPlanAddress), nameof(EffectiveAddress), nameof(EffectiveZipCode), nameof(Region), nameof(RegionDistanceOrder), nameof(RecordSource), nameof(DifferenceStatus), nameof(WalletAddressStatus), nameof(TransportAddressStatus), nameof(CanCopyAddressToMilitary), nameof(CanCompleteMilitary), nameof(SearchText) })
            PropertyChanged?.Invoke(this, new PropertyChangedEventArgs(name));
    }
    ...
    private string SourceAddressStatus(string source)
        => string.IsNullOrWhiteSpace(source) ? "Não informado" : !HasPlanAddress ? "Sem dado no Plano" : Services.PlanCallService.CompareAddresses(PlanAddress, source) switch
        {
            "CONFERE" => "Confere",
...
public sealed class PlanCallImportMatch
{
    ...
    public string CurrentPhone => Current?.Phone ?? string.Empty;
    public string ImportedPhone => Imported.Phone;
    public string ConfidenceText => Confidence <= 0 ? "Novo" : $"{Confidence:P0}";
}

public sealed class PlanCallCopyResult
{
    public int Phones { get; set; }
    public int Addresses { get; set; }
    public int Total => Phones + Addresses;
}
...
public sealed class PlanCallSettings
{
    ...
    public string SortMode { get; set; } = "Região — mais distante primeiro";
