...
{
    [JsonPropertyName("id")] public string Id { get; set; } = string.Empty;
    [JsonPropertyName("chave_ciencia")] public string ReviewKey { get; set; } = string.Empty;
    ...
    [JsonPropertyName("vinculado_indice_pessoa")] public bool IsPersonIndexMatch { get; set; }
    [JsonPropertyName("indice_nota_numero")] public string PersonIndexNoteNumber { get; set; } = string.Empty;
    [JsonPropertyName("numero_nota_detectado")] public string DetectedNoteNumber { get; set; } = string.Empty;
    [JsonPropertyName("indice_vinculo_detalhe")] public string PersonIndexMatchDetail { get; set; } = string.Empty;
    [JsonPropertyName("indice_atualizado_em")] public string PersonIndexUpdatedAt { get; set; } = string.Empty;
    [JsonPropertyName("origem_interpretacao")] public string InterpretationOrigin { get; set; } = "Parser SIGFUR";
    [JsonPropertyName("confianca_ia")] public double? AiConfidence { get; set; }
    [JsonPropertyName("revisao_humana_obrigatoria")] public bool RequiresHumanReview { get; set; }
    [JsonIgnore] public bool Reviewed { get; set; }
    [JsonIgnore] public string ReviewedText => Reviewed ? "CIENTE" : "PENDENTE";
    [JsonIgnore] public string ReviewStorageKey => string.IsNullOrWhiteSpace(ReviewKey) ? Id : ReviewKey;
    [JsonIgnore] public string NoteReferenceDisplay => !string.IsNullOrWhiteSpace(PersonIndexNoteNumber)
        ? PersonIndexNoteNumber
        : string.IsNullOrWhiteSpace(DetectedNoteNumber) ? "—" : DetectedNoteNumber;
    ...
    [JsonIgnore] public string SubjectSourceText => IsPersonIndexMatch ? "Índice SisBol" : "Leitura do BI";
    [JsonIgnore] public string InterpretationOriginDisplay => InterpretationOrigin == "IA_RESCUE" ? "IA — revisar" : "Parser SIGFUR";
    [JsonIgnore] public string SubjectSourceToolTip => IsPersonIndexMatch
        ? string.IsNullOrWhiteSpace(PersonIndexMatchDetail) ? "Assunto e nota conferidos pelo Índice por Pessoa do SisBol." : PersonIndexMatchDetail
        : "Ainda não foi possível conferir esta publicação com segurança no Índice por Pessoa.";
}
...
public sealed class IntelligentBulletinReconciliationResult
{
    public int FilesScanned { get; set; }
    public int FurrielFindings { get; set; }
...
public sealed class IntelligentBulletinSettings
{
    ...
    public string ConsequenceKeyword { get; set; } = "Furriel";
    public string ConsequenceReadFilter { get; set; } = "Pendentes";
    public int ConsequenceReconciliationParserVersion { get; set; }
