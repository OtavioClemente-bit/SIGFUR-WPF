...
public sealed class ExercisePreviousProcess : INotifyPropertyChanged
{
    ...
    public int? CpexStatusYear { get; set; }
    public string CpexStatusCheckedAt { get; set; } = string.Empty;
    public string CpexApprovalStatus { get; set; } = string.Empty;
    public string CpexApprovalReason { get; set; } = string.Empty;

    public string BankOrderPath { get; set; } = string.Empty;
    public string BankOrderAttachedAt { get; set; } = string.Empty;
    ...
    public string RightMaterializationDocument { get; set; } = string.Empty;
    public string BulletinThatRecorded { get; set; } = string.Empty;
    public string NonPaymentExplanation { get; set; } = string.Empty;
...
public sealed class ExercisePreviousSummary
{
    ...
    public decimal CorrectedRevenueNet { get; set; }
    public decimal CorrectedExpenseNet { get; set; }
}
...
public sealed class CpexExerciseStatusResult
{
...
public sealed class CpexExerciseBatchStatusItem
{
    public int ProcessId { get; set; }
    public bool Found { get; set; }
    public CpexExerciseStatusResult Result { get; set; } = new();
}
...
public static class ExercisePreviousDefaults
{
    ...
    public const string ProfessionalRightMaterializationGuide = "O direito foi materializado pelos documentos constantes do processo, em especial pelo Parecer da SEF expedido por intermédio do DIEx nº 103-ASSE1/SSEF/SEF - CIRCULAR, de 03 MAIO 18, segundo o qual, em qualquer fase posterior à formação, inclusive durante o EIPOT, os Asp Of R/2 das Armas, do QMB e do Sv Int oriundos dos OFOR fazem jus ao adicional de habilitação equivalente a 12% (doze por cento). A comprovação individual do período e da situação funcional encontra-se amparada nas folhas de alterações autenticadas do militar, referentes ao estágio realizado no 4º Batalhão de Comunicações.";
    public const string ProfessionalNonPaymentGuide = "A despesa caracteriza-se como exercício anterior porque o militar realizou o EIPOT no período informado no processo e, embora tenha direito ao adicional de habilitação de 12% (doze por cento), conforme entendimento firmado no Parecer da SEF expedido por intermédio do DIEx nº 103-ASSE1/SSEF/SEF - CIRCULAR, de 03 MAIO 18, não recebeu a vantagem remuneratória na competência própria. Assim, o pagamento deve ser processado como diferença de exercício anterior, limitado ao período comprovado e aos valores apurados na ficha financeira e no demonstrativo de cálculo.";
