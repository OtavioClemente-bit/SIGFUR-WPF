...
public sealed class SisbolPersonIndexItem
{
    ...
    [JsonIgnore] public string DisplaySubjectNote
    {
        get
        {
            ...
            if (!string.IsNullOrWhiteSpace(MainSubject) && !string.IsNullOrWhiteSpace(SubSubject))
            {
                if (MainSubject.Equals(SubSubject, StringComparison.CurrentCultureIgnoreCase)) return MainSubject;
