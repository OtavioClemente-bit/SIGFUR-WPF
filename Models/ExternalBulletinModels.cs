...
public sealed class ExternalBulletinFile
{
    ...
    [JsonPropertyName("url_origem")] public string SourceUrl { get; set; } = string.Empty;
    [JsonPropertyName("lido")] public bool IsRead { get; set; }
    [JsonPropertyName("lido_em")] public string ReadAt { get; set; } = string.Empty;
    ...
    [JsonIgnore] public string ReadStatus => IsRead ? "Lido" : "Pendente";
...
{
    [JsonPropertyName("termo_pesquisa")] public string SearchTerm { get; set; } = string.Empty;
    ...
    [JsonPropertyName("cml_online_ano")] public int CmlOnlineYear { get; set; } = DateTime.Today.Year;
    [JsonPropertyName("cml_online_pesquisa")] public string CmlOnlineSearchTerm { get; set; } = string.Empty;
    [JsonPropertyName("regiao_online_ano")] public int RegionOnlineYear { get; set; } = DateTime.Today.Year;
    [JsonPropertyName("regiao_online_mes")] public int RegionOnlineMonth { get; set; }
    [JsonPropertyName("regiao_ocultar_lidos")] public bool HideReadRegionBulletins { get; set; } = true;
}
...
public sealed class ExternalBulletinOnlineFile : INotifyPropertyChanged
{
    private bool _isSelected = true;
    private bool _alreadyInLibrary;
    private string _status = "Disponível para baixar";
    ...
    {
        get => _alreadyInLibrary;
        set => Set(ref _alreadyInLibrary, value);
    }
    ...
    {
        get => _status;
        set => Set(ref _status, value ?? string.Empty);
    }
...
public sealed class ExternalBulletinOnlineDownloadResult
{
    public int Downloaded { get; set; }
    public int AlreadyStored { get; set; }
    public ExternalBulletinImportResult Import { get; set; } = new();
    public List<string> Errors { get; } = [];
}

public sealed class ExternalBulletinRegionPeriod
{
    public int Year { get; set; }
    ...
    public string DisplayName => Month == 0
        ? "Todos os meses"
        : $"{Month:00} - {Name}{(DocumentCount > 0 ? $" ({DocumentCount})" : string.Empty)}";
}
