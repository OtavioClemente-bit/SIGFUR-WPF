...
public sealed class BulletinTemplate : INotifyPropertyChanged
{
    ...
    private string _category = "Outros assuntos";
    private int _order;
    ...
    public string Category
    {
        ...
        set
        {
            var normalized = string.IsNullOrWhiteSpace(value) ? "Outros assuntos" : value.Trim();
    ...
    public string SearchText => $"{Category} {Name} {Text}";
    public event PropertyChangedEventHandler? PropertyChanged;
    public BulletinTemplate Clone(string? newName = null) => new() { Name = newName ?? Name, Text = Text, Category = Category, Order = Order, IsBuiltIn = false };
}

public sealed class BulletinSelectedMilitary : INotifyPropertyChanged
{
    private int _position;
    private string _individualDataStatus = "—";
    private string _individualActionLabel = "Preencher";
    private bool _canEditIndividualData;
    ...
    public string IndividualDataStatus
    {
        get => _individualDataStatus;
        set { if (_individualDataStatus == value) return; _individualDataStatus = value ?? "—"; PropertyChanged?.Invoke(this, new(nameof(IndividualDataStatus))); }
    }
    public string IndividualActionLabel
    {
        get => _individualActionLabel;
        set { if (_individualActionLabel == value) return; _individualActionLabel = value ?? "Preencher"; PropertyChanged?.Invoke(this, new(nameof(IndividualActionLabel))); }
    }
    public bool CanEditIndividualData
    {
        get => _canEditIndividualData;
        set { if (_canEditIndividualData == value) return; _canEditIndividualData = value; PropertyChanged?.Invoke(this, new(nameof(CanEditIndividualData))); }
    }
    public event PropertyChangedEventHandler? PropertyChanged;
}
...
public sealed class BulletinFieldDefinition
{
    ...
    public int TableWidthPercent { get; set; } = 100;
    public bool TableHasHeader { get; set; } = true;
    public string TableDefaultValue { get; set; } = string.Empty;
}

public sealed class BulletinTableData
{
...
public sealed class BulletinPreferences
{
    ...
    public bool ShowLicensedTransferredInBulletin { get; set; }
    ...
    // Campos preenchidos por militar e por modelo.
    // Uso principal: modelos com [[ITEM]] em que cada pessoa tem valor/dias/OM próprios
    // (ex.: Auxílio-Alimentação PRM, transferência de pagamento, diferenças individuais).
    public Dictionary<string, Dictionary<int, Dictionary<string, string>>> PerMilitaryFormValues { get; set; } = new(StringComparer.OrdinalIgnoreCase);
