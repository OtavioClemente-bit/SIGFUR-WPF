...
public sealed class ReminderRecord : INotifyPropertyChanged
{
    ...
    [JsonPropertyName("ordem")] public int Order { get => _order; set { if (Set(ref _order, Math.Max(0, value))) NotifyCalculated(); } }
    [JsonPropertyName("urgente")] public bool Urgent { get => _urgent; set { if (Set(ref _urgent, value)) NotifyCalculated(); } }
    ...
    [JsonIgnore] public string OrderText => Completed ? "✓" : Order > 0 ? Order.ToString("00", CultureInfo.InvariantCulture) : "—";
    [JsonIgnore] public bool IsTopThree => !Completed && Order is >= 1 and <= 3;
    [JsonIgnore] public bool IsNextThree => !Completed && Order is >= 4 and <= 6;
    [JsonIgnore] public string UrgentText => Urgent ? "URGENTE" : string.Empty;
    [JsonIgnore] public string StartupAttentionReason => Urgent ? "Urgência marcada" : DaysRemaining < 0 ? "Prazo vencido" : DaysRemaining == 0 ? "Vence hoje" : "Atenção";
    [JsonIgnore] public string PositionText => Completed ? "Concluída" : Order switch
    {
        1 => "1ª prioridade — executar primeiro",
        2 => "2ª prioridade",
        3 => "3ª prioridade",
        > 3 => $"{Order}ª prioridade",
        _ => "Sem posição"
    };
    ...
    public ReminderRecord Clone() => new()
    {
        ...
        CreatedAt = CreatedAt, ArchivedAt = ArchivedAt, Order = Order, Urgent = Urgent
    };
    ...
    {
        foreach (var name in new[] { nameof(DueDate), nameof(FormattedDate), nameof(DaysRemaining), nameof(DaysText), nameof(Status), nameof(EffectivePriority), nameof(CompletionText), nameof(OrderText), nameof(IsTopThree), nameof(IsNextThree), nameof(UrgentText), nameof(StartupAttentionReason), nameof(PositionText) })
