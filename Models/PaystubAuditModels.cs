...
public sealed class PaystubAuditRow : System.ComponentModel.INotifyPropertyChanged
{
    public event System.ComponentModel.PropertyChangedEventHandler? PropertyChanged;
    private bool _verified;
    public bool IsVerified { get => _verified; set { _verified = value; PropertyChanged?.Invoke(this, new(nameof(IsVerified))); PropertyChanged?.Invoke(this, new(nameof(VerificationText))); } }
    public string VerificationText => IsVerified ? "✓ Verificado" : "Marcar verificado";
    public string AuditReviewLabel => $"{ShortRank} {Name}\n{TransportAuditStatus}\n{Year}/{Month:00}";
    public string TransportAuditStatus => HasVacationPayment && AuxPdf > 0 ? "FÉRIAS + AT NO PDF — VERIFICAR" : HasVacationPayment && AuxPdf <= 0 ? "OK · FÉRIAS PAGAS, SEM AT" : AuxStatus;
    public int Month { get; set; }
    public int Year { get; set; }
    public bool HeaderValidated { get; init; }
    public long PdfModifiedTicks { get; init; }
    public double SalaryPdf { get; init; }
    public double TransportShare { get; init; }
    public double? TransportCalculatedNet { get; init; }
    public string TransportShareText => MilitaryFormatting.FormatMoney(TransportShare);
    public string TransportCalculatedNetText => TransportCalculatedNet.HasValue ? MilitaryFormatting.FormatMoney(TransportCalculatedNet.Value) : "Sem bruto cadastrado";
    public bool HasVacationPayment => PdfOk && Vacation > 0;
    public string TransportVacationStatus => !PdfOk ? "Contracheque não disponível"
        : HasVacationPayment ? (AuxPdf <= 0 ? "Férias pagas · sem AT no PDF" : "Férias e AT pagos no mesmo PDF — verificar")
        : "Sem pagamento de férias no PDF";
    public bool CanUpdateTransport(double tolerance) => HeaderValidated && PdfOk && MilitaryId > 0 && Military is { IsAttached: false }
        && PaymentIsNormal && AuxPdf > 0 && AuxDatabase > 0 && AuxDifference.HasValue
        && Math.Abs(AuxDifference.Value) > 0.05 && Math.Abs(AuxDifference.Value) <= tolerance
        && !HasVacationPayment && !AuxStatus.Contains("MANUALMENTE");
    ...
    public bool PaymentSuspended => PaymentStatus.Contains("SUSPENS", StringComparison.OrdinalIgnoreCase);
    public bool PaymentStatusIdentified => !string.IsNullOrWhiteSpace(PaymentStatus)
        && !PaymentStatus.Contains("NÃO IDENTIFIC", StringComparison.OrdinalIgnoreCase)
        && PaymentStatus != "—";
    public bool PaymentIsNormal => PaymentStatus.Equals("NORMAL", StringComparison.OrdinalIgnoreCase)
        || PaymentStatus.Equals("PAGAMENTO NORMAL", StringComparison.OrdinalIgnoreCase);
    public string Summary => BuildSummary();
    public string Severity => !PdfOk || PaymentSuspended || Situation.StartsWith("Erro", StringComparison.OrdinalIgnoreCase) ? "Critical"
        : BankDivergent || PaymentDifferent || (HasVacationPayment && AuxPdf > 0) || AuxStatus.Contains("DIVERG", StringComparison.OrdinalIgnoreCase) || AuxStatus.Contains("NÃO RECEBEU", StringComparison.OrdinalIgnoreCase) ? "Warning"
        : HasFinding ? "Info" : "Ok";
    public bool HasFinding => !PdfOk
        ...
        || BankDivergent || PaymentDifferent || (HasVacationPayment && AuxPdf > 0);
    ...
    public static PaystubAuditRow FromJson(JsonObject json)
    {
        ...
        return new PaystubAuditRow
        {
            ...
            Fusex = Double(json, "fusex"), HasAnyFusexRubric = Bool(json, "has_any_fusex") || Double(json, "fusex") > 0 || Double(json, "dep_fusex") > 0 || Double(json, "med_fusex") > 0,
            DependentFusex = Double(json, "dep_fusex"), MedicalFusex = Double(json, "med_fusex"),
    ...
    private string BuildSummary()
    {
        ...
        if (Fusex <= 0 && PdfOk && !HasAnyFusexRubric) parts.Add("Sem FUSEx 3%");
        if (Fusex <= 0 && PdfOk && HasAnyFusexRubric) parts.Add("FUSEx localizado — conferir classificação");
        ...
        if (Vacation > 0) parts.Add("Rubrica de férias paga");
        if (HasVacationPayment) parts.Add(AuxPdf>0?"Férias e AT pagos no mesmo PDF — verificar":"Férias pagas · sem AT no PDF");
