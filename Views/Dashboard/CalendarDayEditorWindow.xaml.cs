...
public partial class CalendarDayEditorWindow : Window
{
    ...
    private async Task LoadAsync()
    {
        ...
        _redDayStore = await _redDays.LoadAsync();
        LoadRedDayState();
        RefreshDayItems();
        StartNew("Evento");
    }

    private void LoadRedDayState()
    {
        var info = RedDayService.GetInfo(_date, _redDayStore);
        var key = _date.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture);
        var manual = _redDayStore.ManualDays.TryGetValue(key, out var record) && record.IsRed ? record : null;

        RedDayCheckBox.IsChecked = manual is not null;
        RedDayReasonBox.Text = manual?.Reason ?? string.Empty;
        AutomaticRedDayText.Text = info.Kind is RedDayKind.Weekend or RedDayKind.Holiday
            ? "Automático: " + info.TooltipText
            : string.Empty;
    }
    ...
    {
        await SaveRedDaySelectionAsync();
        var type = SelectedType();
        var description = DescriptionBox.Text?.Trim() ?? string.Empty;
        var hasEventData = _current is not null
                           || !string.IsNullOrWhiteSpace(TitleBox.Text)
                           || !string.IsNullOrWhiteSpace(description)
                           || !type.Equals("Evento", StringComparison.OrdinalIgnoreCase);
        if (!hasEventData)
        {
            _redDayStore = await _redDays.LoadAsync();
            LoadRedDayState();
            RefreshDayItems();
            StatusText.Text = "Marcação de dia vermelho salva.";
            DialogResult = true;
            return;
        }
    ...
    private async Task SaveRedDaySelectionAsync()
    {
        var reason = string.IsNullOrWhiteSpace(RedDayReasonBox.Text)
            ? "Dia vermelho"
            : RedDayReasonBox.Text.Trim();
        await _redDays.SetManualRedDayAsync(_date, RedDayCheckBox.IsChecked == true, reason, "Calendário");
    }
