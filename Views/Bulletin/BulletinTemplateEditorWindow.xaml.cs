...
public partial class BulletinTemplateEditorWindow : Window
{
    ...
    private static (string Description, string Example) AutomaticKeyHelp(string key)
    {
        ...
        return key switch
        {
            ...
            "BI_REFERENCIA" => ("Referência completa do BI escolhido nas chaves do SIGFUR.", $"BI Nr 81, de 04 ABR 26, {OrganizationIdentity.BulletinIssuer}"),
            ...
            "ADT_REFERENCIA" => ("Referência completa do Aditamento do Furriel escolhido.", $"Adt Furr Nr 15, de 06 ABR 26, {OrganizationIdentity.BulletinIssuer}"),
    ...
    private void InsertTableField_Click(object sender, RoutedEventArgs e)
    {
        var menu = new ContextMenu();
        var receive = new MenuItem
        {
            Header = "Receber qualquer tabela no preenchimento",
            ToolTip = "Cria somente a chave. A tabela será colada do Word ou Excel quando o boletim for preenchido."
        };
        receive.Click += (_, _) => InsertEmptyTableField();
        var ready = new MenuItem
        {
            Header = "Criar ou deixar tabela pré-preenchida",
            ToolTip = "Reconhece a tabela copiada agora e permite ajustar conteúdo, linhas, colunas e largura."
        };
        ready.Click += (_, _) => InsertPrefilledTableField();
        menu.Items.Add(receive);
        menu.Items.Add(ready);
        menu.PlacementTarget = sender as UIElement;
        menu.IsOpen = true;
    }

    private void InsertEmptyTableField()
    {
        var key = Ask("Tabela do boletim", "Nome da tabela/chave (ex.: FICHA_FINANCEIRA):");
        if (string.IsNullOrWhiteSpace(key)) return;
        InsertAtCaret($"[[{KeyName(key)}:TABELA=3x3;LARGURA=100;CABECALHO=SIM]]");
    }

    private void InsertPrefilledTableField()
    {
        var recognized = BulletinTableClipboardImporter.TryReadClipboard(out var table, out var source);
        if (!recognized)
        {
            table = BulletinTableCodec.Create();
            source = "Nenhuma tabela foi reconhecida na área de transferência. Cole do Word nesta janela ou monte a tabela manualmente.";
        }

        var editor = new BulletinTableEditorWindow(table, source) { Owner = this };
        if (editor.ShowDialog() != true) return;
        var key = Ask("Tabela do boletim", "Nome da tabela/chave (ex.: FICHA_FINANCEIRA):");
        if (string.IsNullOrWhiteSpace(key)) return;

        var result = editor.Table;
        var data = BulletinTableCodec.EncodeDefinition(BulletinTableCodec.Serialize(result));
        InsertAtCaret($"[[{KeyName(key)}:TABELA={result.Rows}x{result.Columns};LARGURA={result.WidthPercent};CABECALHO={(result.HasHeader ? "SIM" : "NAO")};DADOS={data}]]");
    }
