...
public partial class PaystubCenterWindow : Window
{
    ...
    private readonly ObservableCollection<SippesPersonnelConferenceRow> _sippesConferenceRows = [];
    private readonly int _initialTab;
    private readonly bool _restrictedToSelection;
    private readonly bool _licensedTransferredScope;
    ...
    public PaystubCenterWindow(
        ...
        int initialTab = 0, bool restrictedToSelection = false, bool licensedTransferredScope = false)
    {
        InitializeComponent();
        ...
        _initialTab = Math.Clamp(initialTab, 0, 4);
        _restrictedToSelection = restrictedToSelection;
        _licensedTransferredScope = licensedTransferredScope;
        BindMilitaryLists();
        ExternalPeopleGrid.ItemsSource = _externalPeople;
        SippesConferenceGrid.ItemsSource = _sippesConferenceRows;
    }
    ...
    private void BindMilitaryLists()
    {
        ...
        ScopeText.Text = _licensedTransferredScope
            ? $"Escopo: {_military.Count} licenciado(s)/transferido(s) selecionado(s)"
            : _restrictedToSelection
    ...
    private async Task LoadSettingsAsync()
    {
        ...
        _settings.OutputDirectory = PreferredPaystubRoot();
    ...
    private async Task<CpexPaystubSettings> BuildNativeSettingsAsync(int year, int month, string? sheetCode, bool external)
    {
        ...
        settings.SheetCode = string.IsNullOrWhiteSpace(sheetCode)
            ...
            : sheetCode.Trim();
        settings.OutputDirectory = PreferredPaystubRoot();
        OutputFolderBox.Text = settings.OutputDirectory;
    ...
    private async void ConferenceSippes_Click(object sender, RoutedEventArgs e)
    {
        await RunAsync("Conferindo efetivo no SIPPES...", async () =>
        {
            await SaveSettingsAsync();
            var progress = new Progress<CpexPaystubProgress>(x =>
            {
                var prefix = x.Total > 0 ? $"{x.Current}/{x.Total} — " : string.Empty;
                StatusText.Text = prefix + x.Message;
            });

            var sippesRows = await App.CpexPaystubs.ReadSippesActivePersonnelAsync(progress);
            if (sippesRows.Count == 0)
                throw new InvalidOperationException("O SIPPES abriu a consulta, mas o SIGFUR não conseguiu ler nenhuma linha da tabela. A conferência foi interrompida para não gerar falso alerta de que todos os ativos estão fora da folha.");
            var active = _military.Count > 0 ? _military.ToList() : await _repository.GetAllAsync();
            var licensedTransferred = await App.LicensedTransferred.GetAllAsync(includeHidden: true);
            var (rows, summary) = SippesPersonnelConferenceService.Build(sippesRows, active, licensedTransferred);

            _sippesConferenceRows.Clear();
            foreach (var row in rows) _sippesConferenceRows.Add(row);
            SippesConferenceSummaryText.Text = summary.Display + $" | Gerado às {summary.GeneratedAt:dd/MM/yyyy HH:mm}";
            StatusText.Text = $"Conferência SIPPES concluída: {_sippesConferenceRows.Count} linha(s) de análise.";
        }, RunSippesConferenceButton);
    }

    private async void ExportSippesConference_Click(object sender, RoutedEventArgs e)
    {
        if (_sippesConferenceRows.Count == 0)
        {
            SigfurDialog.Show(this, "Execute a conferência SIPPES antes de exportar.", "SIGFUR", MessageBoxButton.OK, MessageBoxImage.Information);
            return;
        }

        var dialog = new SaveFileDialog
        {
            Title = "Exportar conferência SIPPES",
            Filter = "CSV|*.csv",
        ...
        {
            "SITUACAO;GRAVIDADE;PG;NOME;CPF;IDT_CADASTRO;OM_SIPPES;CADASTRO_SIGFUR;CONFERENCIA"
        };
        lines.AddRange(_sippesConferenceRows.Select(x => string.Join(';', new[]
        {
            SippesPersonnelConferenceService.CsvCell(x.Status), SippesPersonnelConferenceService.CsvCell(x.Severity), SippesPersonnelConferenceService.CsvCell(x.DisplayRank), SippesPersonnelConferenceService.CsvCell(x.DisplayName), SippesPersonnelConferenceService.CsvCell(x.DisplayCpf),
            SippesPersonnelConferenceService.CsvCell(x.DisplayMilitaryId), SippesPersonnelConferenceService.CsvCell(x.SippesOm), SippesPersonnelConferenceService.CsvCell(x.SigfurSource), SippesPersonnelConferenceService.CsvCell(x.MatchKind)
        })));
        await File.WriteAllLinesAsync(dialog.FileName, lines, new UTF8Encoding(true));
        StatusText.Text = $"Conferência exportada: {dialog.FileName}";
    }

    private async void AddSippesOutsideToExternal_Click(object sender, RoutedEventArgs e)
    {
        var candidates = _sippesConferenceRows
            .Where(x => x.Status.Contains("fora do SIGFUR", StringComparison.OrdinalIgnoreCase))
            .Where(x => MilitaryFormatting.Digits(x.SippesCpf).Length == 11)
            .ToList();
        if (candidates.Count == 0)
        {
            SigfurDialog.Show(this, "Não há pessoa 'fora do SIGFUR' com CPF válido para enviar à lista de Pessoas de fora.", "SIGFUR", MessageBoxButton.OK, MessageBoxImage.Information);
            return;
        }
        ...
        foreach (var item in candidates)
        {
            ...
            if (_externalPeople.Any(x => MilitaryFormatting.Digits(x.Cpf) == cpf)) continue;
            _externalPeople.Add(new ExternalPaystubPerson { Name = item.SippesName, Cpf = cpf });
            added++;
        }
        await SaveExternalPeopleAsync();
        StatusText.Text = added == 0
            ? "Todas as pessoas fora do SIGFUR já estavam na lista de Pessoas de fora."
            : $"{added} pessoa(s) enviadas para Pessoas de fora.";
    }

    private static (List<SippesPersonnelConferenceRow> Rows, SippesPersonnelConferenceSummary Summary) BuildSippesConference(
        ...
        IReadOnlyList<LicensedTransferredRecord> licensedTransferred)
    {
        ...
        var activeByName = active.GroupBy(x => NormalizePersonName(x.Name)).Where(x => !string.IsNullOrWhiteSpace(x.Key)).ToDictionary(x => x.Key, x => x.First(), StringComparer.OrdinalIgnoreCase);

        var ltByCpf = BuildLookup(licensedTransferred, x => x.Cpf);
        var ltByIdt = BuildLookup(licensedTransferred, x => x.MilitaryId);
        var ltByName = licensedTransferred.GroupBy(x => NormalizePersonName(x.Name)).Where(x => !string.IsNullOrWhiteSpace(x.Key)).ToDictionary(x => x.Key, x => x.First(), StringComparer.OrdinalIgnoreCase);
        ...
        foreach (var sp in sippesRows)
        {
            ...
            if (TryGet(activeByCpf, cpf, out var activeMatch) || TryGet(activeByIdt, idt, out activeMatch))
            {
                ...
                rows.Add(BuildRow(sp, "OK — ativo no SIGFUR e no SIPPES", "OK", "Ativos", activeMatch.Rank, activeMatch.Name, activeMatch.Cpf, activeMatch.MilitaryId, "Documento conferido por CPF/IDT", 90));
                continue;
            }

            if (TryGet(ltByCpf, cpf, out var ltMatch) || TryGet(ltByIdt, idt, out ltMatch))
            {
                rows.Add(BuildRow(sp, "Recebendo — consta em Lic./Transf.", "ALERTA", "Licenciados/Transferidos", ltMatch.Rank, ltMatch.Name, ltMatch.Cpf, ltMatch.MilitaryId, "Documento conferido por CPF/IDT", 10));
                continue;
            }

            if (activeByName.TryGetValue(nameKey, out activeMatch))
            {
                matchedActiveIds.Add(activeMatch.Id);
                nameOnly++;
            ...
            if (ltByName.TryGetValue(nameKey, out ltMatch))
            {
                nameOnly++;
                rows.Add(BuildRow(sp, "Recebendo — nome bate com Lic./Transf.", "ALERTA", "Licenciados/Transferidos", ltMatch.Rank, ltMatch.Name, ltMatch.Cpf, ltMatch.MilitaryId, "Somente nome; conferir CPF/IDT", 20));
                continue;
            }

            rows.Add(BuildRow(sp, "Recebendo e fora do SIGFUR", "CRÍTICO", "Não localizado", string.Empty, string.Empty, string.Empty, string.Empty, "Não localizado em Ativos nem em Lic./Transf.", 0));
        }

        var sippesKeys = new HashSet<string>(sippesRows.SelectMany(x => new[] { "CPF:" + MilitaryFormatting.Digits(x.Cpf), "IDT:" + MilitaryFormatting.Digits(x.MilitaryId), "NOME:" + NormalizePersonName(x.Name) }).Where(x => !x.EndsWith(':')), StringComparer.OrdinalIgnoreCase);
        foreach (var person in active)
        {
            if (matchedActiveIds.Contains(person.Id)) continue;
            var keys = new[] { "CPF:" + MilitaryFormatting.Digits(person.Cpf), "IDT:" + MilitaryFormatting.Digits(person.MilitaryId), "NOME:" + NormalizePersonName(person.Name) }.Where(x => !x.EndsWith(':')).ToList();
            ...
            rows.Add(new SippesPersonnelConferenceRow
            {
                ...
                MatchKind = "Pode não estar recebendo pela OM / conferir folha",
                SortPriority = 30
            });
        }
    ...
    private static string NormalizePersonName(string? value)
    {
        ...
        text = Regex.Replace(text, @"^(?:SD\s+RCR|SD\s+EV|SD\s+EF|SOLDADO|CB|CABO|ASP\s+OF|CAP|MAJ|TEN\s+CEL|CEL|\d+º?\s+TEN|\d+º?\s+SGT)\s+", string.Empty, RegexOptions.IgnoreCase);
    ...
    private async void OpenLatest_Click(object sender, RoutedEventArgs e)
    {
        ...
        await RunAsync($"Abrindo contracheque de {month:00}/{year} de {selected.WarName}…", async () =>
        {
            var person = ToPerson(selected);
            var settings = await BuildNativeSettingsAsync(year, month, IndividualCodeBox.Text, external: false);
            var existing = await FindExistingPaystubForOpenAsync(selected, person, year, month, settings.OutputDirectory);
            if (!string.IsNullOrWhiteSpace(existing) && File.Exists(existing))
            {
                ShellService.OpenPath(existing);
                StatusText.Text = $"Aberto contracheque salvo de {month:00}/{year}: {Path.GetFileName(existing)}";
                IndividualResultBox.Text = $"Contracheque já salvo e aberto.\n\n{existing}";
                return;
            }

            var password = ResolvePassword(external: false);
            var log = new StringBuilder()
                .AppendLine($"Nenhum contracheque de {month:00}/{year} foi localizado na pasta do militar.")
                .AppendLine("Baixando automaticamente pelo CPEX/SIPPES...")
                .AppendLine();
            IndividualResultBox.Text = log.ToString();
            ...
            var progress = new Progress<CpexPaystubProgress>(p =>
            {
                ...
                if (!string.IsNullOrWhiteSpace(p.Name))
                {
                    log.AppendLine($"[{p.Current}/{p.Total}] {p.Name}");
                    IndividualResultBox.Text = log.ToString();
                    IndividualResultBox.ScrollToEnd();
                }
            });

            var result = await App.CpexPaystubs.DownloadPreparedAsync([person], settings, password, progress, writeFailureReport: false);
            _paystubs.InvalidateCache();
            var downloaded = result.DownloadedFiles.FirstOrDefault(x => !string.IsNullOrWhiteSpace(x) && File.Exists(x));
            downloaded = !string.IsNullOrWhiteSpace(downloaded) && File.Exists(downloaded)
                ? downloaded
                : await FindExistingPaystubForOpenAsync(selected, person, year, month, settings.OutputDirectory);

            if (string.IsNullOrWhiteSpace(downloaded) || !File.Exists(downloaded))
            {
                var failure = result.Failures.FirstOrDefault();
                IndividualResultBox.Text = BuildDownloadSummary(result, [], settings.OutputDirectory, year, month);
                throw new InvalidOperationException(string.IsNullOrWhiteSpace(failure)
                    ? $"Não foi possível baixar o contracheque de {month:00}/{year}."
                    : failure);
            }

            ShellService.OpenPath(downloaded);
            IndividualResultBox.Text = $"Contracheque baixado, salvo e aberto.\n\n{downloaded}";
            StatusText.Text = $"Contracheque de {month:00}/{year} salvo e aberto: {Path.GetFileName(downloaded)}";
        });
    }

    private async Task<string?> FindExistingPaystubForOpenAsync(
        MilitaryRecord military,
        CpexPaystubPerson person,
        int year,
        int month,
        string outputDirectory)
    {
        var existing = await FindExistingPaystubsAsync([person], year, month, outputDirectory);
        var path = existing.FirstOrDefault()?.Path;
        if (!string.IsNullOrWhiteSpace(path) && File.Exists(path)) return path;

        path = await _paystubs.FindBestInDirectoryAsync(military, month, year, outputDirectory);
    ...
    private async void OpenLatestFinancialStatement_Click(object sender, RoutedEventArgs e)
    {
        ...
        await RunAsync($"Abrindo ficha financeira de {year} de {selected.WarName}...", async () =>
        {
            var person = ToPerson(selected);
            var settings = await BuildNativeSettingsAsync(year, DateTime.Today.Month, null, external: false);
            var existing = await FindExistingFinancialStatementForOpenAsync(selected, person, year, settings.OutputDirectory);
            if (!string.IsNullOrWhiteSpace(existing) && File.Exists(existing))
            {
                ShellService.OpenPath(existing);
                StatusText.Text = $"Aberta ficha financeira salva de {year}: {Path.GetFileName(existing)}";
                IndividualResultBox.Text = $"Ficha financeira ja salva e aberta.\n\n{existing}";
                return;
            }

            var password = ResolvePassword(external: false);
            var log = new StringBuilder()
                .AppendLine($"Nenhuma ficha financeira de {year} foi localizada na pasta do militar.")
                .AppendLine("Baixando automaticamente pela Area UA/CPEx...")
                .AppendLine();
            IndividualResultBox.Text = log.ToString();
            ...
            var result = await App.CpexPaystubs.DownloadFinancialStatementsPreparedAsync([person], settings, year, password, progress, writeFailureReport: false);
            _paystubs.InvalidateCache();
            var downloaded = result.DownloadedFiles.FirstOrDefault(x => !string.IsNullOrWhiteSpace(x) && File.Exists(x));
            downloaded = !string.IsNullOrWhiteSpace(downloaded) && File.Exists(downloaded)
                ? downloaded
                : await FindExistingFinancialStatementForOpenAsync(selected, person, year, settings.OutputDirectory);

            if (string.IsNullOrWhiteSpace(downloaded) || !File.Exists(downloaded))
            {
                var failure = result.Failures.FirstOrDefault();
                IndividualResultBox.Text = BuildFinancialStatementSummary(result, [], settings.OutputDirectory, year);
                throw new InvalidOperationException(string.IsNullOrWhiteSpace(failure)
                    ? $"Nao foi possivel baixar a ficha financeira de {year}."
                    : failure);
            }

            ShellService.OpenPath(downloaded);
            IndividualResultBox.Text = $"Ficha financeira baixada, salva e aberta.\n\n{downloaded}";
            AutomationStatusText.Text = $"Sessao oculta CPEx preparada as {App.CpexPaystubs.PreparedAt:HH:mm}";
            AutomationDetailText.Text = "Ficha financeira baixada direto pela Central, sem abrir a Carteira.";
            StatusText.Text = $"Ficha financeira de {year} salva e aberta: {Path.GetFileName(downloaded)}";
        });
    }

    private async Task<string?> FindExistingFinancialStatementForOpenAsync(
        MilitaryRecord military,
        CpexPaystubPerson person,
        int year,
        string outputDirectory)
    {
        var existing = await FindExistingFinancialStatementsAsync([person], year, outputDirectory);
        var path = existing.FirstOrDefault()?.Path;
        if (!string.IsNullOrWhiteSpace(path) && File.Exists(path)) return path;

        path = await _paystubs.FindBestFinancialStatementInDirectoryAsync(military, year, outputDirectory);
        return !string.IsNullOrWhiteSpace(path) && File.Exists(path) ? path : null;
    }
    ...
    private async void OpenSaved_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (selected is null) { WarnSelection(); return; }
        Window window;
        if (_licensedTransferredScope)
        {
            var record = await App.LicensedTransferred.GetByIdAsync(selected.Id);
            if (record is null)
            {
                SigfurDialog.Show(this, "O registro de licenciado/transferido não foi localizado.", "Contracheques salvos", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            window = new LicensedTransferredWalletWindow(App.LicensedTransferred, _paystubs, record, 1) { Owner = this };
        }
        else window = new MilitaryWalletWindow(_repository, _paystubs, selected, 6) { Owner = this };
        window.Show();
        window.Activate();
    }
    ...
    private async void OpenSavedFinancialStatements_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (_licensedTransferredScope)
        {
            ...
            if (record is null)
            {
                SigfurDialog.Show(this, "O registro de licenciado/transferido não foi localizado.", "Fichas financeiras salvas", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
            window = new LicensedTransferredWalletWindow(App.LicensedTransferred, _paystubs, record, 2) { Owner = this };
        }
        else window = new MilitaryWalletWindow(_repository, _paystubs, selected, 7) { Owner = this };
        window.Show();
        window.Activate();
    }
    ...
    private async void ExportMonth_Click(object sender, RoutedEventArgs e)
    {
        ...
        await RunAsync("Pesquisando e exportando PDFs salvos…", async () =>
        {
            ...
            var destination = dialog.CopyForSharing || dialog.ShareOnWhatsApp
                ? GetPaystubClipboardFolder(dialog.Month, dialog.Year)
                : dialog.Folder;
            var result = await _paystubs.ExportAsync(_military, dialog.Month, dialog.Year, destination, progress);
            var copied = 0;
            WhatsAppQuickShareResult? shareResult = null;
            if (dialog.CopyForSharing && result.Files.Count > 0)
            {
                copied = PaystubClipboardService.CopyPackage(
                    result.Files,
                    dialog.ShareMessage,
                    $"Contracheques - {dialog.Month:00}-{dialog.Year}.zip");
                StatusText.Text = $"Pacote copiado: {copied} contracheque(s) e mensagem. Abra a conversa e pressione Ctrl+V.";
            }
            else if (dialog.CopyForSharing)
            {
                StatusText.Text = "Nenhum contracheque foi encontrado. O clipboard não foi alterado.";
            }
            else if (dialog.ShareOnWhatsApp && result.Files.Count > 0)
            {
                shareResult = await WhatsAppQuickShareService.PasteAsync(
                    ...
                    dialog.Month,
                    dialog.Year);
                StatusText.Text = shareResult.UsedArchive
            ...
            else if (dialog.ShareOnWhatsApp)
            {
                StatusText.Text = "Nenhum contracheque foi encontrado para compartilhar.";
            }

            if (dialog.ShareOnWhatsApp && result.Files.Count > 0)
            {
                if (result.Failures.Count > 0)
                    StatusText.Text += $" {result.Failures.Count} militar(es) com falha não foram incluídos.";
                return;
            }

            var operationSummary = dialog.ShareOnWhatsApp
                ? result.Files.Count > 0
                    ? $"Compartilhamento rápido.\n\nContracheques preparados: {result.Files.Count}\nFalhas: {result.Failures.Count}"
                    : $"Nenhum contracheque foi encontrado para {dialog.Month:00}/{dialog.Year}.\n\nFalhas: {result.Failures.Count}"
                : dialog.CopyForSharing
                    ? copied > 0
            ...
            SigfurDialog.Show(this,
                operationSummary,
                dialog.ShareOnWhatsApp ? "Compartilhar contracheques" : "Exportar contracheques", MessageBoxButton.OK,
