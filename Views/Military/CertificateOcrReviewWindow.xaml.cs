...
public partial class CertificateOcrReviewWindow : Window
{
    ...
    [
        ...
        ("HORARIO_NASCIMENTO", "Horário do nascimento"),
        ("LOCAL_NASCIMENTO", "Local do nascimento"),
        ("MUNICIPIO_NATURALIDADE", "Município de naturalidade"),
        ("MUNICIPIO_NASCIMENTO", "Município do nascimento"),
        ("UF_NASCIMENTO", "UF do nascimento"), ("DNV", "Declaração de Nascido Vivo (DNV)"),
        ("AVOS_PATERNOS", "Avós paternos"), ("AVOS_MATERNOS", "Avós maternos"),
        ("CARTORIO", "Cartório"), ("LOCAL_CERTIDAO", "Local da certidão"),
        ("ENDERECO_CARTORIO", "Endereço do cartório"),
        ("NOME_OFICIAL_CARTORIO", "Oficial do cartório"), ("CNS_CARTORIO", "CNS do cartório")
    ];
    ...
    private void OpenOriginal_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            ShellService.OpenPath(_sourcePath);
            return;
        }
        SigfurDialog.Show(this, "O arquivo original não está mais disponível neste local.", "Abrir certidão", MessageBoxButton.OK, MessageBoxImage.Warning);
    }

    private void CopyRawText_Click(object sender, RoutedEventArgs e)
    {
        if (string.IsNullOrWhiteSpace(RawTextBox.Text)) return;
