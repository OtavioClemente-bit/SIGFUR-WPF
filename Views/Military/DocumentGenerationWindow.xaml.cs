...
public partial class DocumentGenerationWindow : Window
{
        ...
        GeneratedDocumentType initialType = GeneratedDocumentType.TransportAid)
    {
        ...
        TemplateFolderText.Text = $"Pasta única de templates do SIGFUR: {App.Paths.UnifiedTemplatesDirectory}";
    ...
    private static async Task EnsureBundledTemplatesAvailableAsync()
    {
        try
        {
            ...
            var bundled = Path.Combine(AppContext.BaseDirectory, "templates", "docs");
            ...
            foreach (var source in Directory.EnumerateFiles(bundled, "*.*", SearchOption.TopDirectoryOnly))
            {
                ...
                if (File.Exists(target) && await FilesEqualAsync(source, target)) continue;
                if (File.Exists(target))
                {
                    var backupDirectory = Path.Combine(App.Paths.DocumentTemplatesDirectory, "Backups_Atualizacao");
                    Directory.CreateDirectory(backupDirectory);
                    var backup = Path.Combine(backupDirectory, $"{DateTime.Now:yyyyMMdd_HHmmss}_{Path.GetFileName(target)}");
    ...
    private static async Task<bool> FilesEqualAsync(string first, string second)
    {
        var leftInfo = new FileInfo(first);
        var rightInfo = new FileInfo(second);
        ...
        while (true)
        {
            var leftCount = await left.ReadAsync(leftBuffer);
            var rightCount = await right.ReadAsync(rightBuffer);
            if (leftCount != rightCount) return false;
            if (leftCount == 0) return true;
            if (!leftBuffer.AsSpan(0, leftCount).SequenceEqual(rightBuffer.AsSpan(0, rightCount))) return false;
        }
    }
    ...
    private async Task SelectTypeAsync(GeneratedDocumentType type)
    {
        try
        {
            ...
            foreach (var field in MilitaryDocumentGenerationService.FieldsFor(type))
            {
                ...
                DynamicFormPanel.Children.Add(IsProfessionalPublicationField(field.Key)
                    ? CreatePublicationEditor(field.Key, control)
                    : control);
            }
    ...
    private FrameworkElement CreatePublicationEditor(string key, FrameworkElement control)
    {
        control.ToolTip = "Escolha uma publicação salva ou digite a referência manualmente.";
        var panel = new StackPanel();
        panel.Children.Add(control);
        var button = new Button
        {
            Content = "Escolher BI / Adt Furr salvo",
            Tag = key,
            MinWidth = 210,
            ...
            ToolTip = "Consulta os Boletins Internos e Aditamentos do Furriel já salvos no SIGFUR."
        };
        button.Click += ChooseSavedPublication_Click;
        panel.Children.Add(button);
        return panel;
    }

    private static bool IsProfessionalPublicationField(string key)
        => key.Equals("PUBLICACAO_LICENCIAMENTO", StringComparison.OrdinalIgnoreCase)
           || key.Equals("PUBLICACAO_FERIAS", StringComparison.OrdinalIgnoreCase);

    private async void ChooseSavedPublication_Click(object sender, RoutedEventArgs e)
    {
        if (sender is not Button { Tag: string fieldKey }) return;
        ...
        var formatted = FormatProfessionalPublication(reference);
        SetField(fieldKey, formatted);
        await SaveCurrentFormCacheAsync();
        StatusText.Text = $"Publicação aplicada: {formatted}";
    }

    private static string FormatProfessionalPublication(SavedBulletinReference reference)
        => reference.ReferenceText;

    private static string PublicationNumberWithoutYear(string? value)
    {
        var text = Regex.Replace((value ?? string.Empty).Trim(), @"\b(?<num>\d{1,5})\s*/\s*(?:20)?\d{2}\b",
            match => match.Groups["num"].Value, RegexOptions.CultureInvariant);
        var match = Regex.Match(text, @"\d{1,5}", RegexOptions.CultureInvariant);
        return match.Success ? match.Value : text;
    }

    private static string AbbreviatedMilitaryDate(string? value)
    {
        var text = (value ?? string.Empty).Trim();
        if (DateTime.TryParse(text, CultureInfo.GetCultureInfo("pt-BR"), DateTimeStyles.AllowWhiteSpaces, out var date))
        ...
        return Regex.Replace(text, @"\s+", " ").Replace(".", string.Empty).ToUpper(CultureInfo.GetCultureInfo("pt-BR"));
    }
    ...
    private async Task LoadTransportDefaultsAsync(GeneratedDocumentType type)
    {
        ...
        try
        {
            ...
            {
                defaults.GetValueOrDefault("ENDERECO1", string.Empty),
                defaults.GetValueOrDefault("ENDERECO2", string.Empty)
            }.Where(IsRegisteredTransportValue)).Trim();
            var buses = Enumerable.Range(1, 3)
                .Select(index => defaults.GetValueOrDefault($"ONIBUS{index}_NR", string.Empty))
                .Where(IsRegisteredTransportValue)
                .Distinct(StringComparer.CurrentCultureIgnoreCase)
                .ToList();

            var addressText = UseTransportAddressCheck.IsChecked == true
                ? $"Endereço cadastrado: {(string.IsNullOrWhiteSpace(address) ? "NENHUM ENDEREÇO CADASTRADO" : address)}"
                : "Endereço: preenchimento automático desativado";
            var busText = UseTransportBusesCheck.IsChecked == true
                ? $"Ônibus cadastrados: {(buses.Count == 0 ? "NENHUM ÔNIBUS CADASTRADO" : string.Join(" | ", buses))}"
                : "Ônibus: preenchimento automático desativado";
            TransportOptionsStatusText.Text = $"Posto/graduação: {fullRank}\n{addressText}\n{busText}";
    ...
    private static bool IsRegisteredTransportValue(string? value)
        => !string.IsNullOrWhiteSpace(value)
           && value.Any(character => character != '_' && character != '-' && !char.IsWhiteSpace(character));
    ...
    private async void Generate_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            {
                // Uma SAT recém-gerada entra sempre com uma única cópia. A fila
                // geral pode guardar outra quantidade para trabalhos avulsos, mas
                // isso não deve multiplicar silenciosamente este documento.
                var printQueue = new PrintQueueWindow(
                    result.Files,
                    initialCopies: 1,
                    forceSingleCopy: _selectedType == GeneratedDocumentType.TransportAid)
                { Owner = this };
