...
internal sealed class ExercisePreviousProtocolWindow : Window
{
    ...
    private Task ReadProtocolAsync()
    {
        try
        {
            ...
            var data = _service.ExtractProtocolDataFromPdf(_pdf.Text);
            if (string.IsNullOrWhiteSpace(data.Protocol)) throw new InvalidOperationException("Não encontrei o número do protocolo no texto interno do PDF. Digite-o manualmente.");
            ApplyProtocolData(data);
            _status.Text = string.IsNullOrWhiteSpace(data.ProtocolledAt)
                ? "Protocolo lido do PDF: " + data.Protocol
                : $"Protocolo e data lidos do PDF: {data.Protocol} — {data.ProtocolledAt}";
        }
        catch (Exception ex) { ShowError(ex); }
        return Task.CompletedTask;
    }
    ...
    {
        var data = _service.ExtractProtocolDataFromPdf(source);
        ApplyProtocolData(data);
        ...
        _pdf.Text = archived;
        _status.Text = !string.IsNullOrWhiteSpace(data.Protocol)
            ? $"PDF arquivado: {Path.GetFileName(archived)} — protocolo lido: {data.Protocol}" + (string.IsNullOrWhiteSpace(data.ProtocolledAt) ? string.Empty : $" — data: {data.ProtocolledAt}")
            : $"PDF arquivado: {Path.GetFileName(archived)}";
    }

    private void ApplyProtocolData(CpexProtocolPdfData data)
    {
        if (!string.IsNullOrWhiteSpace(data.Protocol)) _protocol.Text = data.Protocol;
        if (!string.IsNullOrWhiteSpace(data.ProtocolledAt)) _date.Text = data.ProtocolledAt;
    }

    private void Save()
    {
        var protocol = _protocol.Text.Trim();
        if (string.IsNullOrWhiteSpace(protocol) && File.Exists(_pdf.Text))
        {
            var data = _service.ExtractProtocolDataFromPdf(_pdf.Text);
...
{
    private readonly IReadOnlyList<ExercisePreviousProcess> _processes;
    private readonly Func<IReadOnlyList<ExercisePreviousProcess>, string, string> _saveText;
    private readonly string _dialogCaption;
    private readonly string _copyConfirmation;
    ...
    public ExercisePreviousBulletinWindow(ExercisePreviousProcess process, ExercisePreviousProtocolService service, string text)
        : this(new[] { process }, text,
            "Boletim EA CPEx — Prévia",
            "Prévia do boletim — Exercícios Anteriores",
            "Boletim EA",
            "Texto copiado com nomes de guerra e campos principais em negrito.",
            (items, body) => service.SaveBulletinText(items, body),
            [])
    {
    }

    public ExercisePreviousBulletinWindow(IEnumerable<ExercisePreviousProcess> processes, ExercisePreviousProtocolService service, string text)
        : this(processes, text,
            "Boletim EA CPEx — Prévia",
            "Prévia do boletim — Exercícios Anteriores",
            "Boletim EA",
    ...
    public ExercisePreviousBulletinWindow(
        ExercisePreviousProcess process,
        ExercisePreviousDocumentService documents,
        string text,
        IReadOnlyList<string> missingFields)
        : this(new[] { process }, text,
            "Requerimento BI — Prévia",
            "Nota para o BI — entrada do requerimento de Exercício Anterior",
            "Requerimento BI",
            "Nota copiada para criação do boletim, com os campos principais em negrito.",
            (items, body) => documents.SaveBulletinRequestText(items[0], body),
            missingFields)
    {
    }

    private ExercisePreviousBulletinWindow(
        IEnumerable<ExercisePreviousProcess> processes,
        string text,
        string windowTitle,
        string heading,
        string dialogCaption,
        string copyConfirmation,
        Func<IReadOnlyList<ExercisePreviousProcess>, string, string> saveText,
        IReadOnlyList<string> missingFields)
    {
        _processes = processes.Where(x => x is not null).ToList();
        _saveText = saveText;
        _dialogCaption = dialogCaption;
        _copyConfirmation = copyConfirmation;
        Title = windowTitle; Width = 980; Height = 680; MinWidth = 760; MinHeight = 500; WindowStartupLocation = WindowStartupLocation.CenterOwner;
        var root = new DockPanel { Margin = new Thickness(14) };
        var title = new TextBlock { Text = heading, FontSize = 18, FontWeight = FontWeights.SemiBold, Margin = new Thickness(0, 0, 0, 8) };
        ...
        if (missingFields.Count > 0)
        {
            var warning = new Border
            {
                ...
                BorderBrush = new SolidColorBrush(Color.FromRgb(217, 160, 45)),
                ...
                Child = new TextBlock
                {
                    Text = "Revisão necessária antes de publicar: " + string.Join(", ", missingFields) + ".",
                    TextWrapping = TextWrapping.Wrap,
                    FontWeight = FontWeights.SemiBold
                }
            };
            DockPanel.SetDock(warning, Dock.Top); root.Children.Add(warning);
        }
        var roster = BuildRoster();
        DockPanel.SetDock(roster, Dock.Top); root.Children.Add(roster);
        ...
        var copy = Button("Copiar texto"); copy.Click += (_, _) =>
        {
            ...
            {
                Text = _text.Text,
                BoldRanges = BuildBoldRanges(_text.Text),
                UnresolvedTokens = []
            };
            ...
            SigfurDialog.Show(this, _copyConfirmation, _dialogCaption, MessageBoxButton.OK, MessageBoxImage.Information);
        };
        var save = Button("Salvar .txt"); save.Click += (_, _) =>
        {
            try
            {
                var path = _saveText(_processes, _text.Text);
                SigfurDialog.Show(this, "Arquivo salvo:\n" + path, _dialogCaption, MessageBoxButton.OK, MessageBoxImage.Information);
            }
            catch (Exception ex)
            {
                SigfurDialog.Show(this, ex.Message, _dialogCaption, MessageBoxButton.OK, MessageBoxImage.Error);
            }
        };
    ...
    private UIElement BuildRoster()
    {
        ...
        foreach (var process in _processes)
        {
            var chip = new Border
            {
                ...
                ToolTip = BuildProcessToolTip(process)
            };
            chip.Child = new TextBlock
            {
                Text = BuildChipText(process),
    ...
    private List<BulletinBoldRange> BuildBoldRanges(string text)
    {
        var ranges = BulletinTextFormatter.FindWarNameRanges(text, _processes.Select(x => (x.FullName, x.WarName)));
        AddPhraseRanges(text, ranges,
            "EXERCÍCIO ANTERIOR - Ordem de Saque:",
            "Militar:",
            "CPF:",
            "Prec-CP:",
            "Protocolo CPEx:",
            "Protocolo CPEx/SIPPES:",
            "Período da dívida:",
            "Documento que materializou o direito:",
            "Posto/Graduação:",
            "Nome de guerra:",
            "Nome completo:",
            "Data do protocolo:",
            "Tipo/código do exercício anterior:",
            "Rubrica e valor a receber:",
            "Rubricas e valores a receber:",
            "Valor bruto devido:",
            "Valor corrigido/total solicitado:",
            "Justificativa do não pagamento à época:",
            "Rubricas a implantar:",
            "Valor original:",
            "Valor corrigido:",
            "EXERCÍCIO ANTERIOR - Entrada de requerimento:",
            "Valor requerido/apurado até esta fase:",
            "Classificação informada no processo:",
            "Atualização monetária informada até:",
            "Síntese apresentada para a origem da dívida e o não pagamento na época própria:",
            "Documento que materializa o direito alegado:",
            "Esta publicação registra exclusivamente a entrada do requerimento",
            "EXERCICIO ANTERIOR - ORDEM DE SAQUE INDIVIDUALIZADA",
            "Publicacao individual de exercicio anterior",
            "Justificativa do nao pagamento na epoca propria:",
            "Valores individualizados a implantar para este militar:",
            "Total individual:",
            "Em consequência");
        return ranges;
    }

    private static void AddPhraseRanges(string text, List<BulletinBoldRange> ranges, params string[] phrases)
    {
        foreach (var phrase in phrases.Where(x => !string.IsNullOrWhiteSpace(x)))
        {
            var start = 0;
            while ((start = text.IndexOf(phrase, start, StringComparison.CurrentCultureIgnoreCase)) >= 0)
            {
                ranges.Add(new BulletinBoldRange(start, phrase.Length));
                start += Math.Max(1, phrase.Length);
            }
        }
    }

    private static string BuildChipText(ExercisePreviousProcess process)
        => string.Join(" ", new[] { process.Rank, process.FullName }.Where(x => !string.IsNullOrWhiteSpace(x))).Trim();
    ...
    private static string BuildProcessToolTip(ExercisePreviousProcess process)
    {
        ...
        var pdf = string.IsNullOrWhiteSpace(process.CpexPrintPage) ? "sem PDF vinculado" : process.CpexPrintPage.Trim();
