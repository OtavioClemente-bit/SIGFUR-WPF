...
public partial class BankInconsistencyWindow : Window
{
    ...
    private void BrowseSinfOutput_Click(object sender, RoutedEventArgs e) { var path = Browse(SinfOutputBox.Text); if (path is not null) { SinfOutputBox.Text = path; RefreshSavedReports(); } }
    ...
    private void RefreshSavedReports()
    {
            ...
            try
            {
                foreach (var path in Directory.EnumerateFiles(directory, "*.pdf", SearchOption.AllDirectories))
                {
                    try
                    {
                        ...
                        var source = info.Name.StartsWith("Lancamentos_Criticados_", StringComparison.OrdinalIgnoreCase)
                                     || path.StartsWith(App.Paths.SinfoppesCriticizedOutputDirectory, StringComparison.OrdinalIgnoreCase)
        ...
        var sinfoppesReports = ordered
            .Where(x => x.Source.Equals("SINFOPPES", StringComparison.OrdinalIgnoreCase)
                        && x.Name.StartsWith("Lancamentos_Criticados_", StringComparison.OrdinalIgnoreCase))
            .ToList();
        SinfSavedPdfsGrid.ItemsSource = sinfoppesReports;
        SinfSavedPdfsSummaryText.Text = sinfoppesReports.Count == 0
            ? "Nenhum PDF do SINFOPPES salvo ainda."
            : $"{sinfoppesReports.Count} relatório(s), do mais recente para o mais antigo.";
    }
    ...
    private static string InferCompetence(string name)
    {
        ...
        match = Regex.Match(name, @"(?<!\d)(20\d{2})[_ -]+(janeiro|fevereiro|marco|março|abril|maio|junho|julho|agosto|setembro|outubro|novembro|dezembro)(?:[_ .-]|$)", RegexOptions.IgnoreCase);
        if (!match.Success) return "—";
        var normalizedMonth = match.Groups[2].Value.ToLowerInvariant().Replace('ç', 'c');
        var monthNumber = normalizedMonth switch
        {
        ...
        return monthNumber == 0 ? "—" : $"{monthNumber:00}/{match.Groups[1].Value}";
    }

    private static string InferRun(string name)
    {
        var match = Regex.Match(name, @"(?:^|[_ -])(\d{1,2})(?:a|ª|º)?[_ -]+corrida(?:[_ .-]|$)", RegexOptions.IgnoreCase);
        return match.Success ? $"{match.Groups[1].Value}ª" : "—";
    }
    ...
    private void SinfSavedPdfsGrid_MouseDoubleClick(object sender, System.Windows.Input.MouseButtonEventArgs e)
    {
        if (SinfSavedPdfsGrid.SelectedItem is SavedBankReport item) OpenExisting(item.FullPath);
    }
