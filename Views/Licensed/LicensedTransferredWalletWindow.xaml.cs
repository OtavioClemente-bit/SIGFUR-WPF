...
public partial class LicensedTransferredWalletWindow : Window
{
    ...
    public LicensedTransferredWalletWindow(LicensedTransferredRepository repository, PaystubService paystubs, LicensedTransferredRecord record, int initialTab = 0)
    {
        ...
        PopulateDownloadCompetence();
    }

    private void PopulateDownloadCompetence()
    {
        var years = Enumerable.Range(2000, DateTime.Today.Year - 1998).OrderByDescending(x => x).ToList();
        DownloadPaystubYearBox.ItemsSource = years;
        DownloadFinancialStatementYearBox.ItemsSource = years;
        DownloadPaystubYearBox.SelectedItem = DateTime.Today.Year;
        DownloadFinancialStatementYearBox.SelectedItem = DateTime.Today.Year;
        DownloadPaystubMonthBox.ItemsSource = Enumerable.Range(1, 12)
            .Select(x => $"{x:00} - {new DateTime(2000, x, 1).ToString("MMMM", CultureInfo.GetCultureInfo("pt-BR"))}")
            .ToList();
        DownloadPaystubMonthBox.SelectedIndex = DateTime.Today.Month - 1;
    }
    ...
    {
        if (DownloadPaystubYearBox.SelectedItem is not int year || DownloadPaystubMonthBox.SelectedIndex < 0)
        {
            StatusText.Text = "Selecione o ano e o mês do contracheque.";
            return;
        }
        var month = DownloadPaystubMonthBox.SelectedIndex + 1;
        var military = _record.ToMilitaryRecord();
        ...
        var completionMessage = $"Contracheque {month:00}/{year} baixado e lista atualizada.";
        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(settings.Login) || string.IsNullOrWhiteSpace(password))
            {
                ...
                settings.SheetCode = string.Empty;
            ...
            {
                var progress = new Progress<CpexPaystubProgress>(p => StatusText.Text = p.Message);
                var result = await App.CpexPaystubs.DownloadPaystubForMilitaryAsync(
                    military, year, month, openAfterDownload: true, progress: progress);
                completionMessage = result.Message;
                if (!result.Success)
                    SigfurDialog.Show(this, result.Message, "Baixar contracheque", MessageBoxButton.OK, MessageBoxImage.Warning);
            }
        ...
        catch (Exception ex) { SigfurDialog.Show(this, ex.Message, "Baixar contracheque", MessageBoxButton.OK, MessageBoxImage.Error); }
    }
    ...
    {
        if (DownloadFinancialStatementYearBox.SelectedItem is not int year)
        {
        ...
        try
        {
            ...
            var result = await App.CpexPaystubs.DownloadLatestFinancialStatementForMilitaryAsync(
                _record.ToMilitaryRecord(), year, openAfterDownload: true, progress: progress);
            _paystubs.InvalidateCache();
            Changed = true;
            await ReloadAsync();
            StatusText.Text = result.Message;
            if (!result.Success)
                SigfurDialog.Show(this, result.Message, "Ficha Financeira", MessageBoxButton.OK, MessageBoxImage.Warning);
        }
