...
public partial class ReminderHistoryWindow : Window
{
    ...
    private async void Delete_Click(object sender, RoutedEventArgs e) { if (Selected is null) { StatusText.Text = "Selecione um item."; return; } if (SigfurDialog.Show(this, "Excluir definitivamente a tarefa selecionada?", "Histórico", MessageBoxButton.YesNo, MessageBoxImage.Warning) != MessageBoxResult.Yes) return; await _service.DeleteHistoryAsync(Selected.Id); await LoadAsync(); }
    private async void Export_Click(object sender, RoutedEventArgs e) { if (_records.Count == 0) return; var dialog = new SaveFileDialog { Filter = "Texto|*.txt", FileName = "historico_prioridades.txt" }; if (dialog.ShowDialog(this) != true) return; await _service.ExportTextAsync(dialog.FileName, _records); StatusText.Text = "Histórico exportado."; }
}
