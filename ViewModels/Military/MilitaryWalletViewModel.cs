...
public sealed class MilitaryWalletViewModel : ObservableObject
{
    ...
    private async Task LoadWalletFurrielAddendumMentionsAsync(string normalizedFullName)
    {
        ...
        // A Carteira deve espelhar exatamente a pesquisa oficial do módulo Aditamento Furriel.
        ...
        var results = await Task.Run(() => furrielService.Search(furrielStore, string.Empty, option, null, string.Empty, "Todos", 1000).ToList());
                     ...
                     .ThenByDescending(x => x.Page))
        {
            ...
            // A mesma nota pode vir duplicada do índice do Furriel: uma linha pelo nome
            // e outra pelo CPF/PREC. Na carteira isso deve virar um registro único.
        ...
        // A Carteira usa somente o resultado limpo do módulo Aditamento Furriel.
    ...
    private static string WalletMentionLogicalKey(WalletMentionRecord mention)
    {
        ...
        // ADT Furriel: não duplicar nome + documento da mesma página/nota.
        if (sourceFamily == "ADT_FURRIEL")
